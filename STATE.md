...
## Session 14z-136 — **SLICES H3 AND H4 LANDED: the fingerprint, the suite runner, THE DRIVER CONTRACT,
## THE FAKE MACHINE and THE SWEEP RUNNER — with FIDELITY F4, F6 AND F7 EXACT** (the maintainer's
## "start as planned", then "continue" at 46% context). No build byte moved.

| | |
|---|---|
| opened with | the opener read (CLAUDE.md, STATE, HANDOFF, `docs/NEXT_SESSION.md`), the ROM audit 76/76, then the maintainer: *"start as planned"* — H3 per NEXT_SESSION, `harness_scope.md` §2.3, §2.4 (I2), §3.4, §4's H3 row read first |
| **H3, lifted** | `tools/build_fingerprint.py` → `lib/py/bbh/fingerprint.py`: the PROGRAM key (the program members in member ORDER, from the first image along the `;` search path) and the WHOLE-SET key (every member in the build's own directory — chain-independent by construction), the dual lookup (whole-set first, program loudly with the NOTE) and the UNREGISTERED exit 2, every printed line verbatim; `[fingerprint]` config — `kind` (`zip-members` / `file-sha1` / `command`), `program_member_regex` (group 1 = the member's order), `parent_sets`, `region_rules` + `region_default` for `--full`; no board module imported (the lineage imported `cps2_decrypt` for one regex and hard-named `vsav.zip`). `tests/run_suite.sh` → `bin/bbh-run-suite`: the dispatch loop `.skip` → `.pending` (FAIL) → `.masked` → `.diverge` → `.sha1` → NO-EXPECTATION, `runs_per_replay` with NONDETERMINISTIC on any difference, `--freeze` with the RETIRE of a self-frozen `.diverge`, `SUITE_ONLY` = FILTERED, the hermetic scrub, EVERY verdict line character for character (incl. `NO-EXPECTATION (freeze after review, as a STATE.md decision)` — frozen until H9 by the contract's rule 1); `[suite]` `registry` / `default_set` / `driver` (a path from the root or a bare harness driver name) / `runs_per_replay` / `mask_env` / `rompath_env` / `input_env` / `hermetic_unset` / `hash_cmd`. `drivers/README.md` — THE CONTRACT: `<driver> <set> <replay.rpl> <out.log> [sandbox]`, the replay family (`MASK_RANGES DUMPS POKES SNAP_FRAMES VIDEO_OUT INPUT_OUT TAIL_FRAMES NO_INPUT_CHECK INPUT_INJECT_TEST`), the guard family (`GUARD_* CRASH_VECTORS CODE_RANGES`, guarded drivers only), **the rule: a driver that cannot honour a variable REFUSES (exit 3), never ignores**, the log grammar, the four exit codes. `lib/py/bbh/rpl.py` — the `.rpl` grammar on the python side, pulled forward from H6 because the fake needs a parser (H6 keeps the Lua twin and the equality selftest); its error texts are `replay.lua`'s assertions |
| **THE FAKE MACHINE** (`example/fakesys/fakesys.py`, `drivers/fake.sh`) | 64 KiB of RAM (the whole window; mask offsets from 0), three ports with the lineage's tokens, a frame counter, an input mirror, a PRNG seeded from the ROM, attract → select → match, a 64x64 screen, blake2b-64 per frame (the grammar names no algorithm; §3.4 said FNV-1a64 — corrected in the scope row). Its ROM is `<set>.zip` with `fake.01` (`features=`) and `fake.02` (`seed=`) as the program and `fake.gfx` / `fake.key` as non-program members; the features are the divergence MECHANISMS: `hook` = one byte written a frame late on every player-button press + cycle-cost noise in `$FF80-$FFFF` every frame (the region a consumer masks — under the mask a hooked build equals base, unmasked it differs on every frame, both gated), `select` = one byte held for +60..+159 after 1P start, `attract` = the demo from frame 900 diverging permanently (base `d`, attract `3d+1`: never equal, so the onset is exactly 900 — the first draft's `3d` diverged at 901 with a coincidence at 1028, caught by `describe-shape`); `FAKE_NONDET=1`, `FAKE_CRASH_AT=n` (CRASH/REGS/STACK, END-CRASH, exit 2), `FAKE_BUILD` overrides the ROM. Honours the whole replay family (dumps, pokes, `.ppm` snapshots, a video log, an input log, the INPUT-VIOLATION must-fire); refuses the guard family. 0.2 s for 1,120 frames. Five images from `make_roms.py`, byte-reproducible (`--check`): `base`, `attract`, `build-a` (`hook,select,attract` + gfx v1) and `build-b` (+ gfx v2) — THE DUAL-KEY TWINS, registered by whole-set key — and `hook`, unregistered on purpose. `make_expected.sh` regenerates the whole tree with its provenance: `base` by `--freeze`, the masked basis from two compared runs, `attract` frozen without 05 then the authored `.diverge`, `build-a`'s five `.masked` specs as literals (exact / window 260-359 / flicker 100,250 / composite 220 + 260-359 / diverge 900), a `.skip`, `build-b` a copy. All four registered images SUITE GREEN; `roms/hook` exit 1 loudly |
| **four new selftests** | `test_fingerprint` (§1/1b of the lineage's `test_suite_dispatch` on SYNTHETIC zips: member order, the dual key, silent/loud/precedence, the whole-set-only row unreachable by the program fallback as the must-fire, chain independence, `--full`'s chain dependence and region rules, the consumer regex, `BBH_CONFIG`, the two other kinds, the example's images against their generator and registry); `test_driver_contract` (every row of the contract against `fake.sh`, incl. the refusal, the crash exit, the stale-artifact rule, and three must-fires); `test_suite_dispatch` (every verdict line on the four images, the unregistered image, perturbed `.sha1`/`.masked`/`.diverge` each RED with the exact text, NO-EXPECTATION, PENDING, the mask guard both ways, NONDETERMINISTIC, RUN-FAIL, the hermetic scrub with its must-fire, `--freeze` incl. the RETIRE and the authored-masked exemption, SUITE_ONLY, the driver by name / path / non-executable, the input demand, the search-path fallback — ~40 s, python start-up dominated); `test_rpl` (the grammar, nine malformed shapes with the lineage's reasons, `--sides`, and every lineage replay parses: 181 files). Suite **17 PASS / 0 SKIP / 0 FAIL** |
| **FIDELITY F6 AND F7, MEASURED: EXACT** | **F6**: both fingerprint tools over the same images with every flag (`--sha-only`, `--set-key`, `--full`, the lookup with four registries), stdout + stderr + exit diffed — the synthetic dual-key twin (28 invocations) and, with `ROMDIR`, `$ROMDIR` plus build dirs (`BBH_FIDELITY_F6=all` = every `build/*/rompath`, WIDE and stock, and the `;` chain): identical. **F7**: both SUITE runners over this tree's REAL expectation trees, no MAME — a shadow root with `tests/` and `tools/` symlinked in, a STUB driver that copies a sibling set's frozen log (so most verdicts are FAILs with numbers in them) or the set's own (the PASS shapes), copies of `build_fingerprint.py`/`cps2_decrypt.py` so `__file__` resolves inside, one fabricated zip per set in a synthetic registry — every verdict line identical (`BBH_FIDELITY_F7=all` = every set; the default three cover `.pending`, `.skip`, every masked class and `.sha1`). Both kept in `selftest/test_fidelity_vampire.sh`; the slice pre-commit is `BBH_FIDELITY_F5=1 BBH_FIDELITY_F6=all BBH_FIDELITY_F7=all bbh selftest` with `ROMDIR` set — the H3 pre-commit ran it: **PASS 17 / SKIP 0 / FAIL 0**, the fidelity gate 750 s — F5 all 1,891 masked specs, F6 all 51 `build/*/rompath` dirs + `$ROMDIR` (260 real invocations + 28 synthetic), F7 all 50 expectation sets (`/tmp/bbh_h3_precommit.log`) |
| **two findings about THIS tree, recorded, not fixed** | (1) `tests/lib/enumerate_expectations.sh` has no `diverge` case — a live `.diverge` would be reported UNKNOWN-KIND by `audit_legacy_pairings` (zero live today, so harmless; the harness's copy has the case). (2) A shell trap paid for in the fidelity test: inside a `$( … )` capture under `set -e`, an EXPECTED non-zero exit (the unregistered image, a RED suite) aborts the capture before the status is echoed — F5 had avoided it with `sh -c`; F6/F7 use `(set +e; …)`. Recorded in the memory note |
| **what changed in the plan** | `rpl.py` moved from H6 to H3 (the fake needs the grammar); the fake's hash is blake2b-64 not FNV-1a64; one addition to the lifted `enumerate_expectations.sh` (the `.diverge` kind). Nothing in the contract |
| **THEN H4, THE SAME SITTING — SLICE H4 LANDED: THE SWEEP RUNNER, FIDELITY F4 EXACT** (the maintainer at 46% context: *"do we do H4 or close?"* — answered "continue: the ritual is paid, H4 is bounded, the harness commits at slice boundaries", and their *"continue"*) | `tests/run_all_emulator.sh` → `bin/bbh-run-sweep` (harness commit `81ad426`): lanes in run order, the prereq lane serial and a red there stopping the run, scope and cadence with `--freeze` naming what it dropped, `--only`, `--jobs` with per-slot scratch on the scratch lanes, per-row timeouts, `--resume`, `--dry-run`, `--list`, the banner that fingerprints the builds and names the instruments, the exported env default with the caller-wins rule, the anti-orphan check through the ONE tier classifier (the lineage's inline copy is gone). EVERY project noun became `[sweep]` config: the lanes and the default lanes, the release scope, the cadences and the four-line drop note, the precondition command and its failure text, the five placeholders with the `_RP` rompath suffix, the build sets, the three instruments and the `MAME_BIN` env default, the scratch lanes/variable/default, the `[CPE-24]` citation — the defaults are this tree's literals. The example gained `tests/ci_sweep.tsv` (lanes `prereq fake`, one `out` row) and three gates (the driver as the instrument check, the suite green on `build-a`, the refusal of `hook`); its tier patterns gained `run-suite` because a gate that runs the suite reaches the driver (the sweep's own coverage check found the two rows DEAD before that — the lineage's `run_battery` pattern is the same modelling). `selftest/test_run_sweep.sh`: the lineage's thirteen sections carried, plus `--resume`, `--only`, the precondition hook, the input demand and the shipped example under `--strict`; the env-default must-fire deletes the marked export line from a COPY of the runner (`BBH_HOME` is now honoured when exported, so a copy anywhere runs). **F4 EXACT**: `--list --scope all --lane all` identical over the whole registry (169 lines) and `--dry-run` identical (199 lines: the ROM audit, the fingerprinted builds, the instruments, every lane's resolved command, the coverage report), in `test_fidelity_vampire.sh`. One text divergence noted, not a fidelity one: `classify.sh`'s TIMEOUT detail is `killed (exit N)`, the sweep's row says `killed after Ns` — the sweep overrides it to the lineage's. The H4 pre-commit: **PASS 18 / SKIP 0 / FAIL 0**, the fidelity gate 754 s at full scope (F4, F5 all 1,891, F6 all build dirs, F7 all sets; `/tmp/bbh_h4_precommit.log`) |
| **CLOSE** | `harness_scope.md` §4 carries H3's and H4's landings; NEXT_SESSION points at H5 and H7 (both depend only on H1), then H6; HANDOFF's harness row and the memory note updated; the eight doc checks green; harness commits `c26ba45` (H3) and `81ad426` (H4), PUSHED (standing-authorised); static tier **PASS 135 / SKIP 0 / FAIL 0** twice (`build/static_14z136_close.log` after H3, `build/static_14z136_close2.log` after H4), tree clean during both runs; committed to main, NOT pushed (the maintainer's word, as always) |
