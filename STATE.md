...
| | |
|---|---|
| opened with | the opener read (HANDOFF, STATE, NEXT_SESSION in full), ROM audit **76/76**, `main` == `origin/main` at `3286ad1b`; the maintainer: *"do item 1"* — NEXT_SESSION's item 1, making `tools/build_release_emulators.sh` runnable on Windows and Linux and handing the maintainer one command per machine |
| **MEASURED FIRST: what this Mac can and cannot prove** | no Linux or Windows runtime exists on this host — no docker, podman, lima, colima, multipass or vagrant — so the two new halves CANNOT be executed here, and that shaped every decision rather than being discovered at the end. The deliverable is therefore a script plus a gate that is honest about never having run, and the OS-INDEPENDENT half is proven here instead of asserted |
| **THE TWO BUNDLERS** | `tools/bundle_elf_libs.py` (Linux: the `ldd`/`readelf -d` closure copied flat, `patchelf --set-rpath '$ORIGIN'` on EVERY file — DT_RUNPATH does not propagate to transitive dependencies — the artifact re-read afterwards, and the **glibc floor measured from the artifact's own version symbols**, the twin of macOS's `minos`) and `tools/bundle_win_dlls.py` (Windows/MSYS2: the `ldd`/`objdump -p` import closure beside the .exe, and **nothing to rewrite** — a PE import table names a DLL by bare name and the loader searches the executable's own directory first; there is also no signature to apply, so the sha256 rows are the whole integrity story there). **WHAT IS DELIBERATELY NOT BUNDLED IS THE LOAD-BEARING DECISION, and it is documented in the tool rather than implied:** on Linux, glibc (one unit with the kernel ABI), the GPU stack (libGL/libEGL/libdrm — the entry points to the USER's driver, which load driver modules behind the scenes), the X11/Wayland, udev/dbus and ALSA/PulseAudio clients (each dlopen()s host modules and talks to a host daemon), and libstdc++/libgcc_s (so the artifact states ONE honest floor instead of two) |
| **THE REFUSAL THAT MATTERS MOST** | both bundlers HARD-REFUSE an empty closure. Without it a parser that returns nothing makes a bundler "succeed", after which its verifier finds no leftovers and prints success — a vacuous green on a binary that runs only on the build host, which is the one failure these tools exist to prevent ([VSP-148]: a measurement that comes back clean is a bug report about the measurement) |
| **`tests/test_bundle_parsers.sh` — the half that IS proven** (ci_portable, ~2 s, two must-fire controls) | the parsers against RECORDED `ldd` / `readelf -d` / `objdump -p` output (four ldd line shapes: resolved, the vdso and loader with no `=>`, and `not found`, which must SURVIVE the parse so the caller can fail loudly), both `NEEDED` spellings, RUNPATH and RPATH, both system classifiers in both directions; then the bundlers run END TO END against STUB `ldd`/`readelf`/`objdump`/`patchelf` on PATH — the closure walked transitively, host libraries left out, `$ORIGIN` requested, the empty-closure refusal fired, and a leftover injected into the POST-patchelf verify pass caught by the artifact check. The bbh FAKE MACHINE pattern, applied to a toolchain this host does not have. **IT DOES NOT CLAIM the binaries work** — patchelf really rewriting an ELF, the Windows loader really finding a DLL, and the exclusion policy being RIGHT are facts about a real host, and the gate says so in its own header |
| **THREE DEFECTS OF MINE, FOUND BY RUNNING IT** | (1) the Windows system-DLL regex folded the `api-ms-win-*` API SETS into an alternation anchored at `\.dll$`, so it matched NONE of them and every CRT forwarder would have been bundled; (2) the leftover control fired in the closure WALK instead of the artifact CHECK, proving the wrong thing — the stub now emits it only once the patchelf marker exists, which is exactly the verify pass; (3) a `[ test ] && { …; }` standalone statement, which ABORTS under `set -e` when the test is false (the 14z-144 trap, [VSP-176]'s family). Also removed before it could mislead anyone: a dead `is_system()` in the Windows bundler whose expression was malformed |
| **AND MY CONTROL MODES WERE LIES UNTIL I FIXED THEM** | the first `CONTROL=<name>` modes just echoed FAIL and exited 1 — asserting a hardcoded failure rather than perturbing the real input, which is precisely what [VSP-181] forbids. Rewritten as genuine SHADOW-TOOL perturbations: a COPY of the bundlers with ONE guard (`if not bundled:` / `if bad:`) disabled, after which the gate's own sections fail on their own. Both modes reach the gate's FAIL; an undeclared name REFUSES with exit 3 |
| **THE BUILDER, THREE-OS** | `tools/build_release_emulators.sh` lost its Darwin-only refusal for a per-OS block resolving the bundler, the executable name (`.exe` on Windows), the host description, the SDL3 probe name and the first-run text; `sha256_of`/`sha1_of` fall back to `sha256sum`/`sha1sum` where `shasum` is absent; the `requires` line is MEASURED per OS. **`CHECK=1` resolves and prints the plan for a host without building** — which is how the macOS path was verified here without a rebuild that would have invalidated the PUBLISHED asset's sha256 rows, and it caught its own defect on its first run: it reported MAME's executable as `mame`, which is `cps2`. The sdl2-compat probe is now conditional on the string actually being in the linked SDL2, so a distribution shipping real SDL2 is not refused |
| **THE GATE, THREE-OS** | `tests/test_release_binaries.sh` gained Linux and Windows self-containment checks, and they are deliberately NOT the bundler's own allowlist read back ([VSP-166]): they are RESOLUTION checks with the host's own loader tooling — every path `ldd` resolves must be inside the directory or a system path, nothing `not found` — which answers the question that matters. Where a platform has no signature the gate SAYS so instead of asserting one that cannot exist. **The second must-fire control stays ALIVE on every OS** rather than shipping dead on two of three: same CLAIM (a reference that does not resolve inside the directory must fail), per-OS mechanism (macOS rewrites an install name; Linux/Windows remove a bundled library AND its sha256 row, so the record stays consistent and only a resolution check can see it). An OS the gate does not know still FAILS, naming what to add |
| **A DEFECT FOUND BEFORE IT COULD BITE** | `test_release_roundtrip` §4's inventory regex admitted `emulator/bin/[a-z0-9-]+/…` — no underscore — so **`linux-x86_64` and `windows-x86_64` would both have been rejected as files outside the ruled inventory** on the maintainer's very first build. Widened, with the reason written beside it so nobody tightens it back. macOS was the only os-arch that ever matched |
| **THE RUNBOOK** | `docs/project/WSL2_SETUP.md` §10 — the prerequisites (`patchelf` and `zip` on Linux; the MINGW64 package set on Windows), `CHECK=1` first, the two builds, the gate, then either the upload or handing the directories back; plus **what to expect to go wrong the first time**, including the one thing only a SECOND machine can find (a library wrongly left to the host) and the fact that the glibc floor is whatever the build host has, so the build belongs on the oldest LTS worth supporting |
| **A SECOND RED WITH NO EVIDENCE — SO THE INSTRUMENT THE LAST CLOSE NAMED IS NOW BUILT** | the strict tier came back 144/0/2. `test_annotations_current` was mine and real: an illustrative `ldd` load address in a docstring (`0x00007ffd…`) reads as `PRG:0x007FFD` to `gen_annotations.py` and had entered the address index — elided rather than regenerating the index around a Linux ASLR address. `test_bbh_fidelity` was the other, and it is the SECOND occurrence of the 14z-149 signature: RED inside the tier at 126 s, GREEN alone on every check, both times reporting only `FAIL: see above`. **CAUSE OF THE BLINDNESS, fixed:** `run_all_static.sh` wrote each gate's output to a `mktemp` dir its own EXIT trap deleted, so a red printed four lines and the rest went with it. Failing gates' FULL logs are now kept (`build/gate_failures_static/<gate>.log`, the path relative to the runner's own root so `test_static_runner`'s DELIBERATE failures land in its throwaway fake root and never in the shared evidence path — [VSP-104], paid for once already; only the current run's logs are kept, because stale ones reading as current is the same trap in a different hat). **The flake itself is NOT root-caused** — it is recorded as a pattern rather than a one-off, and the next occurrence will carry its own evidence |
| **(2) THE BUNDLE THE MAINTAINER ASKED FOR** (*"I'll run on windows both, one natively, one through WSL2: can you prepare a bundle with a step by step md file and all the ressources I need"*) | `docs/project/WINDOWS_BUILD.md` — both tracks on the ONE machine (MSYS2 MINGW64 -> `windows-x86_64`, WSL2 -> `linux-x86_64`), start to finish, opening with the statement that they are the FIRST to run these halves and should expect to fix something. **THE MEASUREMENT THAT REMOVES THE BIGGEST PREREQUISITE:** the gate boots the romset, so it needs `vsavjw.zip` — but NOT the build pipeline. `release/merged-m18`'s own pure-Python applier rebuilds it from the three dumps in about a minute, byte-identical to the frozen build (program fingerprint `1d8bedc5`), and `MERGED=build/fromrelease tests/test_release_binaries.sh` was run here and PASSES IN FULL, MAME reproducing `05_timeout_idle`'s frozen masked expectation against the applier's output. So that session needs python3, the dumps and a clone — no merged build, no extraction chain, no harness setup. `tools/preflight_release_build.sh` builds nothing and catches the two expensive failures up front: the MSYS2 **MSYS-vs-MINGW64 shell trap** (a binary built in the wrong shell links the MSYS2 runtime and will not run on a player's machine) and MAME dying minutes in on an SDL3 pkg-config miss; exercised on both branches here with a stub `uname`. `tools/collect_build_report.sh` writes ONE file to send back — names and verdicts only, no ROM data, verified on a real 756-line run. Bundle sent (README, both scripts, WSL2_SETUP, START_HERE; every member text, no ROM byte) |
| **CLOSE** | **Static strict 146 / 0 / 0 GREEN, `fired 148 / declared 148`, `executed 148: honoured 148, lies 0, refused 0, died 0`** (`build/static_14z150_close2.log`, run DETACHED). THE ROUTE THERE IS THE SESSION'S OWN LESSON: the first strict run was 144/0/2 (`test_annotations_current`, mine and real — the docstring address; and `test_bbh_fidelity`, the unexplained one). The kept-log instrument was built between that run and the next, and **on its very first run it named the third red in seconds: MY OWN runner change.** The `(full log: …)` line is VERDICT TEXT, and bbh's F1 compares the two runners line for line — so it is a BOTH-SIDES change (bbh convention 8). Landed in the harness the same sitting (`[classify] fail_logs`, config-driven because where evidence lands is a consumer's decision), bbh selftests 31/2/0, **the harness PUSHED FIRST** (rebased onto the maintainer's own README commit rather than force-pushed; `f9d08a8` + the rebaselines line `1af19c7`), then the tree's `test_bbh_fidelity` green on both halves. **So the two EARLIER reds remain unexplained** — they predate the change that caused the third — and the next occurrence will carry its own evidence, which is the point. Doc gates green after every edit; STATE entry written, NEXT_SESSION rewritten and the 14z-149 opener archived blockquoted at the top of its history (the convention my first attempt missed). **No build byte moved**; M18's patches, MRAs and published assets untouched. Pushed per the close rule. **The maintainer offered a dedicated session on the Windows machine — that is where item 1 stops being written and starts being measured.** |

## Session 14z-149 — **THE PREBUILT EMULATOR BINARIES, macOS: built from the recipe, made SELF-CONTAINED, recorded,
## gated by a BOOT on each, and packaged into M18 — and the shipped recipe was found wrong twice on the way. No build byte moved.**

| | |
|---|---|
| opened with | the opener read (HANDOFF, STATE, NEXT_SESSION in full), ROM audit **76/76**, `main` == `origin/main` at `7b087627`; the maintainer: *"item 1 (the other versions will be done on other machines, btw could the linux version done under windows ubuntu emulation layer?)"* — answered YES: WSL2 is a real Linux kernel and userland, its ELF binaries run on any Linux whose glibc is at least the build distro's; the SDL question is the same as on macOS |
| **MEASURED FIRST, before a byte was built** | both dev binaries link Homebrew's SDL by ABSOLUTE PATH (`otool -L`: `/opt/homebrew/opt/sdl2-compat/…`, `/opt/homebrew/opt/sdl3/…`), so the recipe's product runs only on the build host; the FBNeo submodule carries BOTH patches applied, so a release FBNeo needs a CLEAN worktree of the pin with 0002 only; `sdl2-compat` is a shim that `dlopen()`s SDL3 as `@executable_path/libSDL3.dylib` (no load command shows it — `strings` does); FBNeo's `sdl2_image` drags a codec closure (18 libraries, 12 MB); **every binary and every Homebrew dylib here carries `minos 26.0`** — bottles are built per OS release — so this Mac's prebuilt is macOS 26+, arm64, by inheritance; the dev binary is 43 MB and `strip -x` removes nothing (no symbols in the executable to begin with) |
| **THE TOOLS** | `tools/build_release_emulators.sh fbneo\|mame` — EMULATOR.md's recipe run on this host into `release/emulators/<platform>/<os-arch>/` + `BINARY.txt` (sha256 rows the packager and the roundtrip gate already read; pin; patch sha1 with the statement that it is the ONLY patch; recipe; host; `requires` = the minos the artifact carries; run; Gatekeeper; not-works; provenance). FBNeo: `git worktree add --detach` of the gitlink (asserted equal to the checked-out submodule), 0002 applied, harness ABSENT asserted on the tree AND on the binary (`-hframes` string, present once in the harness build, absent here). MAME: `setup_mame.sh` into its own mirror `~/.cache/vampire-saved/mame-release` — that script IS the 0002-only recipe. `tools/bundle_dylibs.py`: the non-system closure copied FLAT (the inventory admits `emulator/bin/<os-arch>/<file>` and nothing deeper) under the loader's names, `@loader_path` install names, rpaths deleted, everything ad-hoc signed, verified on the artifact. macOS only; refuses elsewhere naming what a Linux/Windows session adds |
| **THE FRESH-TREE BUILD RACE — [CPE-26] CORRECTED IN PLACE** | the first build died at once: `No rule to make target driverlist.h, needed by burn.o` — WITH `SKIPDEPEND=1`, the gotcha's own cure. Mechanism (measured: a serial `-j1` build proceeds, compiling every driver first): `burn.o`'s bare prerequisite is the GENERATED driver list, whose rule depends on every driver object, so a parallel first pass schedules `burn.o` long before the list exists. The dev submodule never showed it because its list has existed since the first build. FBNeo's own CI runs the same line on Linux; make 3.81 here. Fix = TWO PASSES (`-k` first, then plain; a no-op on a complete tree) — in the tool, in the shipped EMULATOR.md recipe (the packager), in the gotcha and in [MFI-26] |
| **THE BUNDLER'S VERIFIER CAUGHT ITS OWN FIRST VERSION** | six leftovers after the first bundle: `libjxl → @rpath/libjxl_cms`, `libwebp`/`libwebpdemux → @rpath/libsharpyuv`, the brotli pair `→ @rpath/libbrotlicommon` — Homebrew libraries name their SIBLINGS through `@rpath` with the Cellar lib dir as rpath, and a walk over absolute references skipped every `@`-prefixed one. The walk now resolves `@rpath/<name>` through the referrer's `LC_RPATH` (then the source dir), bundles the sibling, deletes the rpaths; validated on a FRESH copy of the build output (a re-run on the half-rewritten dir proved nothing: the executable's references were already `@loader_path`, so the walk never reached the dangling libraries). Result: fbneo 23 libraries / 58 MB, mame `libSDL3.0.dylib` only / 82 MB; a headless boot through `@loader_path` on the M18 set: profile line, 31 `(OK)`, running at 20 s. Platform gotcha recorded |
| **THE GATE — `tests/test_release_binaries.sh`** (emulator tier, `mame` lane, release, romset, timeout 300; family platform) | for THIS host's os-arch: record (every row's sha256, every file named, the executable named, the five key lines), self-containment (`otool -L` of every Mach-O: only `/usr/lib`, `/System`, `@loader_path`), `codesign --verify --strict`, profile present, NO harness, and a BOOT of `%MERGED%` on each: FBNeo headless under the SDL dummy drivers 20 s (`rc=124` = still running, the profile line, exactly 31 `(OK)`, no error); MAME `-listfull vsavjw`, `-verifyroms` (below) and **`05_timeout_idle` REPRODUCING ITS FROZEN MASKED EXPECTATION on the release binary** (`MAME_BIN=` + `SUITE_ONLY=` through run_suite) — the release binary and the gate instrument traverse the same RAM. Two must-fires, one function each, both EXECUTABLE: `flipped-library-byte` (a copy with one byte of a bundled library flipped → the record check) and `absolute-reference` (a copy whose executable has one install name rewritten back to `/opt/homebrew/…`, re-signed, the record RE-HASHED so the record check passes → the self-containment check). Plain PASS 74 s; both modes reach the gate's FAIL. On another OS the self-containment and signature checks FAIL naming what that host must add — never a silent pass. Census re-frozen 85 / 0 / 0 |
| **`-verifyroms` CAN NEVER SAY "GOOD" ON A CONTENT SET — the shipped recipe said it must** | the gate's first MAME run failed on `romset vsavjw [vsav] is bad`: the descriptor carries the STOCK CRCs for the members the port rewrites (`vm3j.03d`… `vm3.13m`…) and sentinels for the new ones (`vsw.41-44` `dec0de41…`), exactly [VSP-75]'s "honest cost". HANDOFF's B5 sentence ("reports the romset good") and cps2_wide.md's were measured on the ZERO-FILLED `wide0` overlay and were true there; the packager's EMULATOR.md line `# must say: romset vsavjw [vsav] is good` was FALSE since the first content build and shipped in every release since merged-m10. Retraction pass: the gate asserts the honest expectation — flagged set == exactly the WIDE zip's members that are NOT byte-identical to a pristine twin (**20 at M18**, computed from the zips), nothing NOT FOUND, the `is bad` summary present; the packager's recipe, the record's `run` line and HANDOFF's sentence corrected in place; the `is good` grep over docs/tools/tests now finds only the scoped cps2_wide.md sentence and unrelated prose |
| **M18 REPACKAGED, TWICE** | (1) WITH the binaries — `package_release_platforms.py` with the current MRAs copied out first (it clears the destination): patches, manifests and MRAs byte-identical, `emulator/bin/macos-arm64/` on both emulator sides (24 + 2 files, sha256 verified in), `release/merged-m18/` 150 MB, `test_release_roundtrip` PASS with the binaries inside (8/8 controls); (2) then WITHOUT them for the COMMIT (the resource dir moved aside for that one run), so the pushed release directory does not claim binaries it does not carry: 11 MB, git sees only the two `EMULATOR.md` recipe corrections, roundtrip PASS again. One packager run reproduces (1) once the decision below is ruled |
| **THE DECISION THIS OPENS — recorded under "Decisions pending"** | committing the binaries as they stand adds ~140 MB per RELEASE to git history (the resource dir once, plus a copy per release directory) against a 68 MiB pack today. Nothing binary is committed this session — `release/emulators/` stays on disk, untracked, gated; the tools, the gate, the docs and the corrected release text are committed |
| **(2) THE RULING AND THE MECHANISM** | the maintainer's three questions (delete the previous binaries so GitHub hosts only the latest? host on my own storage? the exact sizes?) answered: only ASSETS can be deleted — a committed binary is history forever; own storage works as a mirror; fbneo 57 MiB dir / 40 MiB executable / 16 MiB zip, mame 81 / 79 / 15 — and RULED (b) with pruning. Built: `.gitignore` (the records tracked, the files ignored under `release/emulators/*/*/` and `release/*/*/emulator/bin/*/`), `tools/upload_release_assets.sh freeze/<name> [--prune] [--dry-run]` (the record verified on disk, its `asset` line written, `<name>-<platform>-<os-arch>.zip` built with the directory inside, the GitHub release created on the tag if absent, uploaded with `--clobber`, then DOWNLOADED BACK and every row re-verified against what GitHub serves, the served record `cmp`-identical to the tree's; `--prune` deletes the previous `freeze/merged-m*` release's zips), the packager's prebuilt paragraphs now name the asset and the release URL (derived from the origin remote), `test_release_binaries` SKIPs and `test_release_roundtrip` §4 notes a record-only directory (a fresh clone), M18 repackaged WITH the binaries (the records inside identical to the resource's), the first upload made on `freeze/merged-m18` |
| **(3) vhunt2 DROPPED FROM THE RELEASE — the maintainer's "what do we need vhunt2 for?" answered by measurement, then RULED** (*"asking the user for a fourth dump is a lose / lose situation"*) | vhunt2 is the port's extraction ORACLE (the vs2/vhunt2 sibling diff classifies every pointer field; `build_donovan.sh --oracle`), never a content source: no atlas range is tagged `VH2`, no manifest row sources from it. The release asked for it only because the packager's source blob held all four dumps; every copy instruction in the M18 patch set attributed to its blob zone (`xdelta3 printdelta`, window offsets applied): vsav 19.9 MB, vsav2 4.75, vsavj 1.34, **vhunt2 0.76 MB — runs byte-identical in vsav2, the encoder picking one of two equal matches**. `SOURCE_ORDER` is three dumps; M18 repackaged (every patch re-encoded, blob 143 -> 97 MB, patch set still 2.5 MB, manifests identical across the three platform dirs at `0acdda85`); `test_release_roundtrip` PASS — 25 members byte-identical from pristine dumps, 8/8 controls (the rule-7 scan KEEPS vhunt2 in its reference index: a verbatim run of it is a ROM byte whether or not the blob holds it); the applier run from a directory holding ONLY the three dumps rebuilt all 25 members verified; `test_release_binaries` PASS on the repackaged tree. Also answered: it is ONE romset — the three platform dirs carry byte-identical copies of the same patch set (the gate asserts it); the control MRA is KEPT for now (maintainer) |
| **(4) EVERYTHING AVAILABLE PUBLISHED, AND THE README EXPLAINS EACH DELIVERABLE** (the maintainer: *"you can already publish everything available when it is available … please make the readme clearly explain what are each deliverable and how to use them"*) | `tools/upload_release_assets.sh` now publishes the three PLATFORM PACKAGES first — `release/<name>/<platform>/` zipped as `<name>-<platform>.zip` (the binaries excluded, their `BINARY.txt` records kept), uploaded, downloaded back, and EVERY served file `cmp`-identical to the committed directory (26 / 26 / 28 files) — then the binaries, then writes the RELEASE NOTES from the assets actually attached (a table of what each is, the no-ROM statement, GitHub's unremovable source archives marked "not needed to play"). The packager inserts **"## The deliverables — what to download, and what each is for"** before "What is in this package" in every platform README: every asset, what it is, who needs it, which prebuilts exist per OS (from the resource dir), "you are reading the <platform> package", the order build → get the emulator/core → play; the roundtrip gate requires the heading (six sections now). **On `freeze/merged-m18`: 5 assets** — fbneo 2.2 MB, mame 2.2 MB, mister 3.4 MB, the two macOS binaries 16 MB each. The source-archives item is thereby SETTLED (option (a), by the packages rather than one release zip). **WINDOWS/LINUX — the maintainer asked how to proceed (remote session I connect to? autonomous session? a script they run?): A SCRIPT THEY RUN.** `tools/build_release_emulators.sh` already runs the recipe on any POSIX host and refuses only at the bundling step; what a session must add before it is runnable there: the Linux bundling half (`patchelf --set-rpath '$ORIGIN'` over the `ldd` closure, or a static SDL) and the Windows half (the DLLs beside the .exe, MSYS2), plus `test_release_binaries`' self-containment/signature checks for those OSes (it FAILS there by design today). Then one command per machine under WSL2 / MSYS2, `test_release_binaries` green there, the two directories sent back or the upload tool run there. I cannot reach a remote Windows session from here, and an autonomous session on that box needs the repository and the ROM audit there anyway — the script is the shape that leaves a verifiable record |
| **CLOSE** | **Static strict 145 / 0 / 0 GREEN, `fired 146 / declared 146`, `executed 146: honoured 146`** (`build/static_14z149_close.log` before the ruling, `_close3.log` after it, both run DETACHED — the run between them, `_close2.log`, was 144/0/1 with `test_bbh_fidelity` RED and NO SURVIVING EVIDENCE: the static runner keeps only a gate's last lines, which read "FAIL: see above"; the gate run alone was green on every check, and green again inside the deciding re-run at 184 s. Recorded as a one-off with its cost: a runner that kept per-gate logs would have said what it saw); `test_release_binaries` PASS 74 s + both modes FAIL as required; `test_release_roundtrip` PASS with and without the binaries; the nine doc checks green after every edit; STATE rolled (the 14z-148 group to the archive with its ledger line, 180 -> 165 KB); NEXT_SESSION rewritten, the 14z-148 opener archived; the memory note `release-inventory` updated. **No build byte moved, M18's patches and MRAs byte-identical.** Pushed per the close rule, with the binaries' home recorded as the one pending decision |
...
- Session 14z-148 CLOSE (2) — **THE --controls CADENCE RULED (a), THE MUST-FIRE READER LIFTED INTO bbh AS A COPY (F1 over declaring stubs, F2 exact at 70 rows, the readout text generic on both sides); THEN THE RELEASE INVENTORY MEASURED AND RULED (patch set + end-user README per platform, MiSTer RBF+MRA on top, emulators recipe AND prebuilt), the applier PURE PYTHON, the stock-emulator STALL measured and locked, release/merged-m18 regenerated byte-identical.** Census 84 / 0 / 0; static strict 145/0/0, 146/146 controls honoured; no build byte moved; pushed  [rolled 14z-149 close, early — STATE was 180 KB]
- Session 14z-147c CLOSE — **STEP TWO OF THE MUST-FIRE MACHINE, ALL THREE PASSES IN ONE SITTING: PASS 1 the contract's READER, the runners that EXECUTE every declared control and the PORTABLE tier (30 gates, 99/99 modes honoured); PASS 2 the STATIC tier (19 gates, 44/44); PASS 3 the EMULATOR tier (34 gates, verified on 14 cheap ones).** Census 83 / 0 / 0; static strict 145/0/0; the `CONTROL`-env collision and the fork-editing lesson recorded; one decision left open (the `--controls` cadence, ruled 14z-148); no build byte moved; pushed  [rolled 14z-148, early — STATE was 178 KB]
- Session 14z-146 CLOSE — **THE THREE KNOWLEDGE ITEMS IN THE MAINTAINER'S ORDER: the RE-HIT RULE MEASURED (the victim's recent-hit slot, written at contact and CLEARED on every gap node — the derivation already encoded it, SA's residue is the juggle gate), the GAUGE COLUMN ARBITRATED (swing 0/3/6 by strength, then the record's +0x14 per LANDED hit; the six cells are hits-landed residues, two workbook defects), and Anakaris's J.2K plus the three timing cells closed by measurement.** Two new emulator gates, two extended, 360 aerial rows; no build byte moved; static strict 144/0/0; pushed  [rolled 14z-147 close, early — STATE was 173 KB]
