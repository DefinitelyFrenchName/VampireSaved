...
## Session 14z-140 — **THE LIVING DOCS EFFORT OPENS AND ITS FIRST TWO SLICES LAND: L1 ROUTING
## ENFORCEMENT and L4 THE RENDERED SITE.** The map now REACHES every declared document, a history twin
## must be two-way, and the whole corpus renders as 80 cross-linked pages with an address index over every
## carrier. Both censuses disagreed with the scope document, and five of L4's six structural findings were
## absent from the plan. No build byte moved.
...
| **(8) L4 BEAT 1 — THE CENSUS HAD TO BE REWRITTEN MID-MEASUREMENT** | The first instrument was LINE-BASED and reported **76 raw-HTML violations**; 34 were `<name>`-shaped placeholders inside a CODE SPAN WHOSE BACKTICKS SIT ON DIFFERENT LINES. A paragraph's lines are joined before the inline pass, so blocks come first and inline second — a correctness requirement, and the reason the numbers were worth nothing until the instrument was rebuilt. Re-measured over 74 files / 50,705 lines / 3.8 MB |
| **the six structural findings, five of them absent from the plan** | (1) code spans wrap line breaks; (2) **21 escaped pipes `\|`** in 8 files — the plan's "a row whose cell count differs from the header FAILS" would have failed **15 tables in 9 files**; (3) **22 rows with no leading pipe**, including `HANDOFF.md:45` whose row opens with two bold anchor markers; (4) real HTML in exactly ONE place (`<details>` in the three generated chars pages) while the other 26 hits are `<word>` placeholders in prose across 22 files — the plan's strict FAIL would have failed 22 documents for writing English; (5) **a pipe inside a code span in a cell is content**, which was four of the seven ragged rows the first strict pass reported; (6) **a table needs its DELIMITER row**, without which a `bbh` subcommand list inside a wrapped code span became a 7-cell table and three "tables with no separator" appeared. With rule 6 that count is ZERO |
| **THE LOAD-BEARING CORRECTION — the address index** | The plan says to parse the rendered `annotations.md` and make an unresolvable section a generator FAILURE. **That page cannot be parsed back:** 688 headings contain its file/section separator ` — `, 103 contain its carrier separator `;`, `cell()` rewrites every backtick to `'`, and `MAX_CARRIERS` truncates to six. Parsing it resolves **540 of 825** carrier pairs to no heading. Importing `gen_annotations.collect()` resolves **329 of 329** document-tier pairs and loses nothing — so the hoped-for enforcement is real and green today, and the site shows EVERY carrier where the page shows six |
| **(8) BEAT 2 — §9 written**, commit `a5f5077c` | the census, the six findings, the address-index correction, the architecture, the gate with five must-fire AND three NEGATIVE controls, S1-S4, nine decisions, the sequencing. **A gotcha paid for TWICE in one edit:** quoting a bold `**[PFX-N]**` marker in prose creates a PHANTOM ANCHOR (§9 registered two for CPE-32/MFI-32), and the gotcha written to record that used a real ID in its own example and tripped the census and checkskills again |
| **(9) THE STOP — all nine ruled**, commit `b9d6f385` | Seven as recommended. **Decision 4 came back STRICTER than my recommendation**: the site does NOT link the drawn MiSTer page, because that page is gitignored and built on demand, so linking it means weakening *every href resolves to a written file* — the walk stays ABSOLUTE and the page gets a NOTE naming the command. **Decision 5 came back WIDER**: the search indexes the 555 `**[PFX-N]**` anchor IDs too, so typing `VSP-167` lands on the rule. The others: `docs/site/` gitignored · auto-links scoped to unambiguous resolution · the palette MOVES to `_pagestyle.py` · publishing stays a separate decision · the subset is taught `<details>`/`<summary>` and nothing else · a `<word>` in prose is TEXT · a leading pipe is optional |
| **(9) S2 INVERTED WHEN IT WAS MEASURED PROPERLY** | `test_mister_page.sh`'s header said ci_portable while its row sat in `ci_static.txt`. `env -u ROMDIR` returned rc=0 and said the header was right — **on a machine that HAS the build dirs**. Re-measured in a real clean checkout (`git archive HEAD`), the gate exits 0 and re-derives 15 figures but PRINTS the generator's two `  SKIP:` lines, and `classify.sh` matches `^ *SKIP`, so the whole gate classifies SKIP — a FAILURE in ci_portable by design. **Registration right, header wrong**; the comment block orphaned in `ci_portable.txt` moved to `ci_static.txt`. Gotcha: a gate's TIER cannot be measured on a developer's machine |
| **(10) `tools/md_subset.py`**, commit `24996159` | the markdown subset as a CENSUS, with `tests/test_md_subset.sh` (ci_portable, 9 must-fire + 9 must-NOT-fire controls). **All 74 corpus files parse, exit 0.** Rules 5 and 6 were not in the plan — they came from reading all seven ragged rows the strict parser reported rather than accepting the number. One bug of mine: requiring the delimiter row made a table-shaped line fall through to the paragraph branch, whose loop broke before consuming anything — an infinite loop, caught by the selftests hanging |
| **(11) `tools/_pagestyle.py`**, commit `1107d1b2` | the theme MOVED, not copied. What moved is the VALUES (palette, colour maths, `THEME_LIGHT`/`THEME_DARK`); each generator keeps its own stylesheet, because the MiSTer page interleaves a property per region ROLE and `--warn` differs by theme (FLAME on light, WARM_L on dark). **Proof it is neutral: the gate green before and after, and the drawn page BYTE-IDENTICAL (`cmp`) to the pre-move script's.** Two costs: the gate COPIES the generator to a temp dir, so the module resolves from `--repo`'s `tools/` too; and `_pagestyle`'s selftests run from `test_mister_page.sh` section 0 |
| **(12) `tools/mk_docs_site.py` — THE SITE**, commit `4105c12d` | 82 files + 2 images, **6.6 MB, 80 pages, 2,958 address rows, 1,639 search entries**, into the gitignored `docs/site/`. Gate `tests/test_docs_site.sh` (ci_portable, ~25 s): 0 unresolved in-tree hrefs, 0 external loads, two renders byte-identical, four must-fire controls and one must-NOT-fire. **THREE OF THE GATE'S OWN CHECKS WERE WRONG THE SAME WAY** — reading TEXT where the invariant is about an ATTRIBUTE: `no http://` fired on escaped prose, `no build/out` fired on HANDOFF for QUOTING a filename, and the href walk read the search box's `'<a href="'+BASE+…` as 80 broken links. **And the address-index control was DEAD TWICE**: its fixture sat where `gen_annotations.CARRIERS` does not look, and a preamble address does not give section `(top)`. What fires it is a real parser disagreement — an unclosed code span swallows the headings after it — which nothing else in the tree catches |
| **(13) docs** | four gotchas appended and indexed (the phantom anchor; a gate's tier needs a clean checkout; a link check must read the ATTRIBUTE; a must-fire control needs its own liveness), plus a fifth: **the [VSP-13] retraction grep MISSES A CLAIM THAT WRAPS**, found when the L4 grep returned three hits and the live fourth was invisible because the sentence wraps between `one rendered` and `page exists`. Grep the shortest distinctive fragment, then a wrap-insensitive scan. `living_docs_scope.md` §2/§4/§9 brought to LANDED; HANDOFF gains a "What exists" row and a routing-table row; `docs/README.md` names the generator |
...
| **CLOSE (2)** — after the maintainer's *"we have ample context left, continue"* and then *"let's go through them one by one"* | L4 executed end to end in the same sitting: beat 1, beat 2, the STOP with all nine decisions ruled, and §9.9's five steps. **The static tier run ALONE a second time, `--strict`: PASS 139 / SKIP 0 / FAIL 0 GREEN** (`build/static_14z140_close2.log`; 137 + `test_md_subset` + `test_docs_site`), tree clean during the run. The eight doc checks green after every edit. **The site is on disk at `docs/site/index.html` and is the slice's "ends when" — it waits on the maintainer opening it.** SEVEN more commits to main (8)-(14), fourteen in the session, **NOT pushed**. STATE **135 KB** after the 14z-139 group rolled |
