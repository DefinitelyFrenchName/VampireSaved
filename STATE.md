...
| **THE RUN'S FIRST DEFECT, AT 6 h 21: A PASS THAT MEASURED NOTHING** | `test_mister_obj_oracle` — a 65-minute Verilator gate — recorded **`PASS 0s`**. Its log is four lines ending `line 87: JTSIM_SCRATCH: set JTSIM_SCRATCH to a dir OUTSIDE the repo`: the gate DEMANDS the variable where every sibling MiSTer gate defaults it, and the demand sits AFTER its `trap … EXIT`, which on macOS bash 3.2 (`/bin/sh` and `/bin/bash`) turns the abort's exit status into **0** — reproduced three ways (with the trap rc 0, without it rc 1, with a status-preserving trap still 0: `$?` is already 0 when the trap runs). The runner's exit-status-first classifier, correct for the `SKIP:`-plus-non-zero case it was built for, therefore wrote PASS. **Every other fast PASS in the run carries the gate's own PASS/ok lines** (`test_movability_liveness`, `test_fbneo_runner_hygiene`, `audit_ff0460_writer`), and the four other `.sh: line N:` lines in 158 logs are MAME teardown segfaults after the summary line ([MFI-12]). Census: FIVE gates carry a `${…:?}` demand after their trap (this one; `test_projectile_params` and `test_tenant_row_owner` with a late `${ROMDIR:?}`; two `rm -rf "${VAR:?}"` guards). Gotcha filed (`docs/project/gotchas.md`, harness bucket). **CONSEQUENCE: the run is NOT green at 164/1/0 — that row is a hole.** The fix (default the scratch; demands as explicit tests; a runner rule that a PASS with a shell-error line and no verdict line is a FAIL) touches `tests/`, so it waits for the run to exit; then the gate is re-run under `--resume` exactly as 14z-133 re-ran its three reds. Written into the checklist as step 0b. This is also the FIRST time any MiSTer-lane gate ran under the runner (the lane is opt-in; no sweep ever included it), and the first gate with a defect was the fourth one it reached |
| **THE RUN'S SECOND DEFECT, AT 7 h 37 AND AGAIN AT 9 h 08: TWO TIMEOUTS AT THE RUNNER'S ONE-SIZE CAP** | `test_mister_gfxc_fetch` killed at 5400 s (frame 3741 of its FIRST leg; its own registry note says *"~93 min a leg, two legs"*), then `test_mister_prg_window` (header: ~2 × 40 min) at the same cap. The runner has ONE `--timeout` (default 90 min) for 165 gates whose rows document runtimes from 0 s to three hours; the emulator lanes never approached it, the MiSTer lane's first four gates included the two longest so far, and `test_mister_qsound_ext` (~2 × 75 min) was next. The kills were clean — `timeout -k 30` took the whole process group, no orphaned `obj_dir/sim` survived (checked by process tree). A SIZING defect of the runner, not the artifact |
| **OPTION A, RULED BY THE MAINTAINER (*"option A"*): STOP, FIX, RESUME ONCE WITH A 4-HOUR CAP** | (1) the runner (PID 26804) and the in-flight `qsound_ext` wrapper stopped by TERM, process tree verified empty, no `qsound_ext` row written; (2) `results_first_pass.tsv` + `runner_first_pass.out` kept beside the live files (the 14z-133 protocol) and THREE rows removed from `results.tsv`: the hollow `obj_oracle PASS 0s` and the two TIMEOUTs — 159 rows stay; (3) the fixes, IN THE MAIN TREE (the runner was stopped, so the tree check has nothing to see): `test_mister_obj_oracle` DEFAULTS `JTSIM_SCRATCH` like its siblings and refuses one inside the repo; the two late `${ROMDIR:?}` demands (`test_projectile_params`, `test_tenant_row_owner`) and the `rm -rf "${PART:?}"` guard (`test_mame_mirror_guard`) are explicit tests; the runner's classifier FAILS an exit-0 log carrying the shell's own `<script>.sh: line N: NAME: message` while the MAME teardown `line N:  <pid> Segmentation fault` shape stays PASS (`test_emulator_runner` §12, both controls fire); a new portable lint `tests/test_demand_after_trap.sh` scans every gate for a `${VAR:?}` demand after its EXIT trap (heredoc stub bodies skipped, self skipped), must-fire both ways — its FIRST draft matched only a line-initial `trap` and passed the tree BLIND, caught by its own control not firing; (4) static tier on the fixes, then commit to `main`; (5) `--resume --timeout 14400` under the runner's shape: six gates — `prg_window`, `gfxc_fetch`, `qsound_ext`, `sdram_census`, `audit_sdram_bank_load`, and the `obj_oracle` re-run — ~9 h. The worktree branch rebases onto `main` afterwards |
| **THE RESUME: obj_oracle and gfxc_fetch GENUINELY GREEN, THEN A REAL RED — `test_mister_prg_window` FAIL at 108 min, "the pair MOVED"** | `obj_oracle` PASS 4278 s (promoted subset 31 on both legs, field-for-field identical, must-fire caught a one-bit change); `gfxc_fetch` PASS 11037 s (= its row's "~93 min a leg, two legs", the cap was the whole problem). Then `prg_window` exit 1 after 6502 s: every STRUCTURAL assertion PASSED — `wide_en` 1/0 reached the RTL, must-fire 75.7 M / 75.5 M reads below `$400000`, the control leg ZERO reads above, the profile-ON leg 1,211,302 program-ROM reads above `$400000` — and the one thing that failed is the FROZEN PAIR (`tests/expect/mister_prg_window.txt`): `first_addr 4be7c0 -> 4c13d0`, `rd_hi 1189750 -> 1211302`, `min 412ba0 -> 413c70`, `max 4d100e -> 4d3c..`; the control line moved by 62 reads and 278 cycles. **ATTRIBUTED, no theory needed:** the pair was frozen 2026-08-24 (14z-107 (11), commit `d3988fce`) on merged-m10's build; `$4BE7C0` is inside the relocated OBJ WALKER block of THAT layout (`code 0x4be6b0 +0x15c obj_walker` in `m3b_merged15`'s fragment), and `$4C13D0` is the same walker family in merged-m16's (`code 0x4c13d0 +0x24c obj_walker: 0x5e52a relocated verbatim + its extended type table`, `patch_notes_fragment.md:1336`) — the hole allocator placed the walkers ~`0x2B00` higher once the 14z-115/117 freezes added extension content before them, and the walker is the first extension code the 68k executes on every boot (frame 1119 on both). The pair has been stale since merged-m11 (2026-08-29) and NOBODY COULD KNOW: the gate is `bitstream` cadence and the MiSTer lane is opt-in, so it never ran at a freeze — while its frozen pair FOLLOWS THE ROMSET (a placement address), exactly the property that makes `sdram_census` and `gfxc_fetch` the two ruled `romset` exceptions. A [VSP-95] stale-constant red plus a cadence misclassification; not the artifact. Also found: `tests/expect/` (three files) is OUTSIDE `test_expectation_provenance`'s scope (`tests/expected/` only). Ruling needed — the entry at the top of "Decisions pending" — and the fix is a tracked-file edit, so it WAITS for the resume to end (the tree check) |
| **qsound_ext WILL TIME OUT TOO, and the maintainer asked the right question: *"can't we have separate instances of the checker?"*** | measured while it ran: its legs are ~3 h EACH (the header said 75 min; `--stats --timing`), the second started at ~7:38, the 4-hour cap lands at ~10:04 — lost by ~35 min. The positive leg's finished outputs (5 MB: jtsim.log, three read-probe files, inputs) SALVAGED to `build/emu_release_m16/qsound_ext_salvage/pos` before the kill. The serial lane, measured: one shared scratch clone is the WHOLE constraint (a run writes `rom.bin`, `sim_inputs.hex`, bank dumps, `wram/`, probes and `obj_dir/` into the clone's core dir); the runner already has `--jobs`; jtsim passes Verilator no threads flag; one sim = one core at 99 %, ~97 MB RSS; a clone 1.4 GB; the machine 8 P + 4 E cores, 16 GB. **RULED (maintainer): build the parallel lane NOW, in the worktree, while the resume finishes** — *"then start as you planned"* |
| **THE FIRST MEASUREMENT FOUND A THIRD DEFECT: A FRESH SCRATCH CLONE COULD NOT SIMULATE AT ALL** | a second clone provisioned in 2 s, then every census leg died in 0 s: `run_sim_jtcps2.sh: line 310:  : command not found`. The 14z-133b module-init line put its comment AFTER a line-continuation backslash — `\ ` is an escaped space, an EMPTY command name — so on the existing clone (fx68k.sv present) the branch never ran and on a fresh one (modules absent) the driver died before initialising any module. The 14z-133b heal gate only ever exercised a HOLLOWED clone, whose modules survive; the tmp-reaper RE-CLONE path was equally broken. Fixed (comment moved above the line); gotcha filed. Re-measured: the fresh clone initialised its five modules, built its Verilator model and generated its `.rom` in under 3 min, and its census simulation ran at 100 % CPU BESIDE the resume's — two simulations coexist, the property the lane depends on |
| **THE PARALLEL LANE, BUILT (worktree), every piece with its control** | (1) `run_all_emulator.sh`: a row's optional 7th column is its own timeout (seconds) overriding `--timeout`; `--jobs N` on the mister lane gives slot 0 the base `JTSIM_SCRATCH` and slot N `<base>-slotN`; printed with the instruments. `test_emulator_runner` §13: a 2-s column kills a 20-s gate under a 60-s global, a 6-column row keeps the global, two mister gates under `--jobs 2` receive DIFFERENT scratches (base / base-slot1), `--jobs 1` keeps the caller's — all fire; §10 validates the column. (2) `run_sim_jtcps2.sh --profile-off`: the control image is a COPY (`<set>.profile-off.rom`, byte 41 asserted 0xFE then 0xFF) simulated from the leg's own clone; the `.rom` is never patched in place; each run writes `rom_path`; `link_rom` tolerates the two-leg race on the shared reference symlinks; guard: `--profile-off` without `--wide` refuses. (3) `test_mister_prg_window` / `qsound_ext` / `gfxc_fetch`: both legs at once on `<scratch>` and `<scratch>-b` (`MISTER_LEGS=serial` keeps one clone), each leg's output prefixed `[pos]`/`[neg]`, both statuses checked, `rom_used.bin` from `rom_path`. (4) Registry: timeouts for the nine MiSTer rows from this run's measured seconds × ~1.5 (wide_inert 3600 … qsound_ext 28800), `prg_window` cadence → `romset`, notes and gate headers carry the MEASURED runtimes (prg_window ~55 min a leg not 40, qsound_ext ~3 h not 75 min, gfxc ~93 min not 65). (5) HANDOFF: the emulator-tier command and the lane paragraph. Expected: the ~11 h serial lane ≈ the longest single leg (~3.2 h) at four jobs; the pending qsound_ext + prg_window re-run ≈ 3.2 h instead of ~8 |
| **qsound_ext DID NOT TIME OUT — it finished both legs in 3 h 8 (so ~94 min a leg, like gfxc; my "3 h a leg" read the start time wrong) and FAILED on ITS OWN LIVENESS LOOKUP** | the substance PASSED on the core: `QSound EXTENSION fetched: 440896 reads, 280 distinct blocks, first at frame 3783 (710AA0-78FFFE)`, every address inside the ledger's banks 0x80-0x8E, the SLOT5_AW mask lossless, `0 reads with wide_en clear`, both QSound-LOW probes counting. The two FAILs: *"bank 3: ZERO — the probe is not counting"* on both legs. **The probe WAS counting**: the salvaged positive leg's `jtsim.log` says `RDPROBE SUMMARY 2 bank 3 … reads 171491620`. The driver numbers probe SLOTS in the order the `--rdprobe BANK LO HI` windows are given (since 14z-107 (10)); the gate arms bank 3 as its THIRD window (slot 2) and then reads `RDPROBE SUMMARY 3 ` — slot 3, which does not exist — so its liveness control read a live probe as dead. In the gate since its birth (14z-108, one day after the driver's slot-by-order form); the 14z-108 record says it was "PROVEN ABLE TO FAIL on … a dead liveness probe" and records the FETCH, not a full-gate PASS; bitstream cadence and an opt-in lane meant no run since. FIXED (worktree): `sum_of` keys on the BANK (`RDPROBE SUMMARY [0-9] bank N `), proved on the salvaged log (bank 0 440896 / bank 1 86612470 / bank 3 171491620; the old lookup: 0 lines). Re-run with prg_window under the parallel lane. Headers corrected: ~94 min a leg, not 3 h and not 75 min |
...
| **PUSHED (maintainer's order: *"1) push 2) check if all learnings have been documented 3) end of session ritual"*)** | static tier for the close **135/0/0 GREEN**, the close commit, `git push origin main`: `b33ccfce..ab6c18d2`, 15 commits, `main` = `origin/main` |
| **THE LEARNINGS AUDIT, grep-driven — what had NO carrier and got one** | three new port-skill rules anchored at their gotchas: [VSP-176] the `${VAR:?}`-after-trap exit-0 class (explicit tests; the runner's exit-0 shell-error rule; a verdict-less PASS is a crash until read), [VSP-177] a comment never follows a line-continuation backslash, and a tool that provisions from nothing has a control that RUNS something in it, [VSP-178] a frozen expectation FOLLOWS whatever moves it whatever the cadence says, and the freeze commit carries the release directory (a new gotcha entry for the two things the first runner-driven MiSTer lane found). Amended: [VSP-95] (frozen constants that follow the romset re-freeze at every freeze), [VSP-100] (the release directory in the freeze commit; the roundtrip gate's positional default is outside `test_header_defaults`'s reach — an open gap), [VSP-164] (the timeout column, `--jobs` on the mister lane, §12/§13); level 0: [MJC-37] one clone = one simulation and a fresh clone must be PROVEN to simulate, [MJC-48] look a probe up by the BANK it watches, [MJC-54] a worktree copy under execution is the same file. Anchored paragraphs extended in place (MSC-48's with the qsound_ext lookup, MSC-54's with the worktree case, VSP-110's with the sixth trap — a `> f` on a command group truncates `f` before the group reads it, paid on the pair file). The two dated `$4BE7C0` citations (`mister_core.md`, HANDOFF) now say the address is the 14z-107 layout's walker, `$4C13D0` on merged-m16. Guides regenerated, census re-frozen (+3 anchors), memory notes: the parallel lane with measured runtimes, the two offered machines, the background-task watchdog. **OPEN, small:** `test_header_defaults` does not cover a positional `[name]` default (the roundtrip gate's said m15 while the code said m14) |
| **CLOSE (2) — static tier, and the red it caught in my own record** | the first close tier after the learnings pass went **133/0/2**: `test_checkskills` and `test_doc_anchor_census` — because the learnings row above had written the three new rule IDs in BOLD inside a session table, which is an ANCHOR to both tools, and STATE anchors are confined to the two standing sections (the checker's own 14z-114 control, firing on the real tree). Plain citations now; both gates PASS; the tier re-run **135/0/0 GREEN** (`build/static_14z134_close3.log`). Committed and PUSHED at the maintainer's word (*"you can push on green"*) |
