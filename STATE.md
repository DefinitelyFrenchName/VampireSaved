...
| **(12) the slide is the PUSHBOX SEPARATION routine** | the only writer of the victim's x through a light hit is vs2 `0x17D30-0x17D7A` (splits an overlap between the two fighters, or gives it to the one not cornered `+0x38`), driven by the reaction nodes' push boxes; `+0x40` stays 0 — no pushback velocity on light/medium. Reference push boxes at `0xA776C` read in the DATA view (the opcode view of a table is crypt noise — dumpops must use the program space for data) |
| open (phase 3) | how `+0x1C` couples to the separation (unread); the unindexed lying/wake nodes (entered by game-logic jumps — the per-victim pose tables of [VSE-44] are the candidate source); projectile parameter records; the `gap_*` auto tables |
...
| **(6) the open fields, by correlation** (same day; part 11 = the contacts BLOCKED, P2 holding away; fields: both bars, victim velocity/combo/freeze) | **`+0x14` = the attacker's meter gain on hit** (13 contacts across 7 records: 6/12/18 L/M/H normals, 3 per LS tick, 9 Ifrit, 2 column; a fraction on block; the victim gains 8 per hit, 0 on block); **`+0x1C` scales the pushback** (27/41/59 px at 15 f for 0x14/0x1E/0x28); observed only: `+0x12` = strength index, `+0x16` = 1 on specials/projectiles; `+0x11/+0x13/+0x15` open; a blocked contact writes class `0xFF`. In the tool, the map tables (`str`/`meter`/`spc`/`push` columns) and `engine_internals`. Rig lesson: P2 "holds back" = R when it faces left — the first block run had Victor walking INTO the attacks |
