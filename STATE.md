...
| **(2d) THE PRE-COMMIT TIER WENT RED ON A FINDER FILE — the gate measured the host** | strict tier **149 / 0 / 1**: `test_release_asset_shape` §5 `left out of every asset: .DS_Store`. The file was `release/merged-m18/.DS_Store`, gitignored, written by Finder at 22:16 local — two hours AFTER the upload (20:28) and after the gate's last green (20:26) — so no published zip holds it (the served file counts also add up with none). Removed: the gate PASSES, both controls fire, both modes FAIL; the whole strict tier re-run before the commit. **OPEN, with a recommendation:** Finder recreates these whenever `release/` is browsed, and `tools/upload_release_assets.sh` lists a platform directory with `find -type f`, so a `.DS_Store` INSIDE one would be zipped into its asset — teach the uploader and the shape gate to ignore `.DS_Store` explicitly rather than deleting by hand before each run. Not done tonight: the tree stays still under item 3's run |
...
| **CLOSE (2) — the ritual** | STATE rolled (the 14z-149 group to the archive with its ledger line, 186 -> 173 KB; the remainder is standing sections, which never roll), NEXT_SESSION rewritten and the 14z-149 opener archived BLOCKQUOTED at the top of its history, the `release-inventory` memory note updated to the three-OS state. **THE DECIDING RUN IS `build/static_14z150_final2.log`: 146 / 0 / 0 GREEN, `fired 148 / declared 148`, `executed 148: honoured 148, lies 0`.** The run before it was 144/0/2 and BOTH reds were one cause, caught by the kept-log instrument again: `docs/project/WINDOWS_BUILD.md` used a bare markdown AUTOLINK (`<https://…>`), which `tools/md_subset.py` does not support — so `test_md_subset` and `test_docs_site` both failed, the latter also reporting the three in-tree hrefs left pointing at a page the render had refused to produce. Fixed in the MARKDOWN (`[text](url)`), not by widening the parser: the subset is a measured census, and a document is the cheaper thing to change. **Three separate reds this session were diagnosed in seconds each by a log that would not have existed yesterday.** |
...
- Session 14z-149 CLOSE (4) — **THE PREBUILT EMULATOR BINARIES, macOS: built from the recipe, made SELF-CONTAINED, recorded, gated by a BOOT on each, and packaged into M18 — then RULED release assets (never git content) with pruning, vhunt2 DROPPED from the release (the oracle, never a source: three dumps, not four), and EVERYTHING AVAILABLE PUBLISHED with every README explaining each deliverable** (5 assets on freeze/merged-m18). Two shipped-recipe defects corrected in place: the FBNeo fresh-tree build needs TWO passes ([CPE-26]), and -verifyroms says "is bad" BY DESIGN on a content set. Census 85/0/0; static strict 145/0/0, 146/146 controls honoured; no build byte moved; pushed  [rolled 14z-150 close, early — STATE was 186 KB]
