...
| not done, by design | item (2) Zabel j.LK — needs the maintainer's hand-played RECORDING first ([VSP-20]); item (3) Jedah's crouching recovery (needs the tick-accurate instrument); the frame-data-privacy implementation (awaits the class ruling); the HANDOFF shape item; the ~90 unadopted mizuumi candidates |
...
**Ledger rollover:** the 14z-122 group (four records) AND — per the rule's
~150 KB line, which the file still crossed after it — the 14z-123 group (two
records) moved verbatim to STATE_HISTORY.md at the 14z-125b close; STATE holds
14z-124 / 14z-125.
