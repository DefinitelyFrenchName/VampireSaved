...
### STANDING NOTE (maintainer, 14z-89 close): **AN ADVERSARIAL CODE REVIEW
### of the project has been run and its findings are being assessed and
### addressed.** Corrections only — nothing about the project's nature or
### its parts changes. But some findings touch THE TEST HARNESS and the
### tooling the current explorations rest on, so the next session's TASK
### ZERO is to go over what was fixed and decide whether it moves the state
### of the work. The bugfixes are worth having; this is an assessment, not
### a rollback.
###
### WHY THIS SESSION IS EXPOSED: 14z-89 produced 93 `.masked` specs, 35 new
### vanilla basis logs, a frozen dispatch census and two root-cause
### attributions — every one of them generated BY the harness. Ranked
### exposure and the ordered check (canary first, then data, then suites)
### are written out in `docs/NEXT_SESSION.md` "TASK ZERO"; the single
### cheapest canary is `VERIFY_BASIS=16_xemu_2p tools/freeze_masked_basis.sh
### tests/expected/vsavj/masked-v2 "<the V2 mask>" 16_xemu_2p`, which
### re-derives a frozen basis log and demands bit-identity. **If it fails,
### do NOT re-freeze to make it green** — re-derive deliberately and record
### which artifacts moved.
###
### WHAT DOES NOT NEED RE-LITIGATING WHATEVER THE HARNESS FIXES: the legacy
### regression was confirmed with RAW FULL-RAM DUMPS (replay 38 P2 HP 87 vs
### 88; replay 24 P1 X 324 vs 570 / HP 144 vs 115) and the two root causes
### by REMOVAL EXPERIMENTS (take the hook out, the divergence goes away).
### Neither depends on the comparators. A harness fix can move the SHAPES we
### froze; it cannot unmake those measurements.
