...
## Donovan throwing JEDAH uses Donovan's own victim keyframes, a 14z-64 fix right on the stock track and wrong on WIDE,
## pre-existing and unnoticed for eighty sessions. TWO NEAR-MISSES OF MY OWN, both now written down: a freeze into an
## EMPTY expectation dir self-freezes the whole legacy corpus, and my first matrix read invented four false differences.**

| | |
|---|---|
| opened with | the opener read (CLAUDE.md, STATE, HANDOFF, `docs/NEXT_SESSION.md`), ROM audit **76/76**, the skill `vampire-saved-port`. The maintainer picked **Pyron row 0x11** from the open-items list, then ruled **"Freeze M17 now"** |
| **(1) THE PORT, and every number in the spec re-measured rather than trusted** | `pyron.toml` `[[data_port]] pyron_capture_keyframes`: `src 0x0C7F98` (vs2's table `0x0D8418` row `0x11`), `orc 0x0C782A` (vhunt2's `0x0D7CAA` row `0x11`, sibling delta `0x76E`), `dst 0x094954` / `dst_old_head 00400140…` (vsavj's row `0x11` aliases row `0x01` = Demitri), `hole wide_ext`, `only_variant_slot`. **`len 0xB80` DERIVED, not assumed:** a `0x40` victim-offset head then 8-byte records, 18 distinct sub-blocks spaced `0xA0`, so `0x40 + 18*0xA0` tiles EXACTLY and the last record ends there. The old sketch's `~0x2040` was the ROOM to the next block, not the extent — wrong by 2.8x — and NEXT_SESSION's "zeros after" was wrong too (a different table begins at `+0xB80`); corrected in three places |
| **the proof, three ways** | STATIC on the shipped image: the placed block byte-identical to vs2's, repoint inventory exactly `0x00-0x0F, 0x11, 0x18`, Demitri's vanilla block untouched. IN-EMULATOR: `audit_pyron_capture_block` vs native `vsav2`, **overlap 9 of union 9** (14z-131: 0 of 15), legacy control 6/6, `EXPECT_MATCH` default flipped `0 -> 1` so the gate PROVED the fix rather than being rewritten to suit it. AND THE MAINTAINER, on a before/after/native sheet at six matched keyframes: *"After and Native look identical or at least consistent, whereas before was inconsistent with native"* |
| **confined to Pyron, measured not asserted** | donovan `8065bc92`, huitzil `08944a7e` and the stock twin `e86e1d04` all rebuild BIT-IDENTICAL (`m5_stock16` byte-identical to `m5_stock15` at member level); `test_shared_writes` unchanged at 115/113/102 — the poke lands inside the measured variant-row exemption |
| **(2) THE M17 FREEZE** | donovan-m21 / huitzil-m28 / pyron-m22 / merged-m17, mark **M17** (14z-132 option A: the mark IS the merged build number). Suite **GREEN 88/88 on all three solo tracks**, `.masked` verdicts 53/52/52 — equal to the carried counts, which is what says the legacy oracle actually ran. Registry rows whole-set keyed; four annotated tags; the 136-file re-point sweep; release `merged-m17` packaged and round-tripping byte-identical; the MiSTer tail done (fork commit `b92bd2a9` pushed, series `0033`, pin bumped) |
| **THE NEAR-MISS THAT MATTERS MOST, and it was mine** | The first freeze created the new expectation sets EMPTY and ran `--freeze`. `run_suite.sh` self-freezes any replay with no `.masked` in the target dir, so it wrote a `.sha1` for the WHOLE legacy corpus and printed `frozen <sha>` for each, exit 0: the three sets came out `masked=0` against predecessors carrying 52-53. **That is [VSP-36] reached by OMITTING A STEP rather than editing an expectation** — the vanilla oracle replaced by a tautology, greenly, for every future session. Caught by counting the files before trusting the run. **The CARRY step was written down NOWHERE**; it is now a gotcha (with the tell: `01_attract_long` reading `frozen` means stop) and a procedure block in HANDOFF's freeze section, with the acceptance check being that the new set's `.masked` count equals its predecessor's |
| **(3) THE MAINTAINER'S ASK, and it paid immediately** | Their words: *"that makes a strong argument for checking the whole combination of VS2 tenants thrown ... as well as our VS2 tenants throwing others"*, noting the existing work was *"only in targeted cases known to be wrong"*. **`tests/audit_capture_matrix.sh`** (ci_static, ~2 s): **640 (attacker,victim) cells over 20 reachable attackers** against native `vsav2`, where the three in-emulator gates covered three. It re-measures the #104 legacy-safety premise instead of citing it (16x16 byte-identical, so #104 was ADDITIVE) and declares its exclusions (Dark Gallon compared against VANILLA; eleven characterless ids) |
| **AND IT FOUND ONE ON ITS FIRST RUN** | **Donovan throwing JEDAH uses the DONOVAN-victim sub-block.** `donovan.toml`'s 14z-64 mirror-victim `fixes = "0x1E:0b30:0d88"` rewrites victim entry `[0x0F]` — RIGHT on the stock track, where Donovan occupies `0x0F`, WRONG on WIDE, where `0x0F` is Jedah restored. The manifest's own note calls it harmless because `[0x0F]` "is never a TENANT victim there": true, and not the question. **225 of 408 bytes differ; the last keyframe is `(-61,166)` against Jedah's `(-76,32)`.** PRE-EXISTING, identical on merged23. Frozen as a KNOWN-OPEN divergence and recorded as a decision — it is a gameplay surface |
| **the second near-miss, mine too** | My first matrix read compared a FIXED 20 records and reported four differing attackers — `0x06/0x08/0x09/0x0D`, which are EXACTLY the four whose sub-block spacing is under 20 records. An alarming result is as much a bug report about the instrument as a clean null is ([VSP-148]'s mirror). The gate now carries that as **control 4b**: the naive comparison must keep inventing differences at exactly those four, or the gate fails |
| **a third, cross-repo** | `test_bbh_fidelity` went red because `~/Developer/blackbox-harness/lib/py/bbh/config.py` carries THIS project's build literals as its defaults — **a lineage freeze rots the harness**, and the in-tree re-point sweep cannot reach it. The fidelity gate is what caught it. Re-pointed there (config, docs, selftest); the sweep's "grep FOUR places" ([VSP-96]) is now FIVE |
| **the merged set, verified not assumed** | merged-m17's expectation set is CARRIED from merged-m16 (53 `.masked`, 19 `.skip`, the mask) because neither the capture row nor the mark is legacy surface — and `audit_legacy_pairings` was RUN on `build/m3b_merged25` at the close: **LEGACY-PAIRING COVERAGE: PASS**, which is the gate the set's own README names as its verification |
| **CLOSE** | ROM audit 76/76 at the opener. Static tier **142/0/0 GREEN** after the sweep, the freeze tail (pointer-flow baselines re-frozen with the 0xB80 relocation and the `code:move_imm` shrink ROOT-CAUSED — the immediate is byte-identical, it became WRITTEN by a relocated data op), the artifact manifests re-frozen with the delta named, and the char pages re-frozen (only the `.html` hashes moved; the frame-data `_anim.md` hashes did not). Committed to main, NOT pushed |
