...
## Session 14z-148 — **THE --controls CADENCE RULED (a), AND THE MUST-FIRE READER LIFTED INTO bbh AS A COPY — F1 over
## declaring stubs, F2 exact at 70 rows, the readout text made generic on both sides. No build byte moved.**

| | |
|---|---|
| opened with | the opener read (HANDOFF, STATE, NEXT_SESSION in full — the next-session file is `docs/NEXT_SESSION.md`, not under `docs/project/`), ROM audit **76/76**, `main` == `origin/main` at `ae2037e5`, the skill `blackbox-harness` loaded before touching the harness. The maintainer's three questions: is anything owed for item 1 before the next freeze/release (no — the sweep IS the verification; the ruling needed a home), item 2 agreed, item 3 explained |
| **THE RULING, recorded in place (commit `4ee33d38`)** | the cadence of `run_all_emulator.sh --controls` at release is option (a), THE RELEASE RUN, not the freeze sweep — *"I agree with item 2's proposal. We can always adjust later if the need arises."* There is no document literally called "release checklist": the release procedure is the port skill's D.4, HANDOFF's emulator-tier command block and STATE's release-scope section, so the invocation now sits in HANDOFF's block (`--scope all --lane all --strict --controls`), the runner's header and the contract page say RULED, STATE's must-fire entry is DECIDED, NEXT_SESSION's bullet struck; the retraction grep for "checklist decides" is empty outside archives. The pre-commit half (`--exec-controls all` default) was never put as a question and stands |
| **ITEM 3 EXPLAINED, then RULED "copy, not externalise"** | bbh is the generic harness; its fidelity contract demands identical output from its runners and this tree's over the same input. 14z-147 taught this tree's static runner the `# MUST-FIRE:` grammar and its readout; bbh's knew nothing of it, so F2 (opt-in, the real portable tier) carried a delta. The maintainer asked whether the lift would be a copy or an externalisation (*"you had me worried"*) — a COPY, under the two standing rulings (this tree never consumes bbh; independent but compatible), the two copies held together by the fidelity gate, not by shared code. *"I agree and you can start on that"* |
| **THE LIFT (harness `02d58f3`)** | `lib/sh/controls.sh` (the four regexes as `bbh_ctl_*`, a copy of this tree's `tests/lib/controls.sh`); `classify.sh` gains the 4th argument (the gate SCRIPT) and `bbh_classify_control` (HONOURED / LIES / REFUSED / DIED / TIMEOUT); `bbh-classify --gate` / `--control`; `bbh-run-static --exec-controls all\|portable\|none` with the ledger, `exec_controls` and the readout printed only when a tier declared or executed anything; `bbh-run-sweep --controls` with the `<gate>@<name>` rows and the readout on every run; `selftest/run.sh` hands the script (the harness eats its own verdicts); `example/tests/g_control.sh` the worked instance (declared, fired, executable, one `perturb` function; registered, indexed); `selftest/test_controls.sh` NEW (the lineage's cases + the executable verdicts, dogfooding two `known-bad` controls); `test_run_static` §10-12 and `test_run_sweep` §16 with a `shadow-tool: reader-unplugged` must-fire each (a copy of the runner with the script dropped from the classify call, under a throwaway home whose `lib/` is the real one); `docs/gate_contract.md` §2 prologue on the grammar, §5.1-5.4 NEW carrying **[BBH-88..91]**; SKILL.md +4 rules (91), GUIDE regenerated, `check-skills` ALL PASS; README layout; `conventions_history.md` the dated entry |
| **TWO VERDICT-TEXT CHANGES, BOTH SIDES, ONE SITTING (bbh convention 8)** | (1) the readout header was `== must-fire controls (tests/lib/controls.sh) ==` in this tree — it named this tree's lib path, which bbh cannot honestly print — so it is the generic `== must-fire controls ==` in `run_all_static.sh`, `run_all_emulator.sh` and both bbh runners; (2) a controls-red gate (exit 0, the block RED) printed `FAIL (exit 0 after a shell error)` on both sides — a lie inherited from the FAIL branch's `_st != 0` test — and now prints `FAIL (controls RED: flip(not fired))`; the shell-error text is untouched for the shell-error case so every existing grep holds. Both re-baselined LOUDLY in bbh's `rebaselines.md` (its follow-up commit, naming this tree's) |
| **MEASURED** | F1 extended with nine declaring stubs through both runners: IDENTICAL — `read: fired 5 / declared 7`, `executed 5 honoured 1 lies 2 refused 1 died 1` (my first expectation said declared 8 / lies 1: the header-only `g_cfired` stub is executed too and LIES, which is right — the mode is what makes FIRED more than a self-report). **F2 re-measured: IDENTICAL, 70 PASS rows** (65 at H1), run with `--exec-controls none` on both sides because executing this tree's ~100 portable controls costs ~20 min per runner; the executed half is proved over F1's stubs. bbh selftest 31 PASS / 2 SKIP (standalone lua; the emulator opt-in) strict; F8 on the real emulators PASS 200 s; `test_fidelity_vampire` PASS with F5=1 F6=all F7=all. This tree: `test_static_runner`, `test_emulator_runner`, `test_controls_contract` green after the text change; the eight doc checks green after every edit |
| **TRAPS PAID THIS SITTING** | (1) bbh's pre-commit NEEDS `ROMDIR` exported or `test_fidelity_mame` SKIPs and F6 runs synthetic only — my first full run did exactly that; (2) `kill <pid>` on the background runner's child left the runner alive: my first selftest kept running under the second, two fidelity runs plus ANOTHER session's BBX selftest on this MacBook, and the OS killed mine for low memory — kill the PROCESS GROUP (`kill -- -$pgid`) and check `pgrep` before relaunching; (3) a python replace anchored on a usage block's column spacing aborted the whole edit script — anchor on a line's tail |
| **the memory notes** | `blackbox-harness-repo` (the lift, "copy not externalise", the ROMDIR trap), the index line |
| **CLOSE** | The ruling recorded (`4ee33d38`), the lift landed on both sides: harness `02d58f3` + the re-baseline line `777dbde`, PUSHED FIRST per bbh convention 8; lineage `f3019df8`. **Static strict 145 / 0 / 0 GREEN, `fired 143 / declared 143`, `executed 143: honoured 143`** (`build/static_14z148_close.log`) — the identity bar unchanged from 14z-147b/c; run DETACHED after the session's low-memory guard killed the first attempt during a Verilator control mode (trap 2 above, now a memory note). The eight doc checks green after every edit; STATE rolled (the 14z-147 group to the archive with its ledger line, 178 -> 159 KB, one group kept as at the last two closes); NEXT_SESSION rewritten, the 14z-147c opener archived; the two memory notes. **No build byte moved, M18 untouched.** Pushed per the close rule |
| **(2) 2026-09-11 — THE RELEASE INVENTORY, MEASURED THEN RULED** (the maintainer's *"do we continue?"* → the first half; *"I agree with the inventory"* → the second) | `release/merged-m18/` measured file by file (11 MB: 2.6 MB per emulator dir, 5.5 MB MiSTer). The deltas: **282 KB literal generated bytes, 17.5 MB copy instructions in ~157,000 fragments, 9 MB fill** (`xdelta3 printdelta`, parsed) — which settles two paths: an MRA-ONLY MiSTer deliverable is NOT reachable (MRA parts do take `offset`/`length` and inline `<patch>` bytes, but the WIDE gfx members are ~26,000 tile-sized copies each), and a dependency-free applier IS (no secondary compression in the files — `xdelta3 printhdr`'s "lzma" line is the BINARY's default, a gotcha). Three defects found in the shipped package: the README's fingerprint printed `?` since the whole-set keying; the play-time pristine zips named only in the platform notes; nothing measured about an unpatched emulator. **RULED (maintainer): MiSTer as today (RBF + MRA on top of the patch set — "the MRA are not a problem"); emulators = recipe AND prebuilt, each user free to choose (1+2)** |
| **THE STOCK-EMULATOR QUESTION, MEASURED** (the maintainer: *"what happens if you try the wide rom on a non-patched emulator: I know mame will scream but it will boot it, I just don't know how bad it is afterwards"*) | a stock MAME does not know `vsavjw`; RENAMED to `vsavj.zip` on the WIDE=0 reference binary it prints WRONG CHECKSUMS on the 8 patched group-A members, boots, and **sits on the QSound legal screen** — five snapshots (frames 300..4200) show the warning then the legal screen held and fading; RAM hashes IDENTICAL to the real WIDE run through frame 468 and divergent from 469 on all three replays (a legacy 2P match, the 0x0F pick, a tenant pick), 300 distinct hashes over the last 300 frames (running, not frozen), **no crash vector on any leg**. So the failure mode is a benign STALL: no gameplay, nothing "fine but wrong" — the old gotcha's "re-creates the music bug while looking fine" described a smaller program. FBNeo unmeasured (no reference build here; its CRC-then-0xFF-fill loader predicts no boot). Locked as `tests/audit_stock_emulator_stall.sh` (mame / out `platform:` / romset; frozen divergence 469; the WIDE-legs-identical instrument check; must-fire `wide-binary-no-stall` as an executable mode — the WIDE binary swapped into the stock leg has NO divergence and the gate FAILS, verified HONOURED). The maintainer tested on their side in parallel (*"we notice different things more or less easily"*) |
| **THE SECOND HALF, BUILT** | (1) `tools/apply_release.py` is PURE PYTHON: a VCDIFF (RFC 3284) decoder embedded in the one shipped file — default code table, address cache, no secondary compression (REFUSED, with a known-bad in the gate), 20/20 members in 1.4 s, verified byte-identical against xdelta3's decoder; one bug paid: the window adler32 is FOUR RAW BYTES, not a varint. (2) `package_release.py`: the END-USER README (what is in the package, what you need = Python 3 + the four dumps by exact name, build the romset, identify, **if it does not work** with the measured stall line, what is patched) and the build identity = the WHOLE-SET KEY (`--set-key`; `00f9cf13` for M18 where the README printed `?`). (3) `package_release_platforms.py`: a "Play on <platform>" section appended per platform; **the prebuilt-binary SLOT** — build resource `release/emulators/<platform>/<os-arch>/{files, BINARY.txt}` (sha256 per file, pin, patch), hash-verified into `emulator/bin/<os-arch>/`, none yet (this MacBook builds macOS only; Windows/Linux are the remote boxes) and EMULATOR.md says so; its "If it does not work". (4) `test_release_roundtrip.sh` §4: THE INVENTORY as an allow-list per platform (a file outside it FAILS), every BINARY.txt's sha256s, the five README sections; three new must-fires (`secondary-compressed-patch` in §1/§2, `stray-file`, `readme-missing-section`), all with modes. (5) `release_format.md` "WHAT A RELEASE IS" (the table, never-ships, the stall) and the superseded "never a binary" bullets corrected IN PLACE + HANDOFF + the packager docstring (claim grep empty). (6) `release/merged-m18/` REGENERATED by the new packager: patches and MRAs byte-identical, README/EMULATOR.md/applier/manifest moved. Roundtrip gate PASS, 8/8 controls fired; census re-frozen **84 / 0 / 0**; gate index 320 scripts |
| **CLOSE (2)** | **Static strict 145 / 0 / 0 GREEN, `fired 146 / declared 146`, `executed 146: honoured 146`** (`build/static_14z148_close2.log`; the three new roundtrip controls in the count; run DETACHED, ~2 h on a loaded machine where the morning run took ~1 h — a re-armed monitor, not a relaunch); the stall gate PASS plain and HONOURED under its mode; the eight doc checks green after every edit; STATE's release-definition decision DECIDED in place; NEXT_SESSION carries the prebuilt-binaries follow-up and the FBNeo half of the stall; the memory note `release-inventory`. **No build byte moved, M18's patches and MRAs byte-identical; the release's README/notes/applier/manifest regenerated.** Pushed per the close rule |
