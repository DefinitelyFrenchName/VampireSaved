...
| **(2) `PRG:0x028D50` — THREE NAMES, ONE ADDRESS, TWO TABLES THAT OVERLAP BY CAPCOM'S OWN LAYOUT — ALL THREE NAMES ARE RIGHT** | Static first (both decrypted views, capstone): the per-id BYTE MAP at `PRG:0x028D00` (vs2 `0x027FD8`, delta `0xD28`) has **64** `lea (pc)` readers in vsavj `0x02392A-0x025F16` (the reaction/effect handlers; vs2 65) and is what BOTH manifest names describe — huitzil's `effect_map_*` pokes and donovan's `hit_class_props_ext_*` port write the SAME six bytes `0F 1B 1F 19 0F 03` to `0x028D4E-0x028D53` (vs2's own entries for ids 0x4E-0x53; the merged build dedupes them to ONE op). The guard-mash MASK TABLE at `PRG:0x028D50` is EIGHT LONGWORDS indexed by press count×4 — the ONE reader in the image is `0x027616 lea $28d50(pc),a0 ; move.l (a0,d0.w),d2`, then `btst d0,d2` on RNG&31; popcounts **0/0/0/8/16/24/32/32** = the atlas's "3: 8/32, 4: 16/32, 5: 24/32, 6+: always". So **map entries 0x50-0x53 ARE mask[0]**. Capcom's vs2 carries the identical bytes there (its check uses press weights and reads no table; its copy at `0x02802C` is dead, one longword shorter). mask[0] is reachable only at count 0, and the count is PRE-incremented (`0x027606 addq.b #1,$170(a6)`, cleared by the block-entry handler `0x024AF2/0x024B1A`, the arm at 8 stops the check): index 1..7 by construction |
| **the in-emulator half, and the gate** | `tests/audit_guard_mask_reads.sh` (mame / release / romset, ~4 min): the advancing-guard rig (Victor 5MP/5HP into a blocking, mashing Demitri — the same legacy pairing `test_advancing_guard` runs) under a `-debug` READ watch over `0x028D50-0x028D6F`, pristine vsavj vs merged-m18. **RESULT, both legs: 22 hits; every in-play read is PC `0x02761E` (the post-instruction PC of the `move.l`), A0 = `0x028D50`, offsets `+4 ×6, +8 ×6, +0xC ×6, +0x10 ×2` (counts 1-4) — OFFSET 0 NEVER, inventories IDENTICAL ours == vanilla**; the `DUMPS` through the same program space shows vanilla `00000000` and ours `1F190F03` at +0, mask[1..7] equal. Three must-fire controls (a synthetic offset-0 read, an empty trace, a foreign reader in play). **So the port's bytes at `0x028D50` are inert on the legacy path — not because "vanilla never writes ids ≥ 0x4B" (true of the MAP) but because the MASK table's entry 0 is never indexed.** No ruling needed: nothing to rename — the manifest names say what the port WRITES, the atlas name says what vanilla READS, both at one address; recorded in both manifests, `engine_internals`, and a platform gotcha |
| **the static tier on the item-2 tree: 142/0/1, and the one red was the COMMENT edit** | `test_charmap_current` — the charmap json/md record the manifest FILE's SHA-1, so a comment added to `donovan.toml` / `huitzil.toml` moves the page. Regenerated per the gate's own instruction; the review diff is exactly FOUR lines, all the `manifest` hash; `charmap_pages.sha256` unchanged (the out-of-tree pages do not embed it). Gate green; the full strict tier is re-run at the item-4 close and covers this tree too |
| **two instrument facts paid for on the way** | (a) MAME's read watchpoint reports the POST-instruction PC (`0x02761E` for the access at `0x02761A`) — the gate accepts either; (b) **a Lua `DUMPS` read through a watched range IS a watched read**: it logged one hit at dump-frame+1 (2000 -> 2001) with the CPU's incidental PC `0x122C`, which the first reducer read as "another reader in play" — the dump now sits at frame 1200, inside the boot window, beside the self-test's own sweep hit (PC `0x926`, frame 1) |
| **(4) THE COMMUNITY CROSS-CHECK AERIALS — the two-direction jump rig BUILT, and the outliers were a JOIN error** | Static first: a2 carries a SECOND aerial slot set `0x18-0x1D` beside `0x12-0x17`, and the two DIFFER exactly where mizuumi lists a neutral `8J.x` beside a forward `9J.x` (BU MP/HP/MK, BI LP/HP, FE LK/HK, LI HK, VI HP, SA LP/LK); elsewhere the forward entry ALIASES the neutral chain (VI all but HP, BU LP/LK/HK, LI HP) or duplicates it. `tools/vanilla_join_rig.py` gained the `jump_fwd` set (UR where `jump` is U, the button on the same frames) and an `--airborne` column (+0x14 left its pre-event value on every press frame); `tests/test_vanilla_aerial_join.sh` runs 15 × 2 × 6 legs and freezes `tests/expected/vanilla_aerial_slots.tsv` (180 rows). **MEASURED: NEUTRAL enters `0x12-0x17`, FORWARD enters `0x18-0x1D`, and where the slots alias the same address is entered — 22 of 90 forward rows.** Two character facts fell out: **ANAKARIS has no neutral-jump attacks** (U puts him in a HOVER chain `a:0x12`, +8 px, ~112 frames, every button ignored; his forward jump fires all six) — declared and ASSERTED, a void leg is never a pass ([VSP-170]); **ZABEL's forward jump enters his NEUTRAL set on all six** although his `0x18-0x1D` hold different chains (what enters those is unmeasured) |
| **three instrument defects on the way, all caught by the gate's own controls** | (a) the analyser took "the first new chain after the event's frame 0", which for a jump recipe is the JUMP itself — 180 identical `a:0x0e` rows, refused by the must-fire control (a vacuous table); it now observes from the button window; (b) the airborne column read a hover's +8 px as "air" — it says what it measures; (c) **the node map's "first chain seen wins" mislabelled AU's neutral LP as `a:0x34`**: a table-a chain with no terminator walked 215 nodes through the start of `a2:0x12`; the map now walks a2 BEFORE a, and `test_vanilla_frame_join` + `test_tick_durations` re-run to prove the standing/crouching tables did not move |
| **the join corrected, and the blanket rule that was WRONG** | first try: "the workbook's aerial row is the forward variant" — BI/BU/FE/VI cleared but ZA and LI went INCONSISTENT: Zabel's rows are his NEUTRAL chain and Lilith's sheet carries `J.HK` TWICE (one per variant). Which variant a workbook row documents is a fact about the WORKBOOK, so it comes from mizuumi's own move lists — `FORWARD_ROWS` in `tools/crosscheck_framedata.py` (BI LP/HP, BU MP/HP/MK/HK, FE LK/HK, VI HP, LI's second `J.HK`; BI's `J.HP8` is neutral by name) — and NEVER from which chain's numbers fit better ([VSP-166], the standing-normal lesson). Re-frozen: every moved verdict moved the right way — **BI active/red/white EXACT (were INCONSISTENT), BU active/red EXACT + startup CONSTANT OFFSET, FE active EXACT, VI active EXACT + startup CONSTANT OFFSET, LI gains a 22nd joined move with its verdicts intact, ZA untouched**; BI startup stays INCONSISTENT 17/18 (spread +0..+1, one move). `engine_internals` "ids 0x18-0x1d entered by nothing" (Donovan, part 13) RE-BOUNDED: ground inputs only; for a tenant the forward-jump set is unmeasured |
| **verified, then the tables the node map feeds re-proven** | `test_vanilla_aerial_join` frozen then re-run clean (180/180, every structural assertion, the control fires); `test_vanilla_frame_join` PASS (the standing table did not move under the a2-first node map) and `test_tick_durations` PASS 18/18 (nor the crouching one); `test_community_crosscheck` re-frozen and green with its controls; the out-of-tree full page regenerated (`framedata_pages.sh`, tenant pages hash-equal). STATE rolled early (14z-144 -> STATE_HISTORY; 161 KB, one live group left) |
| **(5) THE TENANTS' FORWARD-JUMP SET — measured, and it is the ALIAS case, not Zabel's, for two of three** | `tools/name_moves.py` gained `jump_fwd` and a part per tenant (donovan 14, pyron 6, huitzil 10: `j.x [9]`, pinned far, owned by the maintainer's `j.x` rows); rigs committed, `test_move_naming` re-frozen — the ONLY lines that moved are the 18 new ones. **All eighteen forward-jump attacks enter `a2:0x12-0x17` on native vs2.** The extracts say why: Donovan's and Huitzil's `0x18-0x1d` entries ALIAS `0x12-0x17` chain for chain, so the 14z-121 "entered by nothing" was true of the slot numbers and vacuous about the chains; Pyron's `0x1b-0x1d` alias, `0x19/0x1a` near-duplicate `0x13/0x14`, and `0x18` is a DISTINCT 4-node LP chain his forward-jump LP does not enter — Zabel's pattern; *CORRECTED the same day: `0x18-0x1a` are his `j.2P`, MEASURED at 14z-120 and sitting in the maintainer's move list all along ([VSP-155], missed)*. No naming decision taken: the `j.x` rows stay the maintainer's; the measurement sits in their notes and `engine_internals` |
| **(6) THE PHOBOS ±1 DAMAGE RESIDUE — ROOT-CAUSED, and it is the 2026-08-14 defense-row ruling's own observable** | STATIC first: the defender-side DEFENSE CURVE table (vsavj `PRG:0x0B8940` / vs2 `0x0D2ABE`, 32 B per victim id) differs between the games on EXACTLY ids `0x0A`, `0x10`, `0x13` (+ `0x19/0x1A`, not roster victims) — the residue's victim set, no more, no less; `0x10/0x13` are content-SWAPPED (the port keeps vanilla's rows by ruling, so Phobos rides Bulleta's curve and Donovan Victor's — opposite signs), `0x0A` is a CROSS-GENERATION retune of Sasquatch (a legacy character, nothing of ours — the hypothesis STATE carried, confirmed). The reader `0x18C08`: `d0 = (victim id & 0x1F) << 5 + victim's +0x3B3` (= the attacker's id), `move.b (a0,d0.w),d3` — d3 seeds the final 2D damage-map row. **IN-EMULATOR, `tests/audit_defense_row_residue.sh`** (mame/release/romset, ~5 min; the CS rig under a `-debug` read watch, ours vs native, victims 0x10/0x13/0x0A + control 0x03): every leg reads row = victim, column 16 (Phobos), the index sequence identical on both legs; bytes answered **0/2, 2/0, 1/0 and 2/2** — differing on exactly the residue victims, equal on the control. Four controls run the real predicate. PASS on the first run; reproduced. Recorded in `defense_rows.md` (the ruling's observable), `engine_internals`, the geometry gate's header. Nothing to fix: re-ruling option (a) would move the two tenant cells and leave Sasquatch's. Static tier 144/0/1 — the one red was `test_must_fire_census` refusing an UNLISTED declaring gate (this one), which is the census doing its job; re-frozen after review (79 declaring, 10 header-only), gate green |
| **(7) THE TWO VANILLA EDGES FROM THE AERIAL WORK — Zabel's `0x18-0x1D` and Anakaris's hover — ANSWERED, one by enumeration, one by disassembly** | The aerial gate gained a THIRD direction (neutral jump, then D+button; `jump_down` in `vanilla_join_rig`, the press-token rule fixed for `D4`-shaped tokens): 45 legs, 270 rows frozen and reproduced. **Zabel's `0x18-0x1D` are his D+BUTTON aerials, one slot per button** — the workbook's `J.2LP..J.2HK`, now named from the measurement (`vanilla_frames.aerial_slots`, never a fixed layout: `9J.x` where a forward jump enters a second-set chain, `J.2x` where D+button does, neutral slots never renamed) and joined: ZA active/red/white EXACT on 24, startup the constant offset, gauge 23/24 (one `J.2x` cell −6); Aulbath's `J.2HK` is `a2:0x51` (joined, verdicts intact); Anakaris's D+button in the hover is the HOVER DROP `a:0x36` (3 nodes, back to `a:0x00` four frames later) on all six — not an attack; his `J.2K` live off a real jump, unmeasured. **Anakaris's hover, static:** the jump dispatcher tests his id FIRST (`0x022A0E cmpi.b #6,$382(a6) ; beq.w $2678C`) and runs a PRIVATE jump handler (`0x02678C`, its own ten-entry sub-state table `0x0267BA`, sub-state 1 installing seq `0x34`), so the generic handler's attack sub-states never run for him — read from the table; the hover chain and the ignored buttons are the measurement. He is special-cased at the guard-mash check too |
| **(8) BISHAMON'S 5MK STARTUP — the workbook's, not ours** | `tick_durations.py --nodes` (ticks PER NODE of each pass, and the startup in ticks = the ticks before `frame_data.first`); `test_tick_durations` gained BI's standing leg: **all six standing startups equal in engine ticks — 5MK's node 0 spends exactly the 5 ticks its byte says** (5HK 6, 5HP 7, 5LK 4, 5LP 4, 5MP 9). The sheet's 5 is one low under its own +1 convention — the same shape as Jedah's crouching recovery (14z-126b): arbitrated, the residue is the workbook's. BI startup stays INCONSISTENT 17/18 on the page by construction (the column is a strict conjunction) and is now explained |
| **(9) THE LEFTOVERS REVIEWED (maintainer: "were they left behind with a strong reason?" — no; each looked at)** | **Pyron's `0x18`: RETRACTED, not unentered** — his move list had it since 14z-120 (`j.2P`: j.2LP/MP/HP -> a2:0x18/19/1a), [VSP-155] missed for a day; corrected in the doc, STATE row 5 and the opener. **Anakaris's `a:0x36`** is the HOVER DROP: D+button in the hover lands him in `a:0x00` four frames later, on every button — not an attack; his `J.2K` live off a real jump and need a forward-jump-then-D rig (not built). **Zabel's `J.2HK` gauge −6** is one cell of the GAUGE family below, not Zabel's. **The page's remaining cells, by family:** (i) the SETTLED workbook double-count (§2 of the page: MO 5HK, QB 2HK, SA 5HP, VI 2HP, JE 5HK — and by the same shape AN 2MP, LI J.HK/9J.HK, MO/QB/VI white); (ii) the named +1/+2 conventions with the workbook one low on BI 5MK and SA J.MP (startup +0) and LI 2MK (recovery +3, Jedah's class, closed) — plus AU 5HP startup +2, unarbitrated; (iii) the GAUGE column — SA 5HK/2HP (−18), BI 2HK (+6), FE 5MP (−10), ZA J.2HK (−6), LE J.HP (−18): the sheet's `gauge hit − gauge whiff` against our record `meter`, UNARBITRATED — no meter-gain rig exists; (iv) AU 5MP active +9 and LE J.HP active −1, unarbitrated. **A WRONG FIX, TRIED AND REVERTED BY MEASUREMENT (the trap of the day):** SA 5HK is `2(2)3` with both windows on hit id 1 and the hit rig says ONE hit for 12, while our per-hit sequence carried [14, 14] — so I made the sequence count a same-id window once. The re-freeze turned SEVEN characters' red/white EXACT -> INCONSISTENT and the rig refuted the rule itself: JE 5HP `2(5)2(5)2`, all three windows id 1, lands THREE times (4+4+3); MO 5HK, ONE window with two id-1 records, lands FOUR (3+3+3+2). **The re-hit rule is not "a hit id lands once": it is temporal or record-driven, and UNMODELLED** — [VSE-43]'s ring summary is too short; the gate `+0x147` (the multi-hit re-hit gate, atlas) is the place to measure. *(CORRECTED 14z-146: the rig stands at NEAR distance, so its MO leg is CL.5HK — four consecutive records with hit ids 1-4, not "two id-1 records"; the rule is MEASURED and it is record-driven: the victim's recent-hit slot, cleared on every gap node — STATE 14z-146, `tests/test_rehit_ring.sh`. `+0x147` is not involved on vsavj.)* Reverted; the committed verdicts stand; a KNOWLEDGE item, named in the opener |
| **SEVEN BBX FINDINGS ABOUT bbh AND THIS TREE, VERIFIED (maintainer's ask, after the close)** | (1) `MFI-52` does not exist here — MFI ends at 46; the instrument protocol is `[MJC-52]` (their constitution's prefix typo, not this tree's). (2) `304 gates / 4,000 expectations` are the 14z-135 extraction figures (`harness_scope.md`, dated); today `tests/*.sh` = **314**, the recursive glob = 322 = 314 + 8 `tests/lib`, expectation files **4,810** — bbh's README/doctrine reworded as dated. (3) TRUE: bbh's four dirty files were the M18 re-point of 14z-144 left uncommitted, and the toml comment named the wrong freeze — committed and pushed (`f675710` -> next). (4) TRUE: 77 of 314 gates carry "must-fire" as prose in ~15 spellings; no machine reader here — their R10 contract is the shape, not built. (5) TRUE: `example/tests/lib/needs_fake.sh` resolved `BBH_HOME` three levels up instead of two and worked only because every `bin/bbh-*` exports it — fixed with the reason; selftest 30/2/0. (6) TRUE, and sharper: in the agent's Bash tool `grep` is a FUNCTION over the bundled ugrep with `--ignore-files`, so interactive greps skip every gitignored path; gates run `/usr/bin/grep` — a project gotcha now. (7) the static runner's tree check exists and fires (this session too, see the charmap red) |
| **CLOSE (2)** | The sitting continued past its first close at the maintainer's word: the BBX findings verified (bbh fixed and pushed), the must-fire census (step one), the R10 read, the second open-items list (1, 2, 4 of the first; 1-4 of the second) all closed, the leftovers reviewed with one rule tried and refuted, the push-at-close ruling written into the law. **Twelve commits in this sitting, all pushed; no build byte moved; M18 untouched.** Static strict green at every commit (last: 144/0/0); every new gate frozen then reproduced. Two retractions of my own, both the same day: Pyron's `0x18` (prior art), the same-id dedup rule (the rig). STATE holds one live group at ~165 KB; nothing to roll. `emu/fbneo`'s modified content is the two patches, as always |
