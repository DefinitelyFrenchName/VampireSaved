...
## Session 14z-135b — **SLICE H2 LANDED: the comparison classes and the masked vocabulary, with
## FIDELITY F5 EXACT over all 1,891 masked specs** (the continuation, at the maintainer's "continue
## then" from 49% context). No build byte moved.

| | |
|---|---|
| opened with | the maintainer's question at 49% context — *"do we continue or close?"* — answered "continue: the close ritual is already paid, H2 is bounded, the harness commits at slice boundaries", and their *"continue then"* |
| **H2, lifted** | `tools/{compare_flicker,compare_window,compare_composite,check_diverge,describe_masked_shape,s4_thresholds}.py`, `tests/lib/{masked_compare,enumerate_expectations}.sh` and their five ground truths read in full, then lifted into `~/Developer/blackbox-harness` (commit `ef7e899`): `lib/py/bbh/thresholds.py` — FLICKER_MAX / RECONVERGE / MAX_TOTAL declared ONCE, consumer-overridable through `[thresholds]` (read via `BBH_CONFIG`), the proposer and every enforcer importing from there; `logfmt.py` — the `<frame> <hash>` / `END` grammar parsed once (it had three private `load()` copies and two raw-line variants); the five checkers with their VERDICT STRINGS UNTOUCHED and every lineage guard kept (FAIL-SHORT, the END off-by-one, the zero-row log, the truncated-log prefix trap, the inventory cap, the inter-run flag off by default); `masked_compare.sh` with its text frozen and the mask default from `[suite].mask_default`; `enumerate_expectations.sh`; `docs/method/oracle_classes.md` (the classes and what may never loosen one, numbers dropped). Six selftests: the lineage's cases verbatim plus config MUST-FIRE controls — under `flicker_max = 3` a 3-frame stretch is a FLICKER to the enforcer AND to the proposer, and the same shape is a WINDOW without the config. Suite 13 PASS / 0 SKIP / 0 FAIL |
| **FIDELITY F5, MEASURED: EXACT** | every `.masked` spec in this tree — 1,891: window 1,124 / composite 492 / exact 181 / flicker 49 / diverge 45 — paired with a DIFFERENT set's frozen log of the same stem (else its own basis log) and run through BOTH `masked_compare` implementations, verdict text + exit status compared: **1,891 / 1,891 identical to the character, 229 s.** Every class and the baseset/mask guard exercised on real specs. Kept as a section of `selftest/test_fidelity_vampire.sh` (every 4th spec by default so the routine gate stays ~1 min; `BBH_FIDELITY_F5=1` at a slice's pre-commit — the H2 pre-commit ran it at 1: 244 s, identical) |
| **what changed in the plan** | nothing in the contract; one runtime fact: the harness's own pre-commit gate is now ~5 min with F5 at every spec and ~1 min sampled, which is why the knob exists |
| **PUSHED TO GITHUB (maintainer, 2026-09-06: *"can you push it to its own repo on my github, as you proposed, for I think this is indeed the right move (you clone/fork it when you need it)"*)** | `gh repo create blackbox-harness --public --source . --push`, PUBLIC to match this project's own repository, branch `main`: https://github.com/DefinitelyFrenchName/blackbox-harness — three commits then (`803f372` H1, `ef7e899` H2, `ae0d6f3` the fidelity header), two README commits after (`28ecb0f`), `main` = `origin/main`. Pushing the harness is standing-authorised by that instruction; this tree's push state is unchanged (the maintainer's word, as always) |
| **CLOSE** | `harness_scope.md` §4 carries H2's landing; NEXT_SESSION points at H3 (fingerprint + suite + the fake driver — the largest slice, left for a fresh window by design); doc chain green; the 14z-134 group rolled EARLY to STATE_HISTORY (STATE was 156 KB; 131 KB after); static tier **PASS 135 / SKIP 0 / FAIL 0** (`build/static_14z135b_close.log`), tree clean during the run; committed. **FINAL: the tier re-run on the committed tree PASS 135 / SKIP 0 / FAIL 0** (`build/static_14z135b_final.log`); the opener tidied to one harness section; PUSHED at the maintainer's word (*"and then push"*). H3 was NOT started: it designs `fakesys.py`, the example machine, and that wants a fresh context rather than the tail of one |

## Session 14z-135 — **THE TWO BACKLOG DIRECTIONS OPENED, IN THE MAINTAINER'S ORDER: the generic
## harness SCOPED and its first slice LANDED in a separate repository, and the living-documentation
## effort SCOPED in all three of its forms** — two rulings at the plan stage, two scope documents in
## the precedent form, `~/Developer/blackbox-harness` H1 at `803f372` with its selftests 7/0/0 and the
## first two fidelity checks against this tree exact (F1 identical, F3 exact). No build byte moved.

| | |
|---|---|
| opened with | the opener read (CLAUDE.md, STATE, HANDOFF, NEXT_SESSION — the last at `docs/NEXT_SESSION.md`, not `docs/project/`), the ROM audit 76/76, then the maintainer: *"We'll do things in order but first, I need the generic reusable test harness and the living documentation effort. After that we'll tackle the open items lined up."* |
| **THE PLAN STAGE, and what was RULED there (maintainer, 2026-09-06)** | scope first, by the project's own precedent (`mister_scope.md` → D0-D5, `skills_scope.md` §7). Three census agents measured the tree at the opener (the harness inventory piece by piece with its couplings; the precedent scope-doc form and the doc-registration mechanics; the gate conventions over all 304 scripts, every count with its grep) and one design pass produced the four bins and the slice plan. Three questions put to the maintainer, three answers: **the harness is a SEPARATE repository** (recommended; "a top-level directory here" the alternative); **its name is `blackbox-harness`** (over `oracle-harness`, `replay-harness`); **the living documentation takes ALL THREE forms** — a rendered navigable site, routing enforcement in the markdown, fact tables with provenance. Plan approved as written |
| **the census, in numbers (each reproducible by its grep)** | 304 gates (233 `test_`, 67 `audit_`, 4 `run_`); line 2 `# <name>.sh — <claim>` is an API read by `gen_gate_index.py` (304/304 conform); 301/304 share the prologue; ONE verdict classifier duplicated in the two runners and DIFFERING (the sweep copy has the TIMEOUT and the exit-0-after-shell-error branches, the static copy neither); 93/304 gates carry a must-fire control in three shapes (perturb one byte 52, shadow tool 4, synthetic tree / known-bad 9); no common library (7 `tests/lib/*.sh`, `tools/_minitoml.py` the one shared module with 29 importers); 4,021 frozen expectation files in 52 dirs (1,891 `.masked`, 617 `.sha1`, 628 `.skip`, ZERO live `.diverge`); 181 replays; `replay.lua`'s machine couplings enumerated to the line (CPU/space tags, the RAM window, three port tags + 22 field names, 2-char `sys` tokens, the screen); `compare_fields.py` the ONE comparator with a game fact in source (the match-start anchor predicate and the p1/p2 bases); `build_fingerprint.py` needs only a member regex and a parent-set name as config |
| **`docs/project/harness_scope.md`** (REFERENCE, banner) | the extraction question one level above the skill cut — *would this still be true if the thing under test were not this ROM, not CPS-2, not even a game?* — and FOUR BINS: code / config / machine profile / stays. Every piece classified (§2, 40 rows) with its parameterisation and the ground truth that travels; the repository layout and config schema (§3); the slices H1-H9 with their order (H1 → H2 → H3 strict; H4/H5/H7 after H1; H6 after H3; H9 after H4; then the skill; then living docs; then the open items); the FIDELITY contract (§5: the generic tool and the project tool over the SAME input, verdict TEXT diffed; F1-F9; three rules — no verdict-string edit before H9, the generic classifier is the STRONGER copy and a static-tier delta on a shell-crash gate is a finding about this tree, comparison is of text); twelve unclean boundaries (§6); ten decisions (§7: two RULED, eight defaults + veto — `$BBH_HOME` not a submodule, GPL-3.0, citations travel but no `[VSP-N]` anchor, MAME/FBNeo/fake drivers and MiSTer stays, **the eight doc tools are OUT of the harness** (they are the living-docs effort's), this tree never consumes it so its consumer config lives in the harness); cost §8 (six to seven sessions before living docs) |
| **`docs/project/living_docs_scope.md`** (REFERENCE, banner) | the three problems — stale / lost / unreadable — and the three ruled forms mapped one to one; what enforcement and routing already exist (§2, eight mechanisms); the ONE thing the SMS reference has that we do not: `checkdocs.py` there re-derives documented claims FROM THE CARTRIDGE (quote the claim, derive the fact, compare; `--uncovered`), ours locks docs against docs — `annotations.md`'s 2,970 rows are where addresses are CLAIMED, not which claims are CHECKED; measured gaps (two docs with a shape row and no README listing — `gate_scoping_method.md`, `tables/community_crosscheck.md`; README completeness unenforced; twins declared one-way, zero missing back-links in text; two entry points without a routing table); the slices L1 routing → L4 site → L2 rule-5 census → L3 ROM re-derivation, with the order argued; six decisions; cost four to five sessions after the harness. The rebuild-from-docs option is NOT scheduled — L2's census number is what makes it decidable |
| **registration and the tier** | `doc_shape.tsv` rows, README Contents + two routing rows, the two STATE decisions marked STARTED with the rulings; the eight doc checks green; static tier **PASS 135 / SKIP 0 / FAIL 0** (`build/static_14z135_scope.log`, tree clean during the run). Commit `06589755` |
| **H1 BUILT AND LANDED — `~/Developer/blackbox-harness` at `803f372`** (39 files, git-initialised; ~~NO remote: GitHub and any push are the maintainer's~~ PUSHED 14z-135b at the maintainer's word — see that entry) | `lib/sh/classify.sh` THE classifier, one copy (exit status first; 124/137 TIMEOUT; exit 0 + the shell's `<script>.sh: line N: NAME:` FAIL; `^ *SKIP` + exit 0 SKIP; SKIP in prose PASS); `bin/bbh-run-static` (two tiers tallied separately, `--strict`, `--list`, the anti-orphan report, the tree snapshot) with its output byte-identical to this tree's runner over the same gates; `lib/py/bbh/tier.py` the transitive classifier with every wrapper name as config; `demand_after_trap.py`; `toml_subset.py` + `config.py` — a TOML subset REFUSING what tomllib reads differently (dotted names, arrays of tables, duplicates, signed hex, escapes, floats, nested inline tables) with arrays and inline tables added and a `tomllib`-agreement selftest; `prologue.sh` (`bbh_demand` exits 1 AFTER a trap — the whole point); `docs/gate_contract.md`, `docs/config.md` (every key, default, origin bin), `README.md`, `bbh.toml.example`; `example/` a complete tiny consumer GREEN on both tiers; `example/consumers/bbh.vampire.toml` this tree's config, kept THERE by decision 8. One addition to the plan: `[project].root` so a consumer config may live outside its tree. `selftest/run.sh` eats its own classifier: **7 PASS / 0 SKIP / 0 FAIL** (test_classify, test_config, test_demand_after_trap, test_fidelity_vampire, test_prologue, test_run_static, test_tier — each with must-fire controls: a perturbed config must reach the classifier, every refused TOML shape must be refused, the depth-3 source chain must NOT be reached at depth 2 and MUST at depth 3, a synthetic demand after a trap must be reported) |
| **FIDELITY F1 AND F3, MEASURED, AND KEPT AS A SELFTEST** | **F1**: both static runners over one synthetic fake repo (9 stub gates + a MISSING + an orphan + an emulator gate) — IDENTICAL output (durations normalised); with a gate that exits 0 after a shell error, the diff is EXACTLY the known delta — this tree's static runner says PASS, the generic says `FAIL (exit 0 after a shell error)` — 7 diff lines (the row, its tail, the tally, the failed list). **F3**: the generic tier classifier over the 304 gates: INSTRUMENT − the plain registries − `run_` names == the 165-row sweep registry EXACTLY; every PLAIN gate is in a plain registry; every sweep row names a real gate; `--unregistered` prints this tree's ok line verbatim. The first formulation was too strong (it expected the raw INSTRUMENT set to equal the registry, and 14 registered emulator-free gates mention a wrapper name inside a FIXTURE STRING — `test_static_runner`, `test_emulator_runner`, …; and the two soaks are registered in the sweep, so `_soak` is excluded by the static runner's rule and NOT by the sweep runner's — read from `run_all_emulator.sh:461`); the corrected set algebra is the runner's own. `selftest/test_fidelity_vampire.sh` reruns both (SKIPs when this tree is absent; F2 opt-in with `BBH_FIDELITY_F2=1`, never beside another gate run here). **A finding about THIS tree, recorded, not fixed:** `run_all_static.sh` has no exit-0-after-shell-error branch, so a portable/static gate that dies at a `${VAR:?}` after its trap reads PASS here (the `test_demand_after_trap` lint is what prevents the shape; the sweep runner has the branch since 14z-134) |
| **host facts measured for the harness** | macOS bash 3.2; python 3.9 only (no tomllib — the agreement section reports "not run", the parser runs unchecked on this host; `bbh doctor` says so); coreutils `timeout` present; no standalone `lua` (the H6 `.rpl` parser equality selftest will SKIP here); `sh -c ': "${X:?}"'` exits 127 at top level |
| **memory** | two notes: the harness repo, its `$BBH_HOME` convention and "this tree stays as it is"; the living-docs effort's three forms and order |
| **CLOSE** | the 14z-133b group rolled EARLY to STATE_HISTORY (STATE was 174 KB; 152 KB after, two groups kept); NEXT_SESSION rewritten (H2 next); HANDOFF gained the harness rows; `harness_scope.md` §4 carries H1's landing; the eight doc checks green; static tier **PASS 135 / SKIP 0 / FAIL 0** (`build/static_14z135_close.log`), tree clean during the run. Two commits here (`06589755` the scope documents, then this close), one in the harness (`803f372`). NOT pushed — the maintainer's word |
