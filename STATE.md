...
| open (phase 3) | the hold-release counter's home (next: a `-debug` tap on the victim's `+0x21` at the return frame gives the releasing PC); the unindexed lying/wake nodes (entered by game-logic jumps — the per-victim pose tables of [VSE-44] are the candidate source); projectile parameter records; the `gap_*` auto tables |
