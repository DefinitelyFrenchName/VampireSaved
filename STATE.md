...
## Session 14z-139 — **THE FOUR RECORDED-NOT-FIXED FINDINGS ABOUT THIS TREE, FIXED** (the maintainer's
## "let's start with" them): ONE verdict classifier for the three runners, the enumerator's `.diverge` case,
## the ref-rot gate's image pick named instead of inherited from the filesystem. No build byte moved.

| | |
|---|---|
| opened with | the opener read (CLAUDE.md, STATE, HANDOFF, `docs/NEXT_SESSION.md`), the ROM audit 76/76, then the maintainer: *"let's start with — Four recorded-not-fixed findings about this tree"* (NEXT_SESSION item 3). Each site read in full first, beside the harness's already-lifted resolution of it (`lib/sh/classify.sh`, `lib/sh/accounting.sh`, `lib/sh/enumerate_expectations.sh`, `lib/py/bbh/ref_rot.py`) — the harness had fixed all four generically at H1/H3/H5; this tree had them recorded |
| **(1)+(4) ONE CLASSIFIER — `tests/lib/classify.sh`** (the 14z-135 census's finding: the PASS/SKIP/FAIL logic DUPLICATED in the two runners and DIFFERING, the sweep's copy with the TIMEOUT and exit-0-after-shell-error branches, the static runner's with neither; and 14z-137's: the battery's `bat` reading exit 0 by the SKIP grep alone) | the sweep's branches MOVED, not rewritten, into `vs_classify <exit> <log> [width]` → `VS_VERDICT`/`VS_DETAIL`, sourced by `run_all_static.sh`, `run_all_emulator.sh` (which keeps only its TIMEOUT detail, `killed after Ns`) and `run_battery_m2.sh` (whose `bat` now stops on an exit-0 crash with exit 1 and the reason). The static runner writes each gate's output to a file and prints the harness's own `FAIL  Ns  (exit 0 after a shell error)` row, so the two static runners are now IDENTICAL on that shape too. **Ground truth:** `test_static_runner` §8 (the shell-crash stub FAIL, the MAME teardown-segfault stub PASS, the tally, the non-zero exit, and THE LOCK: all three runners `grep`-asserted to source the lib and none to carry the regex inline — a third copy is how it recurs); `test_battery_accounting` §5 (the exit-0 crash stops the battery, named with its reason) and §6 (the control: a clean battery incl. the segfault shape still prints GREEN); `test_emulator_runner` §12 unchanged and green through the lib. The two runner fixtures symlink the lib into their synthetic repos (the runner sources it relative to `$REPO`) |
| **(2) the enumerator's `.diverge` case** (14z-136: `tests/lib/enumerate_expectations.sh` had no case for a ratified kind `run_suite.sh` dispatches before `.sha1` and `audit_merged_legacy` evaluates — zero live `.diverge` files, so the first one ratified would have read UNKNOWN-KIND and failed the merged audit) | `diverge) … |diverge|EVAL` added with the reason in the file; `test_audit_merged_dispatch` §1 gains the `ff_diverge` fixture (6 kinds now) |
| **(3) the ref-rot gate's image pick** (14z-137: `test_build_ref_rot.sh` took the `vsavjw` zip else DIRECTORY ORDER — `glob.glob` unsorted — so a stock build's rompath, `vsavj.zip` beside the pristine parent `vsav.zip`, was judged by whichever the filesystem listed first; on this host `vsavj.zip`, so the stock twins were found in the registry; on another disk the parent, and `no registry row`) | `IMAGE_PREFER = ("vsavjw", "vsavj")` over a `sorted` glob, name order as the fallback, in ONE `images_of()` used by both the rot check and the currency report — the harness's `[ref_rot].image_prefer` default, so F9 stays identical. `test_build_ref_rot` on this tree: 108 live / 0 ROTTED, output unchanged. **NEW GATE `tests/test_ref_rot_image_pick.sh`** (ci_portable, family review-triage, ~2 s): the REAL gate symlinked into a synthetic repo with three rompaths, each with an ADVERSARY zip shaped exactly like the rot signature beside the image that should be judged (so a wrong pick is LOUD — ROTTED — where the real tree's would be quiet); §2 the MUST-FIRE — a copy of the gate with the preference rewritten to prefer the adversary reads both ROTTED and exits 1; §3 the fallback is NAME order (the rotted zip created first, the ok one judged). One fixture trap paid for on the first run: `grep ROTTED` matched the gate's own summary line `0 ROTTED` — the assertion is the ROW shape `^  ROTTED ` |
| **docs** | two gotchas appended (`docs/project/gotchas.md`: the filesystem-order pick, and "a classifier copied into three runners is three classifiers"), index regenerated; `harness_scope.md` §5 rule 2 RESOLVED in place; NEXT_SESSION item 3 struck through per finding; HANDOFF's pre-commit section names the lib; the eight doc checks green (exit statuses captured) |
| **the harness side** (its F1 fidelity asserted the known delta — lineage PASS, generic FAIL — on the shell-crash gate, which this session erased) | `selftest/test_fidelity_vampire.sh` F1 now asserts IDENTITY on the shell-crash gate too and that both read it `FAIL (exit 0 after a shell error)`, its fixture symlinking the lineage's `tests/lib/classify.sh`; the three lineage-history notes dated (`docs/config.md` `image_prefer`, `ref_rot.py`'s docstring, `accounting.sh`'s header) |
| **(2) THE EIGHT DEFAULTS OF `harness_scope.md` §7, RULED** (the maintainer: *"Before the harness skill, can we try and rule on … but I'll need more context and details"* — each default measured in the harness as it stands, its veto costed, a recommendation; then eight answers) | **3** keep, *"just document it … (documenting default applies to all other defaults of the bbh)"* → the harness gains `docs/conventions.md`, the register of every ruled default, linked from its README; **4** keep GPL-3.0 and it is NOT a default (*"a license loadbearing on both legal and type of use scopes, not a tech scope"*), so no register row; **5** keep the citations (33 session + 23 issue travelled, zero anchors), the three dangling `[CPE-N]` to be worded at the skill; **6** keep the MiSTer driver here until a second jtframe consumer (the lane: 584 lines, 44 couplings, dumps not the log grammar); **7** keep the doc tools out (*"if and only if we extend the bbh with a top-layer that also handles the documentation"*); **8** keep, AMENDED at my recommendation: `[project].root` in the consumer config is ONE host's layout and `test_bbh_fidelity.sh` hard-FAILED on any other — now the gate passes `BBH_FIDELITY_ROOT` and the fidelity test derives a private config copy with that absolute root (a resolver-level env override was written first and withdrawn within the hour: it would have leaked into F1's fake-repo config and the example's); **9** ratified RESOLVED on the measured claim — F2, the whole live portable tier through both static runners, run for the FIRST time: IDENTICAL, 65 PASS rows; **10** EXPIRED at H9 and REPLACED by the co-commit rule, *"such a change needs to be loud though"* → `docs/rebaselines.md` in the harness, one dated line per re-baseline (this session's F1 the first), its newest line printed by every fidelity run as `LAST RE-BASELINE:`. Every ruling marked DECIDED in place in §7 with the maintainer's words |
| **(3) THE HARNESS SKILL, SCOPED** (the maintainer's *"start on the items in the planned order"*, then plan mode) | `harness_scope.md` §9, the plan before the work in the form every slice had: the skill `blackbox-harness` / `[BBH-N]` at the harness's `skill/` placeholder, level 0 (no game, build, board or lineage token in `SKILL.md`; the GUIDE generated); eight sections mapped to the nine harness pages (76 KB read in full) with a yield of ~75-80 rules and the ONE-ANCHOR rule for the four rules several pages carry; the staleness pass S1-S7 (the README promises a `docs/doctrine.md` that does not exist — written in the pass as the doctrine rules' anchor home; the README Layout block stale); the checker's rules for the skill (the docs as their own LOG; no cross-references, enforced by `forbid` bracket tokens because the checker IGNORES a foreign prefix); sequencing pass → H10 → distillation → this tree's close. **A second reader (a forked agent, both trees) corrected the draft eight ways before it landed** — the TOML subset refuses dotted headers and inline tables with arrays, so the config is `[skills] prefixes` + one `[skill_<PFX>]` table per skill; three lineage names hard-coded in the two tools must become config for the guides to render byte-identical (the archive exemptions, the guide header's origin/tool/session strings); "no cross-references" is not enforced by the checker as written; §7.5's "three bare `[CPE-N]`" is ONE ID in four places (corrected in §7.5); S7 the README Layout block. **RULED at the plan stage (maintainer): the lock TRAVELS, config-driven, with fidelity F11; and the session STOPS after §9 lands for the rulings on decisions 2-7.** Docs only; the eight doc checks green |
| **CLOSE** | harness selftest with `ROMDIR`: **PASS 29 / SKIP 2 / FAIL 0** (`test_rpl_lua` no standalone lua, `test_fidelity_mame` opt-in; `test_fidelity_vampire` 106 s — F1 identical on the shell-crash gate, F9 ref-rot identical), harness commit `3eff2d4` PUSHED (standing-authorised); this tree's static tier run ALONE afterwards: **PASS 137 / SKIP 0 / FAIL 0** (`build/static_14z139_close.log`; 136 + `test_ref_rot_image_pick`), tree clean during the run; committed to main, NOT pushed (the maintainer's word, as always). **CLOSE (2), after the eight rulings:** harness selftest again **29 / 2 / 0**, harness commit `45b5724` PUSHED (`docs/conventions.md`, `docs/rebaselines.md`, `BBH_FIDELITY_ROOT`); this tree's fidelity gate PASS through the new input and the must-fire on a non-tree root exit 1; the static tier ALONE again **PASS 137 / SKIP 0 / FAIL 0** (`build/static_14z139_close2.log`), tree clean during the run; committed, NOT pushed. NEXT_SESSION's order unchanged: the harness SKILL, then living docs, then the remaining open items |

## Session 14z-138 — **SLICES H6 AND H9 LANDED: the MAME Lua layer under a MACHINE PROFILE, the real
## drivers and the recording corpus — with FIDELITY F8 EXACT on the real emulators — and the ONE gate this
## tree gains, green on its first full run** (the maintainer's "do h6 then h9"). No build byte moved.

| | |
|---|---|
| opened with | the opener read (CLAUDE.md, STATE, HANDOFF, `docs/NEXT_SESSION.md`), the ROM audit 76/76, then the maintainer: *"do h6 then h9"* — `harness_scope.md` §2.4 (I1, I3-I9), §3.3, §4's H6 row, §6 items 4-5, the harness's contract docs, then every H6 source in this tree in full (the seven Lua files, the four runners, the two recording tools, the crash-guard and corpus gates) |
| **H6, lifted** (harness commit `512747b`, PUSHED) | `lua/mame/profile.lua` — the loader: `BBH_PROFILE` names the board, a REQUIRED key missing is refused BY NAME, the guard scripts refuse a `crash.*` key they read; `rpl_parse.lua` — the `.rpl` grammar as ONE strict module for every script (the lineage carried one strict copy and FIVE lenient ones in the tap instruments, which silently DROPPED an unknown side or token — a malformed replay now fails loudly on every instrument, the lineage's own error texts); `profiles/cps2.lua` — every literal `replay.lua` / `replay_guard.lua` / `inp_guard.lua` carried about CPS-2 (`:maincpu`, `program`, `:screen`, `$FF0000-$FFFFFF`, the three port tags and 22 field names, `sys` width 2, active-low, vectors `2,3,4,5,6,7,24`, the group-0 frame layout, the code window, the register names, `$FF0000` as the exception store with `vec = code + 2`, `$FF8004 == 0x40000`, the ALIVE fields); **`profiles/cps2w.lua`** — the WIDE variant, differing ONLY in the code window (`0x600000`): **the lineage's `replay_guard.lua` still carried the stock `0x400000` while `inp_guard.lua` carried `0x600000`, so the two guards drew different STACK sketches of the same crash** — one machine, one profile now; `profiles/TEMPLATE.lua` every key with a comment. The seven scripts lifted onto the profile with their logic intact (`replay.lua`, `replay_guard.lua` with its whole probe family, `inp_guard.lua`, `snapshot_frames.lua`, `trace_writes.lua`, `tap_writes.lua` — COLLECT's CPS-2 constants became `COLLECT_STRIDE/OFFSET/PORTED` env, `read_tap.lua`), every one staging CANONICALLY (`held[frame + 1]`, census'd); `rpl_dump.lua` the canonical parse, standalone or under MAME. `drivers/mame.sh`, `mame_guarded.sh`, `fbneo.sh` under THE CONTRACT over `lib/sh/mame_sandbox.sh` (the lineage's `run_mame.sh`: headless, every host input provider off, a fresh sandbox per run): the plain driver refuses the guard family, the guard refuses `MASK_RANGES` / `NO_INPUT_CHECK` / `VIDEO_OUT` / `INPUT_OUT` BEFORE the emulator starts (exit 3), FBNeo refuses masks, snapshots, the integrity family and any `TAIL_FRAMES` but the frontend's 120, maps `DUMPS` → `-hdump`, `POKES` → `FBNEO_HPOKE`, `VIDEO_OUT` → `FBNEO_HVIDEO`, and builds its overlay from a FIRST-WINS chain (`FBNEO_ROMPATH="build;ref"` and the lineage's single overlay dir are the same thing). `bbh inp-play` (the lineage's `run_inp_guarded.sh` with the recording dir explicit — a zero-frame playback stays UN-terminated) and `bbh inp-corpus` (`test_inp_corpus.sh`, verdict text verbatim, the liveness controls on every run); `[machine]` and `[inp]` config; `bbh run-suite` exports `BBH_PROFILE` from config; `doctor` knows lua / `MAME_BIN` / `FBNEO_BIN`; `docs/lua.md` |
| **five selftests** | `test_profiles` (a static key census — every `P.x`/`C.x` a script reads is in the TEMPLATE, the loader's REQUIRED list agrees with the test's — and the loader under a standalone lua with the missing-`ports` must-fire), `test_rpl_lua` (`rpl_parse.lua == rpl.py` over the example, ten malformed shapes and the lineage's 181 replays; **SKIP with the reason on this host, which has no standalone lua** — the same equality then runs under MAME's OWN interpreter in F8), `test_mame_drivers` (the sh half of the contract for the three real drivers and `inp-play` against a STUB emulator that records its argv and environment: every refusal, the profile resolved to an absolute path, every isolation flag, the sandbox dirs, the stale-artifact rule, the canned INPUT-VIOLATION → exit 1, the canned CRASH → exit 2, the FBNeo overlay's first-wins order, the mappings, the playback terminator and the zero-frame must-fire), `test_inp_corpus` (every verdict line on a fabricated corpus and a stub: clean, a crash with its NOTE, OPEN (as frozen), a rotted DEFECT, a DEFECT with no crash, a dead run, no `.inp`, `--only` and `ONLY`, an empty selection, a missing build, the demanded input variable), `test_fidelity_mame` (F8) |
| **FIDELITY F8, MEASURED: EXACT** (opt-in `BBH_MAME_FIDELITY=1`, ~60 s; `BBH_FIDELITY_F8=all` every recording, 188 s) | F8a `107_four_directions` through `run_replay_mame.sh` and `drivers/mame.sh`: logs `cmp` IDENTICAL (521 lines), unmasked and under the default mask (and the mask moved the hashes); F8b `VIDEO_OUT` / `INPUT_OUT` / `DUMPS` / `POKES` / `SNAP_FRAMES`: every artifact byte-identical, the two PNGs too, and the pokes moved the RAM log on both sides alike; F8c `INPUT_INJECT_TEST=200`: both exit 1 with the same `INPUT-VIOLATION 1 frame 200 port :IN0 expected 7f77 got 7f67` line; F8d the cheap guard on the crash guard's negative control (`02_demitri_vs_cpu`, `CODE_RANGES` + `GUARD_MATCH`): logs identical AND equal to the lineage's FROZEN `vsavj` `.sha1` — **the harness driver reproduces a frozen expectation of this tree**; F8e the authoritative guard on `test_crash_guard`'s positive control (the planted ILLEGAL, built with `patch_prg.py` + `pack_build.sh`): both exit 2, `CRASH 2886 vec4 PC 0bf800 SP 00ff7ff0 ADDR -` / REGS / STACK / END-CRASH identical, the whole `-debug` log too; F8f the grammar under MAME's Lua: `rpl_dump.lua` == `bbh rpl dump` over all 181 replays (205,546 lines); F8g `crash-merged-m8-01` (and all seven at `=all`) through `run_inp_guarded.sh` and `bbh inp-play`: `inp_guard.log` identical (8 ALIVE lines, END 5181), then `test_inp_corpus.sh` vs `bbh inp-corpus`: verdict lines and exit identical; F8h FBNeo: `run_replay_fbneo.sh` vs `drivers/fbneo.sh`: the log, the `-hdump` file and the video log identical. **Two defects mine, found by F8 on its first run**: `rpl_dump.lua` `dofile`'d a bare `BBH_PROFILE` name literally (the drivers resolve it; now the dumper does too), and `rpl.py`'s missing-side text read `'None'` where the lineage's Lua prints `'nil'` (the python side follows the lineage now; `test_rpl` updated). And one trap paid for in the sh: `VAR=x funcname` before a shell FUNCTION does not reach the external command the function runs the way it does for a command — the drivers `export` `REPLAY`/`CHECKSUM_OUT` explicitly |
| **H6 pre-commit and commit** | `BBH_MAME_FIDELITY=1 BBH_FIDELITY_F8=all BBH_FIDELITY_F5=1 BBH_FIDELITY_F6=all BBH_FIDELITY_F7=all bbh selftest` with `ROMDIR`: **PASS 30 / SKIP 1 / FAIL 0** (the SKIP is `test_rpl_lua`, no standalone lua; F8 188 s; the ROM-free fidelity 757 s; `scratchpad`/`/tmp/bbh_h6_precommit.log`). Harness commit `512747b`, PUSHED (standing-authorised) |
| **THEN H9 — THE ONE FILE THIS TREE GAINS** | `tests/test_bbh_fidelity.sh` (`ci_static`, never `ci_portable`): finds the harness by `$BBH_HOME`, else looks beside this tree AND beside its parent, SKIPs when absent; asserts the harness's consumer config resolves to THIS tree (else the fidelity would be measured against another clone); runs `selftest/test_fidelity_vampire.sh` (F1, F3-F7, F9, F10 — F6 real over `$ROMDIR` since `ci_static` has it) and, with `BBH_MAME_FIDELITY=1`, `test_fidelity_mame.sh` (F8) — the latter never beside another gate run here, because F8 runs gates of this tree. Registered in `ci_static.txt` and `gate_index.tsv` (family `runner`), the index regenerated, `header-defaults` green, the tier classifier reads it as PLAIN (no instrument token in its body — by construction, so it never migrates to the sweep registry). **THE FIRST FULL FIDELITY RUN RECORDED HERE: PASS** (`build/bbh_fidelity_first_14z138.log`, 20 ok lines, 1:50 wall beside the running pre-commit). **A finding at that first run, fixed in place:** the scope's "`$BBH_HOME` default `../blackbox-harness`, a sibling of this tree" was ONE LEVEL SHORT — the ruled location `~/Developer/blackbox-harness` sits beside `~/Developer/Vampire_Saved/`, the PARENT of this tree, so the first run SKIPped with exit 0 while claiming nothing (exactly the shape `ci_static` exists to make visible); the gate now searches both levels and says which it found |
| **CLOSE** | `harness_scope.md` §4 carries H6's and H9's landings and §5 the F8 measurement; NEXT_SESSION rewritten (the harness SKILL next, then living docs L1 → L4 → L2 → L3, then the open items) with the 14z-137 opener archived; HANDOFF's harness row; the memory note; the 14z-135b group rolled to STATE_HISTORY (the newest three kept); the eight doc checks green (one HANDOFF wording fixed on the way — a backticked `docs/<x>.md` path reads as a link into THIS tree to `checkdocshape`, and the harness's page is not here); static tier **PASS 136 / SKIP 0 / FAIL 0** (`build/static_14z138_close.log`; 135 + the new gate, `test_bbh_fidelity` PASS 106 s), tree clean during the run; committed to main, NOT pushed (the maintainer's word, as always) |

| **CLOSE (2) — the two things that surfaced, at the maintainer's word (*"we're at 55% do we fix the two things that surfaced?"* — yes, both)** | (1) the gate's location default was already fixed in place (both levels searched). (2) `tests/lua/replay_guard.lua`'s code window: both constants `0x400000` → `0x600000` (the WIDE window, as `inp_guard.lua`), with the reason in the file; no frozen expectation carries a STACK line and no gate asserts one, so nothing could move. **Validated:** `test_crash_guard` PASS (both positive controls trip: `CRASH 2886 vec4 PC 0bf800`, `CRASH 2886 vec3 PC 022354 ADDR 000bf801`; 33 s, `build/crash_guard_14z138.log`); the harness's F8 guard comparison moved to its `cps2w` profile (harness `882d422`, PUSHED) and stays EXACT (59 s). Gotcha appended (`docs/project/gotchas.md`, the guards' window — an instrument's "plausible address" bound belongs on the profile-bump checklist), index regenerated; NEXT_SESSION's finding marked FIXED. The eight doc checks green; static tier **PASS 136 / SKIP 0 / FAIL 0** again (`build/static_14z138_close2.log`), tree clean during the run. The mid-run ritual audit (maintainer-asked, read-only while the tier ran) found two owed items, done here: the 14z-136 group rolled EARLY (STATE was 148.8 KB against the ~150 KB rule) and harness_scope's status header no longer calls the harness a "sibling of this tree" (the header edit itself landed in CLOSE (3), `63611983` — the (2) commit's script had asserted on a line break after writing the rollover). **PUSHED at the maintainer's word (*"push"*): `origin/main` = `63611983`, then this row** |

| **CLOSE (5) — THE DEFAULTS CENSUS (maintainer's question: *"does this apply to the example or do we have default values that are not explicitly described as such (e.g. a max mame replay size…)?"* — measured: yes, on both counts; *"do it"*)** | Harness slice **H6b**, commit `7d2456d`, PUSHED. The census over `lua/mame/` (grep, not memory) found: three BOARD facts still hiding as literals — the exception store read as 16 bits, COLLECT's stride 8 / offset 4 / 16-bit code (the CPS-2 sprite entry), the printed widths (`%04x` ports, `%06x` PCs) — now `crash.store_width`, `collect = {stride, offset, width}`, `port_hex_digits` in the profile (PC widths derive from `pc_mask`; TEMPLATE carries them, `test_profiles`' key census demands them); five POLICY constants that were neither profile, config nor env — the STACK sketch's 64 walked / 16 shown, PCWEEDS's cap of 10, GUARD_BREAK's boot pass of 100 frames, the ALIVE heartbeat every 600 — now `GUARD_STACK_DEPTH` / `GUARD_STACK_SHOWN` / `GUARD_WEEDS_MAX` / `GUARD_BREAK_AFTER` / `ALIVE_EVERY` with their defaults in the headers; `read_tap.lua`'s `FRAMES` default of **5450 — one lineage replay's length** — now 3600 like the other taps; and the one that applied to the EXAMPLE: `[machine].profile` defaulted to `cps2`, so the example's `run-suite` exported a CPS-2 profile its fake driver ignores — **no default now, a board is never implied** (nothing exported without one, a MAME driver refuses; `[inp].profile` falls back to it and FAILS naming the three; the example says why it omits the section). No maximum replay length exists on the MAME side; the FBNeo frontend's 10,000,000-frame cap is in this tree's C patch — a cap would be a `[suite]` policy key. The register is `docs/lua.md` §5 in the harness (every literal, its bin: board / policy / config). Selftest with F8: **PASS 30 / SKIP 1 / FAIL 0**, the cps2 log formats unchanged. Committed here (docs only), NOT pushed |

## Session 14z-137 — **SLICES H5 AND H7 LANDED: the hygiene bin (provenance, header defaults, reference rot,
## the gate index, shadow tools, the accounting rule) and the field comparator + dump checker — with FIDELITY
## F9 AND F10 EXACT** (the maintainer's "H5 then H7, in that order"). No build byte moved.

| | |
|---|---|
| opened with | the opener read (CLAUDE.md, STATE, HANDOFF, `docs/NEXT_SESSION.md`), the ROM audit 76/76, then the maintainer: *"H5 then H7, in that order"* — `harness_scope.md` §2.5, §2.2 C6-C7, §4's rows read first, then every H5 source in this tree in full |
| **H5, lifted** | ONE header parser, `lib/py/bbh/gate_header.py` (the `#` block after the shebang; the first paragraph as the claim; the session token; the runtime), where the lineage had two private readers. `gen_gate_index.py` → `bbh gate-index` (`--check`, `--stdout`; `[gate_header]` families, kinds, the needs keywords, the preamble as config LINES; the tier labels DERIVED from the registries' file stems and the instrument word, so the lineage's `ci_portable / ci_static / emulator` and the example's `driver` both render from the same code). `audit_header_defaults.py` → `bbh header-defaults` (the rule and its three exemptions verbatim; `--fix` refuses to guess between two code defaults; `[header_defaults]` token and prefix regexes). `test_build_ref_rot.sh`'s python → `bbh ref-rot` (rot vs currency, every line verbatim; `[ref_rot]` a stale MARKER `[member prefix, required member, reason]` or a `predicate` command, an ORDERED `image_prefer`, the family regex, the no-row note and the advice as config). `test_expectation_provenance.sh`'s python → `bbh provenance` (`[provenance]` scope dirs with row prefixes, exclusions, the closed vocabulary, the `rests on` column). `shadow_tools.sh` → `lib/sh/shadow_tools.sh` (`bbh_shadow_tool` / `bbh_shadow_restore`; `BBH_TOOLS_DIR`, `BBH_SHADOW_LINK_DIRS`, `REPO` or `BBH_ROOT`; no root is an error). `run_battery_m2.sh`'s `bat` → `lib/sh/accounting.sh` (`bbh_bat` / `bbh_bat_group_skip` / `bbh_bat_report`; the verdict from the ONE classifier, so an exit-0 shell error and a timeout STOP the battery where the lineage's grep read them as PASS). `config.consumer()` — `(cfg, root)` from `--config` / `$BBH_CONFIG` / nothing — so every tool runs with no config at the lineage's defaults. `docs/hygiene.md`; `docs/config.md` four sections; the dispatcher; `bbh.toml.example` |
| **the example consumer** | `tests/gate_index.tsv` (four families), `docs/gate_index.md` GENERATED, `expected/PROVENANCE.md` (its own four-class vocabulary), `tests/g_hygiene.sh` — the four checks as a CONSUMER runs them, one portable gate |
| **six selftests, each with must-fire controls** | `test_gate_index` (the lineage's four controls, the rendered rows, a family absent from the config is a PROBLEM, `--stdout`), `test_header_defaults` (the lineage's A-F + a `<template>` token + two defaults both named, `--fix`'s refusal, the token regex must-fire), `test_ref_rot` (every default idiom and outcome on a synthetic root, the ordered image preference, the currency report with SUPERSEDED / no row / a family at two generations, the stale image made current flips the verdict, a predicate command replaces the marker), `test_provenance` (the lineage's A/B, an undefined class, a second scope dir, a missing page, `--page`), `test_shadow_tools` (the copy runs, the perturbation reaches the copy and NOTHING else — the tracked tool byte-identical to a pristine copy — restore, config, no root is an error), `test_accounting` (the lineage's §3-5 + the two verdicts its wrapper could not see) |
| **FIDELITY F9, MEASURED: EXACT** | each of the five hygiene tools with `bbh.vampire.toml` against the lineage's over THIS tree, stdout + exit diffed: header-defaults (2 lines), `gate-index --check` (2 lines) AND the rendered index byte-identical to the committed `docs/project/gate_index.md`, provenance (the gate's sections 1-2, 5 lines), ref-rot (the whole 268-line report incl. the 43-row currency report), demand-after-trap — all identical. Three test defects were mine (a miscount, `'BATTERY GREEN'` inside the INCOMPLETE advice matching a grep, a predicate config without the image preference); ONE was the tool's: the fingerprint EXITS (`sys.exit`) on an image with no program members, and the currency report's guard caught `Exception` only — a `SystemExit` escaped it and aborted the verdict. Fixed; the selftest asserts it |
| **A FINDING ABOUT THIS TREE from the fidelity work (recorded, not fixed):** | `test_build_ref_rot.sh` picks the image it judges by DIRECTORY ORDER when no `vsavjw` zip is present (`glob.glob` unsorted); on this host `vsavj.zip` happens to list before `vsav.zip`, so the stock builds (`build/donovan6`, `build/m5_stock15`) are judged by their own 9-member image and found in the registry — on another filesystem the same dirs would be judged by the parent's 21 members and read `no registry row`. The harness names the preference (`image_prefer = ["vsavjw", "vsavj"]`, the default). Latent; NEXT_SESSION carries it. AND the battery wrapper reads exit 0 by grep only (the accounting finding above) — the demand-after-trap lint already bars the cause here |
| **H5 pre-commit and commit** | `BBH_FIDELITY_F5=1 BBH_FIDELITY_F6=all BBH_FIDELITY_F7=all bbh selftest` with `ROMDIR`: **PASS 24 / SKIP 0 / FAIL 0**, the fidelity gate 767 s (`scratchpad/bbh_h5_precommit.log`). Harness commit `b533715`, PUSHED (standing-authorised). Drafted H7 in the scratchpad while it ran: nothing in the harness is edited during its own gate run, and the fidelity gate also runs lineage gates in this tree |
| **H7, lifted** | `compare_fields.py` → `bbh compare-fields`: every printed line the lineage's; `[fields]` carries what the ONE comparator with a game fact in SOURCE had there — the anchor CLAUSES (`[address, width, value]`, all true; the debounced rising edge is the anchor), the `bases` inline table (a TSV's own `# base p1=…` header lines override it, the scope's I2 note), `stable` (the debounce, 30), `settle` (120), `anchor_hint`, `dump_regex`; the error vocabulary is `abs|` + the configured base names, so the lineage's `abs|p1|p2` renders unchanged. `check_wram_dumps.py` → `bbh check-dumps`, verbatim, `dump_regex` and `integrity_hint` from config. The example gains `tests/fields.tsv` (mode / timer / credits / the two HPs / the P1 port as `settled` / the hook's late byte as `phase`, with `# base` headers) and `tests/g_fields.sh` — a `fake`-lane sweep row: base vs `build-a` agree at the match-start anchor and +30/+60, and `--exact` MUST fail on the phase field (the gate's own must-fire) |
| **two selftests** | `test_compare_fields` on the FAKE machine — the lineage's positive and negative controls without an emulator: two runs whose anchors sit on DIFFERENT frames (401 vs 408, a 7-frame skew authored into the replay) agree on every stable field at the anchor and after; different content FAILS with exit 3 and the MISMATCH line to the character; the §4 shape (a hooked build agrees at anchors, differs frame-exact — 141 common frames); settled fields joining at `--settle`; the debounce NOTE and the window-starts-true WARNING on fabricated dumps; a frame missing at a follow offset; the bases must-fire (a wrong base reads an address nobody dumped: MISSING; the TSV headers override it); the anchor hint and the TSV's error texts. `test_check_dumps` — every verdict on fabricated directories, `--quiet`, the driver-prefixed `out.dump_…` name, a custom regex and hint. Two fixture traps paid for, in the memory note: the fake writes its mode byte BEFORE its transition (the first frame that READS `match` is 401, not 400), and a dump file's address is 6 hex digits |
| **FIDELITY F10, MEASURED: EXACT** | (added to the contract this session, `harness_scope.md` §5) synthetic dump directories shaped like this tree's — its addresses, `tests/fields_m2a.tsv`, a driver-prefixed copy, a hole, a window that starts true, a transient edge — through both comparators in every mode (10 invocations) and both dump checkers for every verdict (11): stdout + stderr + exit identical |
| **H7 pre-commit and commit** | the same pre-commit: **PASS 26 / SKIP 0 / FAIL 0**, the fidelity gate 761 s (`scratchpad/bbh_h7_precommit.log`). Harness commit `8867dbb`, PUSHED |
| **CLOSE** | `harness_scope.md` §4 carries H5's and H7's landings and §5 the F10 row; NEXT_SESSION rewritten (H6 next — a fresh window; then H9, the skill, living docs) with the 14z-136 opener archived; HANDOFF's harness row; the memory note; the 14z-135 group rolled to STATE_HISTORY (the newest three kept); the eight doc checks green; static tier **PASS 135 / SKIP 0 / FAIL 0** (`build/static_14z137_close.log`), tree clean during the run; committed to main (`792a29e2`) and PUSHED at the maintainer's word (*"push"*) |
