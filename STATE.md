...
| push | **NOT PUSHED** — main is ahead of origin by this freeze commit; the fork `80e08111` is local; tags `freeze/{donovan-m16,huitzil-m23,pyron-m17,merged-m12}` local. Push at the maintainer's word |
...
| **the fix that landed** | Pyron's medallion: `PRG:0x05F9D0`'s P2 branch no longer writes row `0x1A`. **`build/m3b_merged19` (`af21bc88`) carries it and is UNREGISTERED / UNFROZEN** — the manifest is on origin, no shipping artifact has it |
