...
## Session 14z-140 — **THE LIVING DOCS EFFORT OPENS: L1 ROUTING ENFORCEMENT LANDED** — the map now
## REACHES every declared document and a history twin must be two-way, with routing tables at the two
## entry points that had none. The census disagreed with the scope document's own §2.2. No build byte moved.

| | |
|---|---|
| opened with | the opener read (CLAUDE.md, STATE, HANDOFF, `docs/NEXT_SESSION.md`), then `docs/project/living_docs_scope.md` and the execution plan it points at (`build/living_docs_plan_14z139.md`, untracked, written by Fable at the 14z-139 close for this session to execute); the ROM audit **76/76**; the skill `vampire-saved-port` loaded; the memory notes the plan names. The maintainer: *"begin 14z-140 with beat 1 of L1"* |
| **(1) BEAT 1 — MEASURE, and it corrected the scope document** | A throwaway census over `docs/doc_shape.tsv` and `docs/README.md` rather than reading §2.2. **69 shape rows** (33 REFERENCE / 19 HIST / 7 GENERATED / 3 REGISTER / 3 LOG / 3 INDEX / 1 ORIENT); **60 listed** in the `## Contents` block (README lines 107-176); **9 not** — five of them the level-0 entry points the Contents preamble excludes on purpose, and **FOUR real gaps where §2.2 said TWO**: `gate_scoping_method.md` (REFERENCE, named NOWHERE in the README — and it carries `[VSP-167]`..`[VSP-174]` and sits in `checkskills.py`'s `_PORT_DOCS`), `tables/community_crosscheck.md`, `HANDOFF_HISTORY.md` and `harness_hardening_history.md`. The two HIST rows were missed because the first census read the buckets and not the HIST group. Also measured: **43 Contents entries carry a shape tag and ZERO disagree**, the other 17 are the HIST group untagged by design; **all 9 declared twins are two-way today**, every twin naming its live document in its own `# ` line; `tables/chars/` is 3 `.md` + 3 `.json` with no shape rows, excluded by a hard-coded prefix in `walk_docs()`, and DIFFERENT documents from the declared `tables/{donovan,huitzil,pyron}.md`; neither `HANDOFF.md` nor `docs/game/atlas/README.md` carries a routing table. §0.6's figures re-measured: 60 hand-written docs, `annotations.md` 2,958 distinct / 473 atlas, `doc_locks.tsv` 19, `tables/` 10, highest anchor `[VSP-178]` |
| **the measurement that changed a design** | The matcher is not free to be loose. A matcher accepting any backticked basename anywhere turned **three rows green that Contents does not list** — `docs/README.md` resolved from the atlas line's `README.md` token, `GOTCHAS.md` and `NEXT_SESSION.md` from routing-table prose. Only a `](path)` link, or a DIRECTORY link whose own line names the member in backticks, counts. Gotcha appended |
| **(1) BEAT 2 — the plan section** | `living_docs_scope.md` §8, in `harness_scope.md` §9's form: STATUS line, the ground truth above, what the two checks assert, the matcher and THREE mechanisms for the level-0 five with a recommendation, the two routing tables' rows, six must-fire controls (j-o, two of them because a directory entry and the entry-point exemption are the two MECHANISMS that could wave a document through in silence), the staleness table S1-S6, six numbered decisions each with its veto and cost, and the sequencing. Commit `3d923498`; the top banner amended in place |
| **(2) THE STOP — all six decisions settled the same sitting** | **RULED (maintainer, 2026-09-07):** (1) the level-0 exemption is DECLARED — an `entry-point` token in `doc_shape.tsv`'s `requires` column exempts a row from Contents but still demands it be named somewhere in the README; (2) `tables/chars/*.md` get declared, one GENERATED row each plus a Contents directory entry naming the three; (5) the three uncheckable counts are DROPPED from `docs/README.md`, not re-derived. Decisions 3 (a directory entry's tag must equal EVERY member's class), 4 (two-way twins HARD) and 6 (the tables' wording is the maintainer's to amend) were stated as defaults at the STOP and not vetoed — recorded as that, not as rulings. Each marked in place with its veto struck |
| **(2) THE STALENESS PASS, before the checks** | S1 §2.2's "two are not" -> FOUR, with the mechanism named and §4's L1 row — the live carrier the [VSP-13] grep found — corrected with it; S2 "the Contents list is a CONVENTION" put in the past tense; S3 the port skill's "161 rules" (**178**) DROPPED; S4 "304 entries ... 46/79/179" (**57/111/221 = 389**) DROPPED, the sentence now saying WHY it is not quoted — HANDOFF dropped exactly this figure for going stale by 25 within a few sessions; S5 "~195 places cite `docs/GOTCHAS.md`" (**141 across 81 tracked files**) DROPPED; S6 the document's own dated ground-truth paragraph STAYS (it is dated and says to read the tree). [VSP-13] re-grep shown: the claim survives only in `STATE_HISTORY.md` (archived, never rewritten) and in the staleness row recording the correction. Commit `46d920f6` |
| **(3) THE TWO CHECKS** (commit `45f116ab`) | `check_readme_completeness` and `check_twins_two_way` in `tools/checkdocshape.py` — **no new gate file**, both land in a tool `tests/test_docshape.sh` already runs, so there is no `ci_portable.txt`, `gate_index.tsv` or HANDOFF row to add; what grows is the control list. `walk_docs()` loses its hard-coded `tables/chars/` exclusion (an exclusion the completeness check cannot see is how an unclassified document arrives). **On its first run the check found exactly the four documents the census predicted and nothing else**, plus the three newly-declared `chars/` pages; all seven listed, `chars/` as a directory entry naming the three so a fourth tenant's page fails until someone lists it. **ONE DEFECT OF MINE, caught by control j on its first run:** `_readme_rel` used `lstrip("./")`, which strips a character SET — `../HANDOFF_HISTORY.md` became `docs/HANDOFF_HISTORY.md` and the `../` branch never fired, so a document one line after being listed read MISSING. Fixed as a prefix strip, with a selftest assertion on it and a gotcha |
| **ground truth, both directions** | `checkdocshape.py`'s selftests gain: a clean synthetic map passes; a document dropped from Contents, a flipped tag, a MISSING tag, a directory entry that stops naming a member, an entry-point row named nowhere, and each twin direction — every one caught; and the must-NOT-fire side, an entry-point row NOT demanded in Contents while it is on the map. `tests/test_docshape.sh` gains **controls j-o on a perturbed copy**, each with a grep proving the perturbation applied: **15 controls, all fire**. EXEMPT rows are skipped by the completeness check (the class means "out of the pass's scope") |
| **(4)+(5) THE ROUTING TABLES**, one commit per document | `HANDOFF.md` (`a32b9138`): **IF YOU WANT TO DO X, READ/RUN Y**, 16 rows under the opening paragraph, each naming the SECTION or the COMMAND — 1,446 lines whose way in was to guess a heading. `docs/game/atlas/README.md` (`38757605`): **IF YOU WANT TO KNOW WHAT ADDRESS X IS, READ Y**, 8 rows; the last two are the ones worth having — any other `PRG:` address to `docs/annotations.md`, and the MECHANISM behind an address to `engine_internals.md`, two documents previously reachable only by knowing they exist |
| **docs** | two gotchas appended to `docs/project/gotchas.md` (the `lstrip` character-set trap; a completeness check is only as good as its matcher), index regenerated; `living_docs_scope.md` §2.1's enforcement row now names ROUTING, §4's L1 row **LANDED** with its three commits, §8's STATUS and the top banner brought to it; the eight doc checks green (exit statuses captured directly) after every edit |
| **CLOSE** | the ROM audit **76/76** at the opener; the portable tier mid-slice **PASS 65 / SKIP 72 (static tier, ROMDIR unset) / FAIL 0** with *"no tracked file changed during the run"*; the static tier run **ALONE** at the close, `--strict`: **PASS 137 / SKIP 0 / FAIL 0 GREEN** (`build/static_14z140_close.log`; `test_bbh_fidelity` 109 s), tree clean during the run and nothing edited while it ran. The eight doc checks green after every edit, exit statuses captured directly. **The 14z-138 group ROLLED EARLY** — STATE reached 155.9 KB against the ~150 KB rule — verbatim to `STATE_HISTORY.md` with its LEDGER line; STATE now 144 KB, the newest two groups. `docs/NEXT_SESSION.md` rewritten (L1 landed, **L4 the rendered site next**, scoped first as every slice is) with the 14z-139 opener archived verbatim; HANDOFF's docs-split section states the three things a new document needs in ONE commit, now that the third is enforced. Memory: the living-docs note carries L1 and the lesson for L4; **the STATE-rollover note CORRECTED** — its `**SPLIT 2026-08-20` end marker is now the file header at line 3, above every group, so the old recipe computes a negative slice; my script asserted and stopped rather than archiving the ledger, which is the durable half of that note. Six commits to main, **NOT pushed** (the maintainer's word, as always) |
