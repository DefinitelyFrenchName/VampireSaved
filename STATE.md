...
| **(4) THE SKILL — §9.6 EXECUTED under the seven rulings** (the maintainer answered decisions 2-7 the same day: keep the name/prefix/location, level 0 plus the lineage tokens, no cross-references enforced by `forbid`, the docs as their own LOG *"improved"* by the twin convention — *"use the anchored key system to keep the docs lean … and have `_HISTORY` files with the complete LOG separately"* — `doctrine.md` in the pass, the user-level install *"improved with the symbolic link"*) | **The pass** (harness `e5bc3a5`): `docs/doctrine.md` written (the extraction question, the seven sentences each with incident + mechanism, the documentation convention, how an extraction is proved, what the doctrine is not); the convention as `conventions.md` 9 with `conventions_history.md` the first twin (the rulings, F2's first run, the withdrawn override, the selftest's measured 350 s); the one dangling lineage ID explained where it is set; the README Layout brought to the tree. **H10** (harness `185158e`): `bbh check-skills` / `bbh skill-guide` lifted config-driven — `[skills]` + one `[skill_<PFX>]` table per skill; `test_skills` with ten must-fires (two fixture defects of mine on the first run: a `run` helper passing its directory through, the generator script pointed at the wrong tree; and one selftest fixture — a header at file offset 0 is invisible to the section finder, as it is in the lineage); **F11 EXACT**: 18 lines identical over the eight skills, both guides CURRENT. **The skill**: `skill/blackbox-harness/SKILL.md` — 87 rules in eight sections (against ~75-80 estimated), placed by a script that inserted each anchor before a unique phrase and refused any phrase not found exactly once; three doc paragraphs split so each carries one rule; `GUIDE.md` 69.5 KB generated, zero "not found" incidents; `skill/skills.toml` the lock (level 0 + the lineage tokens + the nine bracket IDs forbidden); installed as the symlink `~/.claude/skills/blackbox-harness`; harness commit `e643219` PUSHED after its selftest **30 / 2 / 0**. This tree: `harness_scope.md` §4 H10 + skill rows, §5 F11, §9 LANDED; NEXT_SESSION rewritten (living docs L1 next); the memory note |
| **CLOSE** | harness selftest with `ROMDIR`: **PASS 29 / SKIP 2 / FAIL 0** (`test_rpl_lua` no standalone lua, `test_fidelity_mame` opt-in; `test_fidelity_vampire` 106 s — F1 identical on the shell-crash gate, F9 ref-rot identical), harness commit `3eff2d4` PUSHED (standing-authorised); this tree's static tier run ALONE afterwards: **PASS 137 / SKIP 0 / FAIL 0** (`build/static_14z139_close.log`; 136 + `test_ref_rot_image_pick`), tree clean during the run; committed to main, NOT pushed (the maintainer's word, as always). **CLOSE (2), after the eight rulings:** harness selftest again **29 / 2 / 0**, harness commit `45b5724` PUSHED (`docs/conventions.md`, `docs/rebaselines.md`, `BBH_FIDELITY_ROOT`); this tree's fidelity gate PASS through the new input and the must-fire on a non-tree root exit 1; the static tier ALONE again **PASS 137 / SKIP 0 / FAIL 0** (`build/static_14z139_close2.log`), tree clean during the run; committed, NOT pushed. **CLOSE (3), after the skill:** the static tier ALONE a third time **PASS 137 / SKIP 0 / FAIL 0** (`build/static_14z139_close3.log`, `test_bbh_fidelity` carrying F11 through the harness's own test), tree clean during the run; committed, NOT pushed. STATE at ~152 KB — the 14z-137 group rolls at the next close if it grows NEXT_SESSION's order unchanged: the harness SKILL, then living docs, then the remaining open items |
| **(6) THE LIVING DOCS EXECUTION PLAN, WRITTEN FOR ANOTHER SESSION TO EXECUTE** (the maintainer, nearly out of Fable capacity: *"make the whole plan for the living docs (all the slice, the opening, ending, directions, things to keep in mind, etc.) so that I can ask Opus to actually execute the plan"*) | three explorations (the doc tooling behind L1/L4, the session ritual and the harness §9 precedent form, the L2/L3 material) and one design pass on the four risk points; the plan written OUTSIDE the tree at `~/.claude/plans/well-i-m-almost-out-glimmering-koala.md`, copied untracked to `build/living_docs_plan_14z139.md`, pointed at from NEXT_SESSION item 1 (`e84a8874`). What it holds: the standing frame every living-docs session runs inside (the opener, the eight-check battery with exit codes captured, the ALONE rule with the runner's real `GREEN`/`NOT GREEN` lines, the close, eleven paid-for traps); the per-slice shape copied from `harness_scope.md` §9 — measure → the slice's plan section written into `living_docs_scope.md` (§8 L1, §9 L4, §10 L2, §11 L3) → STOP for the rulings → execute in a fixed order → land; one section per slice in the ruled order with the tool and gate shapes, must-fire controls, staleness rows, sequencing and the numbered decisions for the maintainer (L1 five, L4 seven, L2 seven, L3 seven); and the one cross-slice decision — no NOTE verdict exists, so the recommendation is a `NOTE: <key> <value>` marker surfaced by an advisory block in `run_all_static.sh`, never a fifth verdict in the shared `classify.sh`. **Ground truth the scope document already had stale, measured today:** 60 hand-written docs, not 66; `annotations.md` 2,958 distinct / 473 carried by atlas / ~440 ROM tier (the L3 denominator, not 2,970); the six `probe_*.toml` are GITIGNORED, so `git ls-files build/manifest` is L2's mechanical canon; `qs_voice_map.md` declared REFERENCE while its body says GENERATED; `test_mister_page` registered ci_static with a ci_portable header; `tests/lib/classify.sh` knows only PASS/SKIP/FAIL/TIMEOUT. Not executed here, by design: the next session (14z-140) folds L1's section into the scope document and stops for the rulings |
| **CLOSE (4)** | the ROM audit 76/76; the static tier ALONE **PASS 137 / SKIP 0 / FAIL 0** (`build/static_14z139_close4.log`), tree clean during the run; the memory note `living-docs-effort` updated with the plan's location; HANDOFF untouched (nothing it describes changed — the plan is outside the tree); NEXT_SESSION amended IN PLACE (the 14z-139 opener, not re-archived — the 14z-138 CLOSE (2) precedent); the eight doc checks green; committed to main and PUSHED at the maintainer's word (*"do the end of session ritual and push after that"*) |
