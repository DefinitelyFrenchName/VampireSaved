...
## Session 14z-147 — **STEP TWO OF THE MUST-FIRE MACHINE, PASS 1: the contract's READER, the runners that EXECUTE every
## declared control, and the whole PORTABLE tier retrofitted — 30 gates declaring under one grammar, every control fired,
## every mode honoured. No build byte moved.**

| | |
|---|---|
| opened with | the opener read (HANDOFF, STATE, NEXT_SESSION in full), ROM audit **76/76**, `main` == `origin/main` at `77f54e92`, the skills `vampire-savior-engine` + `vampire-saved-port`. The maintainer: *"step two of the must-fire machine"* |
| **the scope, measured first** | 78 declaring gates (27 portable, 19 static, 34 emulator; `run_battery_m2` in none) in ~30 spellings, 10 header-only; BBX's `docs/controls.md` re-read: R10 the grammar, **R29 the executable form (`CONTROL=<name>`) and R30 the header rule (the leading comment block; a bare `#` CONTINUES it) had both landed since 14z-145 — the "compatibility trap" the STATE entry warned about is resolved by R30**. The plan: the reader + classifier + runners + ground truth first, then the retrofit tier by tier with one strict run per pass as the identity bar (the agreed point 3) |
| **THE READER AND THE RUNNERS** | `tests/lib/controls.sh` — the four regexes COPIED from BBX (`vs_ctl_declared` / `vs_ctl_none` / `vs_ctl_mode` / `vs_ctl_fired` / `vs_ctl_dead` / `vs_ctl_read`), never a dependency; `classify.sh` gained a 4th argument, the gate SCRIPT: on a PASS the header's declarations are read against the log's `CONTROL FIRED:` / `DEAD:` lines and a red block is plain **FAIL** (no fourth verdict, the agreed point 4; UNDECLARED is a counted NOTE — 236 gates predate the grammar); `vs_classify_control` — ONE copy of the executable verdict (HONOURED / LIES / REFUSED / DIED / TIMEOUT) so no runner carries a second shell-error regex (test_static_runner §8's lock caught my first draft doing exactly that). `run_all_static.sh --exec-controls all\|portable\|none` (default all): every declared control of a PASSING gate re-run as `CONTROL=<name>`, the readout `fired N / declared N` + `executed N honoured/lies/refused/died`, failures named `<gate>(control:<name>:<verdict>)`. `run_all_emulator.sh`: the readback on every run, `--controls` for the executable rows (`<gate>@<name>`, PASS = honoured, in the tally and under `--strict`), off by default. `run_battery_m2.sh`'s `bat` hands the script too. All three runners declare `# MUST-FIRE: none — …` |
| **ground truth** | `tests/test_controls_contract.sh` (NEW, ci_portable: the grammar's four regexes, the leading-block rule incl. a bare `#`, an indented FIRED line is prose, declared-vs-fired → OK/RED/UNDECLARED/NONE, the classifier's FAIL on a DEAD line and on an undeclared firing, identity for SKIP/FAIL/undeclared and for the 3-argument form, the mode + REFUSED exit 3 — dogfooding two `known-bad` controls); `test_static_runner` §11 (read: PASS/FAIL per stub, `fired 1 / declared 3`, tally PASS 3 FAIL 3) + §12 (executed: LIES/REFUSED/DIED rows, `--exec-controls none` and `portable` as the must-not-fire) + §13; `test_emulator_runner` §14; `test_battery_accounting` §7-8 — every one green through the shipped runners |
| **THE CENSUS on the grammar** | `test_must_fire_census.sh` rewritten: `declares` (R10 lines in the leading block, grows only), `header-only` (no `CONTROL FIRED:`/`DEAD:` literal or `vs_ctl_fired`/`dead` call outside comments, shrinks only), **`retrofit-debt`** (a pre-grammar mention with no declaration, shrinks only) — with its own two declared controls, and honouring its own modes. FROZEN: **30 declaring / 0 header-only / 53 retrofit debt** (= 19 static + 34 emulator; nothing portable left). A NOTE line carries the three counts |
| **THE PORTABLE TIER RETROFITTED — 27 gates + the census + the contract + 2 runner ground truths** | the pattern (now `docs/project/must_fire_contract.md`): header lines after the first bare `#`; ONE `perturb <name> <copy>` function setting `EXPECT`, called by the control section (prints the FIRED/DEAD line) and by the mode, where the perturbed copy IS the main check's input (`ROOT` for the doc gates, `MANIFEST`, `REGISTRY`, `PAGE`, `SIDE_B`, a shadow tool's path, the fixture for a known-bad). Per gate, verified with a loop through the shipped classifier: plain run PASS with every control FIRED, then every `CONTROL=<name>` run HONOURED — **30/30 gates, 91/91 modes**. Among them: `test_mister_wide_gate` (7 modes, five of them Verilator benches; the bench modes REFUSE where Verilator is absent rather than passing), `test_docshape` (15), `test_checkskills` (11), `test_md_subset` (9), the three runner ground truths (`shadow-tool: reader-unplugged` — a copy of each runner with the 4th argument dropped must let a dead-control stub read PASS), the two former header-only debts `test_ref_rot_image_pick` (its adversarial copy) and `test_tenant_id` (a real control: donovan.toml minus `id_by_profile` must move the WIDE resolution off 0x13) |
| **the identity bar** | the close's strict 144/0/0 at `77f54e92` is the BEFORE (a background baseline run I then edited the runner under was VOID — [VSP-110], the byte-offset trap, recorded in NEXT_SESSION); AFTER: the portable tier **PASS 70 / SKIP 0 / FAIL 0** with `fired 99 / declared 99` and `executed 99: honoured 99, lies 0, refused 0, died 0` — **21 min 53 s wall** where the tier alone was ~3.7 min: the executed controls cost ~18 min (test_mister_wide_gate's seven Verilator modes ~47 s each, docshape's fifteen ~11 s, checkskills' eleven ~10 s). That cost is part of the open cadence decision below and the full static strict in the close row |
| **docs and the close** | `docs/project/must_fire_contract.md` (REFERENCE, shape row, README Contents), [VSP-181] anchored in HANDOFF "How to test" + the routing row + the usage lines, the skill rule, PROVENANCE row (three classes), two project gotchas (the bashism stub; the `set -e` list + comment-stripped-read traps), the gate index (319 scripts), annotations, gotchas index, the anchor census re-frozen (+1), the eight doc checks green. Decisions pending: the must-fire entry marked (pass 1 landed; passes 2-3 open; ONE decision — the cadence of `--controls` at release, recommendation (a) the release checklist). Retractions of mine this sitting: the baseline run (void), the census's verdict regex (missed the lib's helpers — 25 false header-only until fixed), the "PASS 3" tally expectation in §14 (PASS 4 is right: the honoured row counts) |
| **the first strict run was RED — 144/0/1, `test_bbh_fidelity` F1** | the lineage's static runner printed the new controls block over bbh's declaration-free fake repo, and bbh's runner does not: the F1 identity broke on exactly the delta this session added. FIX in the lineage: the block prints only when the tier declared or executed anything, so a declaration-free tree is byte-identical to bbh's output again (F1 exact, the whole fidelity gate PASS, `test_static_runner` still green). The honest residue: F2 (opt-in, the real portable tier through both runners) carries the block as a KNOWN delta until the reader is lifted into bbh — that lift is the follow-up, recorded in NEXT_SESSION, and bbh itself was not touched this sitting |
| **the cadence question the measurement raises** | `--exec-controls all` is the agreed shape ("the runner invokes it") and it turns the everyday pre-commit portable tier from ~4 min into ~22 min. Kept as the default — a lying control must not pass a pre-commit — with `--exec-controls none` as the developer's iteration knob; the maintainer's call whether the executed form belongs to every pre-commit or to the close and the release only (STATE "Decisions pending", the must-fire entry) |
| **14z-147b — PASS 2, THE STATIC TIER (the maintainer: *"pass 2 now"*)** | the 19 static-tier gates of the `retrofit-debt` class retrofitted on the same pattern, each verified plain-PASS with every control FIRED and every `CONTROL=<name>` mode HONOURED: `audit_capture_matrix` (2), `audit_mister_map_fit` (3), `test_capture_kf_ownership` (3), `test_charmap_current` (2), `test_charmap_overrides` (3), `test_checkdocs_rom` (5), `test_community_crosscheck` (1), `test_df_startup_provenance` (1), `test_escape_triage` (1), `test_freeze_artifacts_current` (6), `test_mister_mra_map` (2), `test_mister_page` (3), `test_mra_build_line` (1), `test_pcrel_escapes` (1), `test_reconcile_matcher` (1), `test_release_roundtrip` (5), `test_tables_current` (1), `test_tenant_anim_relocation` (1), `test_win_quote_decode` (2) — **44 controls, 44/44 fired, 44/44 modes honoured**. Where a gate's controls lived in a python body the FIRED/DEAD lines print from python; where a control needs an artifact the host may lack (a jtcores clone, a release dir, a registered build) the mode REFUSES rather than passing. Two "controls" were NOT declared because R10 does not call them must-fires: a must-not-fire (`history-is-not-a-target`, a missing workbook SKIPs) and a positive check (`charmap_overrides` compile writes one row); `test_charmap_current`'s (b) became a real must-fire (an override must CHANGE the map). The census re-frozen: **49 declaring / 0 header-only / 34 retrofit-debt** (= the emulator tier, pass 3). One trap paid again, the `&&`-as-last-statement abort under `set -e`, in `test_mister_page`'s shadow builder — caught by the plain run, fixed with an `if` |
| **CLOSE (14z-147b)** | pass 2 landed and locked; the census 49 / 0 / 34; **static strict 145 / 0 / 0 GREEN on the final tree, `fired 143 / declared 143`, `executed 143: honoured 143`** (36 min 07 s wall — the static tier's 44 executed controls added ~6 min); the maintainer's new open item recorded (the release contents definition + the end-user how-to README, below in Decisions pending); no build byte moved; pushed per the close rule |
| **CLOSE** | Step two, pass 1, complete and locked: the reader, the classifier, three runners, four ground truths (one new), the census on three frozen classes, thirty gates declaring under the grammar with every control fired and every mode honoured (99 of 99 both ways), the spec, the rule, the skill, two gotchas. **Static strict 145 / 0 / 0 GREEN on the final tree** (30 min 35 s wall; the first strict run's one red, F1, fixed and recorded above); the eight doc checks green; STATE rolled (14z-146 to the archive, 173 -> 163 KB); opener rewritten (previous archived). **No build byte moved, M18 untouched.** Pushed per the close rule |
| **14z-147c — PASS 3, THE EMULATOR TIER (the maintainer's opener item "Must-fire pass 3")** | the 34 emulator-tier gates of the `retrofit-debt` class retrofitted on the same pattern — each declares its must-fire control(s) under R10, prints `CONTROL FIRED`/`DEAD`, and honours a `CONTROL=<name>` mode that perturbs the gate's REAL measured input (a trace, table, dump or image — never a synthetic copy) and reaches the gate's own FAIL. The census re-frozen: **83 declaring / 0 header-only / 0 retrofit-debt** — the retrofit is COMPLETE across all three tiers. The shapes: compare-baseline (frame/aerial join), demand-the-forbidden-outcome (df_dead_family candidate-reached, tech_roll tap-must-not-roll, don_reactions knockdown-forbidden, suite_dispatch program-fallback, shadow_tenant four-presses-morph, kill_poke_shape two-byte-flowed, ladder_selector mask-not-load-bearing), env-override (hui_grunt, pyron_ring), inject-into-real-stream (voice_borrow, guard_mask_reads, defense_row_residue, rehit_ring), perturbed-frozen-copy (df_accumulator, df_startup_invuln, prg_window, dualtrack), force-value (walker_repoint, meter_gain, tick_durations), wrong-map/wrong-page (sdram_census, obj_records, obj_oracle, tenant_oracle), palette (down_flash, pyron_medallion), EXPECT-flip (clone_beam_lines), self-contained expected-set (random_select). A control the host cannot run (a pruned build, an absent recording, no Verilator/jtcores) REFUSES its mode with exit 3, never self-skips into LIES. `audit_guard_corpus`'s control is the 14z-93 crash rig (build/hui41 + 26_don_arcade_mash) as an executable mode |
| **the four forks and what they taught** | the retrofit editing was FANNED OUT to four forked agents on disjoint file sets, then STOPPED: three completed cleanly (`test_meter_gain`, `test_suite_dispatch`, `test_vanilla_aerial_join`) and one left `test_tick_durations` half-done (killed mid-edit), while a fifth fork's self-report HALLUCINATED being the parent ("I'm stopping all four") — a false alarm; the tree audit showed no file double-edited and none corrupted. Lesson recorded: fork editing parallelises the tedium but the observability is poor (a killed fork leaves a partial file; a confused fork misreports), so the remaining 29 gates were done in-session, reviewably, with ROM-free census checks between batches |
| **two fixes caught in verification** | `test_random_select_tenants` used `CONTROL` as its OWN env var (the previous-build path), colliding with the must-fire mode selector so the plain run got REFUSED — renamed to `PREV_BUILD` and the control made SELF-CONTAINED (the mode empties the expected roster so the real tenant-including draw mismatches section 2), removing its dependency on a pruned old build; `audit_walker_repoint`'s mode needs its positional build arg to verify honestly (a run without it dies on the usage line and reads as a false HONOURED). New project gotcha recorded (the `CONTROL`-env collision) |
| **CLOSE (14z-147c)** | pass 3 landed and locked; census **83 / 0 / 0**; **static strict 145 / 0 / 0 GREEN, `fired 143 / declared 143`** — the identity bar UNCHANGED from 14z-147b (the 34 are emulator-tier; the static tier does not run them and the frozen census passes); controls-contract, static-runner ground truths green. VERIFIED end to end (plain PASS + mode HONOURED) on 14 cheap gates spanning every shape; the HONOURED run of the ~20 expensive gates (reducers, the four MiSTer/Verilator gates ~1h each, guard_corpus ~41 min) is the freeze/release emulator sweep, per NEXT_SESSION's scoping of pass 3. No build byte moved, M18 untouched. Commit `f8b8ee28`; pushed per the close rule |
...
- **DEFINE WHAT IS IN A RELEASE, AND SHIP THE HOW-TO README (maintainer, 2026-09-10,
  ...
  how-to since we don't provide any ROMS or such copyrighted assets, ever"*).
  ON THE OPEN-ITEMS LIST; not scheduled.** What exists to start from:
