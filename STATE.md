...
## CHARACTER — neither global nor inherited), THEN THE MAINTAINER'S QUESTION "WHERE DO THE VALUES COME FROM?"
## TURNED IT INTO A PRESERVATION FINDING: vs2 AND vh2 carry the VS-style DF handlers for ALL 18 and never reach
## them, so the port RESTORED Capcom's own values. A dedicated document opened for that class. AND the frame-data
## privacy rule RULED AND SHIPPED the same session. No build changed. Strict static 124/0/0/0. PUSHED.**

| | |
|---|---|
| opened with | the 14z-125b close; NEXT_SESSION's ordered arc, and the maintainer's "proceed as planned, in order" |
| **item (1), ANSWERED** | the DF startup window is `+0x147` (the victim's invincibility timer, the hit test's gate at `PRG:0x018064`, decremented by the System Timer Reducer `0x02246E`), armed in the first sub-state of the per-character seq-0x16 handler `dispatch_16` selects — **neither a global property of the activation (the shared body arms only `+0x143` = 0x14, throw immunity) nor inherited from the shell**: the tenants' rows are repointed to their own vs2 handlers (Donovan 64 / Huitzil 79 / Pyron 41 ticks; shells 59 / 41 / 41). All 15 vanilla windows measured and frozen for the first time. Gate `tests/audit_df_startup_invuln.sh` (22 trace legs + 4 contact legs, both checkers ground-truth tested). No gameplay change proposed |
| **the finding it became** | the maintainer asked where the values come from, given vs2 shows no window. Measured across all three ROMs: vs2 and vh2 carry VS-style seq-0x16 handlers for **all 18** characters and switched them off for everyone (the new activation body clears seq 0x16 the same frame); every vanilla row arms the SAME value in vsavj/vsav2/vhunt2, the three tenant rows the same in vs2 and vh2 — and NOT the value of the vsavj row they alias. So the port RESTORED Capcom's own numbers. Locked by `tests/test_df_startup_provenance.sh` (ci_static). The maintainer's reading is recorded AS ASSESSMENT, with measured and inferred kept apart and "when the handlers were written" stated as unknowable from the ROMs |
| **the document the maintainer asked for** | `docs/game/preserved_data.md` (REFERENCE, banner, routed from `docs/README.md`) — the class "what a shipped ROM carries but never reaches": the DF handler family, Oboro's complete dataset in vsavj with no player-facing path, the executable-but-unused third Z80 song class, and a qualifying rule (bytes present; unreachability MEASURED with a positive control; coherent content). The dead-row class is explicitly excluded so the two do not blur |
| **the last candidate, closed the same session** | the maintainer said "do it": `tests/audit_df_dead_family.sh` probed the VS-style field setter at vs2 `0x02622A` on native vsav2 — 0 hits on a tenant AND a vanilla leg, while the positive control `0x02619E` fires exactly once at f3260 on both. Folded into entry 1; the Candidates list is empty |
| **the frame-data privacy rule, RULED AND SHIPPED (option (b))** | the class rule: every per-move ROM-derived table, OURS AND THIRD-PARTY ALIKE, is generator output under `../charpages/framedata/` (`tools/framedata_pages.sh`). Seven files left the tree; `community_crosscheck.md` became VERDICT-ONLY (1060 → 359 lines, 0 per-move value rows, no workbook values) with `--md-full` writing the full page out of tree; currency hash-locked (`charmap_pages.sha256` 6, `vanilla_hit_damage.sha256`), the perturbed-hash control proven to fire. Public history ACCEPTED, not rewritten |
| **three defects found, all ours** | (1) the artifact publish was refused by the auto-mode classifier and went through only after the maintainer's explicit word — ask BEFORE spending the ~50 k-context reads per page. (2) `framedata_pages.sh`'s in-repo refusal was INVERTED (the default textually starts with `$REPO`; `./nope` inside the repo passed) — canonicalised before any `mkdir`, so a refused run leaves no litter either. (3) A LATENT bug in `test_vanilla_frame_join.sh` predating this session: section 3's re-freeze kept the header with `head -6` where it is SEVEN lines, silently dropping the column legend on every `FREEZE=1` — invisible because the compare strips `#` lines. Fixed and the line restored |
| **two 14z-123 attributions corrected, in mizuumi's favour** | `0x2246E` is the System Timer Reducer, not "the class-0xFF block handler" (the block-ENTRY handler `0x2395A`-`0x23966` opens the advancing-guard window; the write tap had named the decrementer); `+0x161` is a per-character DF work byte — "Oboro Fight Flag" is Bishamon/Oboro's use of it. Both wiki disagreements from the 14z-124 inventory are closed; four wiki rows adopted into `ram.md`, ~90 candidates left [C] |
| the errand | the three character-page artifacts REPUBLISHED (Donovan `85d7fd52`, Huitzil `f0dddc83`, Pyron `ad618f12`) from the committed generator output, each live page and file read in full first; they are now regenerated OUT of tree, so future republishes come from `tools/framedata_pages.sh` |
| atlas rows added / upgraded | `+0x147`, `+0x143`, `+0x134`/`+0x11E`, `+0x145`, `+0x1B3` (Anakaris's own flag — mizuumi's lead, character-specific), `+0x161`, `+0x1AB`. Instrument note frozen with the data: `field_trace` samples one reducer tick AFTER an opener's write |
...
| the close battery, exit statuses captured directly | census `rc=0` · `checkdocshape --no-pending` `rc=0` · checkdocs `rc=0` · checkskills `rc=0` · `gen_gate_index --check` `rc=0` · gotchas index `rc=0` · annotations current `rc=0`. Portable tier 61/0; **strict static tier PASS 124 / SKIP 0 / FAIL 0 / MISSING 0** |
| patch_notes / patch_index | **checked, and correctly NO entry**: `git diff` over `build/manifest` and `tools/gen_donovan_patch.py` since the last push is EMPTY — no shipped byte moved and nothing was frozen |
| rollover | STATE crossed the ~150 KB line with this entry, so the 14z-124 group moved VERBATIM to STATE_HISTORY with its ledger line. STATE holds 14z-125 / 14z-126 |
| not done, by design | **item (2) Zabel j.LK — the maintainer will provide the recording in a FUTURE session**; item (3) Jedah's crouching recovery (needs a tick-accurate instrument); the HANDOFF shape item; the ~90 unadopted mizuumi candidates |
| push | **PUSHED** |
...
| **the errand — DONE on the second pass** | the three character-page artifacts: the first publish attempt was REFUSED by the auto-mode permission classifier (outward-facing action) and not worked around; after the maintainer's explicit "do it" the same call went through. All three REPUBLISHED from the committed generator output (`4a3483f`): Donovan `85d7fd52`, Huitzil `f0dddc83`, Pyron `ad618f12` — each live page and each committed file read in full first (~50 k context per page, as budgeted). The pages now carry the 14z-125 `active` fix, the per-strength strip labels, the sprite/composite styles and the move-named projectile tables |
...
| gotcha filed | project: "A WRITE TAP ON A COUNTDOWN FIELD NAMES THE DECREMENTER, NOT THE OPENER" (+ the sampled-one-tick-late face) |
| **THE PRESERVATION FINDING (the maintainer's question "where do the values come from?", then "we absolutely need to document this")** | measured three ways: vs2 and vh2 carry VS-style seq-0x16 DF handlers for ALL 18 characters, switched off for everyone by the new activation body; every vanilla row arms the SAME `+0x147` value in vsavj, vsav2 and vhunt2; the three tenant rows arm the same value in vsav2 and vhunt2 (handlers identical but for pointer-shifted operands) and NOT the value of the vsavj row they alias. Locked by the new static gate `tests/test_df_startup_provenance.sh` (ci_static, seconds, must-fire control). The maintainer's assessment, recorded as assessment in engine_internals: by repointing the rows the port RESTORED Capcom's own values for these characters on this mechanism — "incredibly lucky", and evidence the port is likely as close as Capcom intended, balance untested. Open and stated: WHEN the handlers were written (pre-VS-cut or early vs2 development) cannot be told from the ROMs. At the maintainer's request a small dedicated document now catalogues the class: **`docs/game/preserved_data.md`** (REFERENCE) — this family, Oboro's dataset in vsavj, the unused Z80 song class, and one unmeasured candidate (the vsav-style DF field setter at vs2 `0x2622A`) |
| **the frame-data privacy route, RULED AND SHIPPED (option (b))** | the maintainer agreed with the recommendation, so the class rule landed the same session: per-move ROM-derived tables, ours and third-party alike, are generator output under `../charpages/framedata/` (`tools/framedata_pages.sh`, new; it canonicalises and refuses an in-repo output dir). SEVEN files left the tree (3 `_anim.md`, 3 `.html`, `vanilla_hit_damage.tsv`); `community_crosscheck.md` became the VERDICT-ONLY rendering (1060 → 359 lines, 0 per-move value rows, no workbook values) with the full page written out of tree by `--md-full`; currency is now hash-locked (`charmap_pages.sha256` 6 entries, `vanilla_hit_damage.sha256`), and the perturbed-hash control was proven to fire. Carriers re-pointed: engine_internals ×3, the tables README, HANDOFF, both gate headers |
| **two defects found while shipping it, both ours** | (1) `tools/framedata_pages.sh`'s in-repo refusal was INVERTED — the default `$REPO/../charpages/...` textually starts with `$REPO`, so a prefix match rejected the correct path while `./nope` INSIDE the repo passed; fixed by canonicalising first ([VSP-108]) and both directions re-tested. (2) A LATENT bug in `test_vanilla_frame_join.sh` predating this session: section 3's re-freeze preserved the header with `head -6` where the header is SEVEN lines, so every `FREEZE=1` run silently dropped the column legend — invisible because the compare strips `#` lines. Fixed to preserve the whole leading comment block; the dropped line restored |
...
| not done, by design | the maintainer will provide the Zabel recording in a FUTURE session — item (2) Zabel j.LK — needs the maintainer's hand-played RECORDING first ([VSP-20]); item (3) Jedah's crouching recovery (needs the tick-accurate instrument); the frame-data-privacy implementation (awaits the class ruling); the HANDOFF shape item; the ~90 unadopted mizuumi candidates |
| green | `audit_df_startup_invuln` PASS (22 + 4 legs, both controls); `test_df_startup_provenance` PASS; strict static tier **124 / 0 / 0 / 0** |
| push | **PUSHED** at the maintainer's word: `26255a9..4d97ec9` (five commits) |
...
| **an open question it raised about the TENANTS** | `moves_huitzil.toml` labels `a2:0x01/03/05` as `6LP`/`6MP`/`6HP`, filled by `name_moves.py` performing `6`+button at a FAR pin. On vanilla characters those odd slots are the FAR STANDING NORMAL — and the maintainer's own note on those rows reads "Alternate attack: different from 5LP (usually longer reach and different data)", which describes a far normal exactly. So the tenants' `6XX` labels may be a naming artifact of the rig's input choice rather than distinct command normals. NOT measured and NOT corrected: settling it means running the two-distance rig on native vs2 for the three tenants. Nothing shipped depends on the label — it is documentation |
...
**Ledger rollover:** the 14z-124 group moved verbatim to STATE_HISTORY.md at
the 14z-126 close — STATE had crossed the rule's ~150 KB line with the close
entry. STATE holds 14z-125 / 14z-126.
