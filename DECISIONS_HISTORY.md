...
**The question (14z-183, asked before any byte moved), verbatim:** *"#157's fix changes shipped bytes. Its candidate fix corrects only the three hit-pair write sites in each tenant's throw code, and leaves the two state-byte clears where they are, since it was never shown which rows broke Donovan's throw in 14x. I would build it as a probe build, not a freeze. First I'd measure how far it reaches, then run the throw-pair, tenant-throw, capture, move-parity (meter AND throw damage, the check you ruled at 14z-182) and lag gates plus the legacy oracle. You would then play it with a recording, because 14x's breakage only showed in play. Nothing is frozen or shipped until you rule on the result. Does the 'analysis first' ruling still hold #157 back, given #177 (no movement gate) and #178 (Hop Kick in Dark Force never compared) were opened at 14z-182?"* The option chosen: *"Probe build now (Recommended)"* — *"M19 already landed four #136 fixes, so every divergence family counts as attributed; #177/#178 are coverage gaps, not unattributed divergences. Scope, build the probe, measure, then hand it to you to play. No freeze."*

**What it means.** The 2026-09-18 ruling "#136: ALL the analysis first" no longer holds #157 back: its families are attributed, and #177 / #178 are coverage gaps. The probe's rows live in manifest COPIES under `build/`, so no tracked manifest and no shipped byte moves; the rows enter the tracked manifests only after the maintainer rules on the probe's measurements and playtest, through the rule-checker's `build` kind, and a freeze is a separate decision.
...
The four findings: (1) **movement physics, measured live** (#136's body) — no gate compares a tenant's movement with native vsav2: Phobos's `test_hui_walk.sh` compares with frozen values and `test_hui_air.sh` asserts thresholds, neither with a native leg; none for Donovan or Pyron; the parity gate compares x/y only inside move events — **GitHub #177**; (2) **Hop Kick in Donovan's Dark Force** (the thread's "native lands, ours whiffs") connects on neither leg in either rig — `move_parity_events.tsv` `donovan_6 21` NOT-IN-DF, `df_moves.tsv` `donovan_dfx2 22` SAME with no hit — so it was never compared — **GitHub #178**; (3) **the throw-damage scaler question** ("the scaler's effect on tenant throw DAMAGE is unmeasured") — throw damage IS compared per event (the parity gate's `p2hp`), the mechanism is not, and #157's pair-store fix is what would move the scaler's reads — a named check on **#157** (commented there; `docs/game/engine_internals.md` points to it); (4) **`docs/project/coverage_matrix.md`'s tenant-moves row** read COVERED while its body still listed the victim, the guard cancels and hitboxes in play as uncovered — corrected. #136 stays closed (an item left open is its own ticket); a comment on #136 records the re-derivation.
