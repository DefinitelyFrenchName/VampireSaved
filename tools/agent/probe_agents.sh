...
#       as a leg so the day it starts firing this probe says so
...
#
# Usage: tools/agent/probe_agents.sh [SCRATCH_DIR]   (~3 min: ten short Haiku/Sonnet runs)
...
defn() { # name model effort tools [extra frontmatter]
  {
    echo "---"; echo "name: $1"; echo "description: Probe worker $1."; echo "model: $2"
    [ -n "$3" ] && echo "effort: $3"
    echo "tools: $4"
    [ -n "${5:-}" ] && printf '%s\n' "$5"
    echo "---"
...
fail=0
run() { # tag model effort gate prompt
  PROBE_LOG="$S/log_$1.jsonl" PROBE_AGENT_GATE=$4 claude -p "$5" --model "$2" ${3:+--effort "$3"} \
