...
def render_md(vanilla, cmp_, full=False):
    ...
    A("- **DE, MO, FE, LE, LI** take odd at far / even at near on every button, LP included;")
    A("  **SA** the same, his HP a knife edge at the rig's near separation (0x04 at 59 px,")
    A("  0x05 at 60); **GA, VI** have no close LP (`0x00` at both), GA none for MK either;")
    A("  **BU** has no close variant for HP, MK or HK, **AU** none for HP. (Until 14z-183,")
    A("  GitHub #134, this read \"LP is `0x01` at both distances\": the rig's first event, LP,")
    A("  was pressed at mid range because the round intro ate its walk-in.)")
    A("")
    ...
    A("alternative boxes for ONE hit — the victim's recent-hit slot refuses the second")
    A("([VSE-43]) — so only one of them can land. We take one; **the workbook sums them**.")
    A("The rule itself was MEASURED 14z-146 (`tests/test_rehit_ring.sh`): the slot holds the")
    A("last landed id and the engine clears it on every tick the attacker's node has no")
    A("attack record, so consecutive attack nodes sharing an id land once and a same-id")
    A("record lands again after any gap node — JE 5HP `2(5)2(5)2` lands three times, and")
    A("SA CL.5HK `2(2)3` lands once for a different reason (its first hit launches the")
    A("victim and the juggle gate refuses the second: a victim-state effect, not the chain's).")
    A("")
    A("")
    if not full:
