...
# they are what the committed rows were generated under. They are pinned to
# TODAY'S MEASURED BEHAVIOUR, not to a better one: changing either re-resolves
# rows and therefore moves built bytes, which is CLAUDE.md rule 6 and a
# separate ratified step (#43(b) — the maintainer split this issue on exactly
# that line).
#
# Measured before the refactor landed, over all 328 reconciliation rows x the
# 5 retry windows below: the drifted copy and the canonical matcher at these
# values agree on 1640 of 1640 probes, and 183 of 1640 CHANGE at (None, True).
# The first number is why this refactor cannot move a row; the second is why
# the parameters are not decoration.
HIT_CAP = 64
