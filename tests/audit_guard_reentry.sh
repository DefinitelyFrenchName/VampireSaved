...
# and two `pixels` rows (added 14z-168): snapshots at hit+14..+26 of the 3640 event,
...
# difference between vsav2 and vsavj was visible there, so whether a player feels the
# re-entry is the maintainer's question, not a finding of this gate.
#
# Usage: ROMDIR=... [MAME_BIN=...] [BUILD=build/m3b_merged26] [FREEZE=1] tests/audit_guard_reentry.sh
#   emulator tier, MAME; three field_trace and three snapshot legs in parallel — measured 14z-168 on this MacBook, solo: ~10 s wall
set -eu
...
case "$CONTROL" in ""|vs2-pattern) ;; *) echo "REFUSED: no control named '$CONTROL' is declared by this gate"; exit 3 ;; esac
