...
#   rompath_dir default: build/hui58/rompath — the current huitzil freeze.
...
RPDIR="${1:-$REPO/build/hui58/rompath}"  # re-pointed 14z-128 <- build/hui4 (19 members, no vsw.z01 — pre-WIDE-v1.1 and unbootable on current binaries; the sweep found it as "no dump files", and test_build_ref_rot could not see the $REPO/-prefixed form until the same session)
