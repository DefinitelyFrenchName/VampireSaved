...
#
# Usage: ROMDIR=... [BUILD=build/m3b_merged27] tests/audit_hui_grunt.sh
#
...
BUILD="${BUILD:-build/m3b_merged27}"   # merged-m7 since the 14z-110 freeze  # re-pointed 14z-117b (random-select freeze) <- 14z-117  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
...
OURS_BY_BUILD = {
    ...
    "m3b_merged27": [None, "02a2", None, None, None],   # 14z-113 merged-m10: same program as merged16 (one-zip repackaging)  # re-pointed 14z-117b (random-select freeze) <- 14z-117  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
}
if ours_a2:
