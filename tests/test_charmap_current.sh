...
#
# Usage: tests/test_charmap_current.sh   [DON=build/don_m24 HUI=build/hui58 PYR=build/pyron43]  # re-pointed 14z-127 (the boot-title freeze; the ONLY leaves that moved were provenance -- /inputs/manifest, /inputs/verify_data.bin, /sources/ours/set -- zero character-data bytes) <- 14z-119 (physics-port freeze) <- 14z-117b
#
...
DON="${DON:-build/don_m24}"  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
HUI="${HUI:-build/hui58}"  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
PYR="${PYR:-build/pyron43}"  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
fail=0
ok()  { printf '  ok    %s\n' "$1"; }
bad() { printf '  FAIL  %s\n' "$1"; fail=1; }
