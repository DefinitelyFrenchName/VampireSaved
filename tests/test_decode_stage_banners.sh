...
for spec in build/hui41:0x10 build/pyron26:0x11 build/don_m21:0x13; do  # don re-pointed 14z-111; 14z-117  # re-pointed 14z-117b (random-select freeze) <- 14z-117  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
  d=${spec%:*}; cls=${spec#*:}
...
PY
  ...
  case "$d" in
    build/don_m21) [ "$n" = 0 ] || fail "donovan must be clean, got $n" ;;  # re-pointed 14z-117b (random-select freeze) <- 14z-117  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
