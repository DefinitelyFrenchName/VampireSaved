...
#
# WHAT: the level-0 skills' GUIDE.md files are CURRENT against SKILL.md and the
#   documentation paragraphs each rule is anchored to (the human rendition, each rule
#   followed by its incident), generated by tools/gen_skill_guide.py.
# HOW: the generator's --check (~1 s); controls edit a rule's text in SKILL.md, edit an
#   anchored incident paragraph, and hand-add a line to a guide.
