# `donovan-m23-stage4` — the M2a stage-4 gate's target

*(CARRIED 14z-170 from `donovan-m19-stage4`: the stage-4 image MOVED by exactly ONE byte, `108f7523` -> `2fa7c2f1` — the reaction hook's case_a4 immediate 0x52 -> 0x38, emitted at stage >= 4 — measured against HEAD's rebuild reproducing `108f7523` (build/rc170/freeze/stage4_delta.txt). The 14 `.masked` specs and the mask are copied verbatim and verified on `build/rc170/freeze/stage4` by the battery's own masked leg (STATE 14z-170). The body below is the carried README; the fingerprints it names are its history.)*
