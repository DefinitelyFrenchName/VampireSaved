...
*(CARRIED 14z-183 from `donovan-m23-stock` at the M20 freeze — GitHub #157, the six hit-pair stores per tenant, and the mark M20. The authored `.masked` specs, the `.skip` markers and the mask are copied verbatim from the predecessor and then frozen/verified on the M20 build (STATE 14z-183). The body below is the carried README; the fingerprints it names are its history.)*

# `donovan-m23-stock` — the M2 battery's stage-6 target

*(CARRIED 14z-170 from `donovan-m19-stock`: the stock twin MOVED at the M19 freeze, `e86e1d04` -> `c54f1fb8`, because the four ruled #136 fixes that are not `only_variant_slot` — the EX-site branch, the column's native 0x52 records, the meter-adder test, the reaction hook's 0x38 marker and the regrown ls_freeze/es_type51 thunks — and the x2b7ef4 resolver reach this track; the defense rows do not. The 14 `.masked` specs and the mask are copied verbatim and verified on `build/m5_stock18` by the battery's own masked leg (STATE 14z-170). The body below is the carried README; the fingerprints it names are its history.)*
