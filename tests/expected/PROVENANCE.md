...
| `facing_rule.tsv` | `audit_facing_rule.sh` | the victim FACING RULE 5 (GitHub #159): the compare chain of each game's facing-rule resolver (vsavj 2,3,4; vs2 2,3,4,5), every write to Demitri's `+0x5D` from 3850 to 3960 on the #136 rig donovan_3 (Killshread Summon (ES)) with its writer PC, and Demitri's x at 3924 and 3946 on both legs — THE DEFECT FROZEN AS MEASURED (ours writes 4 at `0x1886C` and holds at 835; native's rule-5 branch writes 1 / 0 and slides to 755); a fix re-freezes it deliberately | in-emulator (reference + ours) + static — MAME, native `vsav2` and the WIDE build, non-debug read taps and field_trace samples; the decrypted opcode images | `FREEZE=1 tests/audit_facing_rule.sh` | 14z-167 |
