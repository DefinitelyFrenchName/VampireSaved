...
| `vanilla_aerial_slots.tsv` | `test_vanilla_aerial_join.sh` | which anim chain each VANILLA character's jumping normals enter from a NEUTRAL jump (U) and from a FORWARD jump (UR), measured on vsavj — the aerial JOIN: a2 0x12-0x17 is one direction's set and 0x18-0x1D the other's | in-emulator (reference) | `FREEZE=1 tests/test_vanilla_aerial_join.sh` | 14z-145 |
