...
| `move_parity_events.tsv` | `audit_move_parity.sh` | the per-EVENT verdict (IDENT / DIFF with its first frame and fields / VOID / NOT-IN-DF / DF-NOT-ENTERED) of every event of every tenant-move rig part, ours vs native `vsav2`, each event in its own window at a pinned speed level and RNG, both legs real cursor picks, the tenant's fields plus the meter fraction and Victor's HP (GitHub #136; replaced the per-part `move_parity.tsv` at 14z-164 — 27 first-divergence rows that left 103 events unjudged) | in-emulator (reference + ours) (MAME, 60 legs, 30 parts) | `ROMDIR=... ALL=1 FREEZE=1 tests/audit_move_parity.sh` then a verify run | 14z-164 |
...
| `projectile_census.txt` | `test_projectile_census.sh` | which `$FF9400`-pool types each TENANT's moves spawn, replayed on native vs2 | in-emulator (reference) | `FREEZE=1 tests/test_projectile_census.sh` (re-frozen 14z-160 on real cursor picks, GitHub #151) | 14z-120 |
