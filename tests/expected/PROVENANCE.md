...
| `move_parity.tsv` | `audit_move_parity.sh` | the per-part verdict of every tenant-move rig, ours vs native `vsav2`, at a matched speed level with the RNG pinned — IDENTICAL, or the first divergent event | in-emulator (reference + ours) | `FREEZE=1 PARTS="…" tests/audit_move_parity.sh` | 14z-159 |
