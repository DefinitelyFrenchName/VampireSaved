...
| `vanilla_aerial_slots.tsv` | `test_vanilla_aerial_join.sh` | which anim chain each VANILLA character's jumping normals enter from a NEUTRAL jump (U), a FORWARD jump (UR) and a neutral jump then D+button, measured on vsavj — the aerial JOIN: a2 0x12-0x17 is the neutral set and 0x18-0x1D the second set (forward-jump for most characters, Zabel's D+button set) | in-emulator (reference) | `FREEZE=1 tests/test_vanilla_aerial_join.sh` | 14z-145 |
