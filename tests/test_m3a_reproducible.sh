...
MANI_MERGED="fd1f514d48da892fea592d0b775935c25e8244df 25"  # 14z-143 (delta: SIX members — vm3j.03d, vm3j.04d, vm3j.07b and vsw.42 for Pyron's capture row, plus the two glyph tiles for the M17 mark.)  # previous: 14z-132 (delta: THE M16 MARK FREEZE — the in-game mark set to the merged build number, maintainer-ruled option A; GROUP C vsw.33m/37m ONLY, one character of one authored glyph; PROGRAM UNTOUCHED on every track, so all five program fingerprints are unchanged and these rows are whole-set keyed; count unchanged; QSound/Z80 untouched)  # previous: # 14z-127 (delta: THE BOOT-TITLE FREEZE + M13 mark: PROGRAM vm3j.03d (the three `boot_title_saved_*` poke16 at PRG:0x01C822/24/26 — the name-screen display script's JAPAN entry, "S A V I O R" -> "S A V E D"; declared in all three manifests, deduped to ONE shared row); GROUP C vsw.33m/37m (the M13 glyph); count unchanged; QSound/Z80 untouched)
