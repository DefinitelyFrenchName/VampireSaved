...
MANI_WIDE="295bc616b012424dc4820dfe26975aa05037b7d5 25"  # 14z-132 (delta: THE M16 MARK FREEZE — the in-game mark set to the merged build number, maintainer-ruled option A; GROUP C vsw.33m/37m ONLY, one character of one authored glyph; PROGRAM UNTOUCHED on every track, so all five program fingerprints are unchanged and these rows are whole-set keyed; count unchanged; QSound/Z80 untouched)  # previous: # 14z-127 (delta: THE BOOT-TITLE FREEZE + M13 mark: PROGRAM vm3j.03d (the three `boot_title_saved_*` poke16 at PRG:0x01C822/24/26 — the name-screen display script's JAPAN entry, "S A V I O R" -> "S A V E D"; declared in all three manifests, deduped to ONE shared row); GROUP C vsw.33m/37m (the M13 glyph); count unchanged; QSound/Z80 untouched)
...
MANI_HUI="9819b0eceaa2a645450e5be96c4382432cfdc72a 25"  # 14z-132 (delta: THE M16 MARK FREEZE — the in-game mark set to the merged build number, maintainer-ruled option A; GROUP C vsw.33m/37m ONLY, one character of one authored glyph; PROGRAM UNTOUCHED on every track, so all five program fingerprints are unchanged and these rows are whole-set keyed; count unchanged; QSound/Z80 untouched)  # previous: # 14z-127 (delta: THE BOOT-TITLE FREEZE + M13 mark: PROGRAM vm3j.03d (the three `boot_title_saved_*` poke16 at PRG:0x01C822/24/26 — the name-screen display script's JAPAN entry, "S A V I O R" -> "S A V E D"; declared in all three manifests, deduped to ONE shared row); GROUP C vsw.33m/37m (the M13 glyph); count unchanged; QSound/Z80 untouched)
...
MANI_PYR="06982bb03f5d65ab134f63d7b516e7b84acc03fd 25"  # 14z-132 (delta: THE M16 MARK FREEZE — the in-game mark set to the merged build number, maintainer-ruled option A; GROUP C vsw.33m/37m ONLY, one character of one authored glyph; PROGRAM UNTOUCHED on every track, so all five program fingerprints are unchanged and these rows are whole-set keyed; count unchanged; QSound/Z80 untouched)  # previous: # 14z-127 (delta: THE BOOT-TITLE FREEZE + M13 mark: PROGRAM vm3j.03d (the three `boot_title_saved_*` poke16 at PRG:0x01C822/24/26 — the name-screen display script's JAPAN entry, "S A V I O R" -> "S A V E D"; declared in all three manifests, deduped to ONE shared row); GROUP C vsw.33m/37m (the M13 glyph); count unchanged; QSound/Z80 untouched)
...
EXPECT_MERGED="f42f7569a45fbd5311140c0171c5ba8d8010367b"   # 14z-127 merged-m15  # RE-FROZEN 14z-127 (was 6649523a): the boot name screen SAVIOR -> SAVED — three `boot_title_saved_*` aux_poke poke16 rows at PRG:0x01C822/24/26, declared in all three tenant manifests and deduped to one shared engine row on a merged build; plus the M12 -> M13 mark (gfx). Legacy unaffected: audit_merged_legacy 96/0 with 01_attract_long bit-identical, and work RAM identical to pristine over 1,621 frames of boot+attract.  # previous:   # 14z-119 merged-m14 (the physics port: + the same three Donovan value ops, 826 ops, no address moved; M12 mark)
...
MANI_MERGED="528f177c0eca812d8c002b9eb1c0bf22f1731ab0 25"  # 14z-132 (delta: THE M16 MARK FREEZE — the in-game mark set to the merged build number, maintainer-ruled option A; GROUP C vsw.33m/37m ONLY, one character of one authored glyph; PROGRAM UNTOUCHED on every track, so all five program fingerprints are unchanged and these rows are whole-set keyed; count unchanged; QSound/Z80 untouched)  # previous: # 14z-127 (delta: THE BOOT-TITLE FREEZE + M13 mark: PROGRAM vm3j.03d (the three `boot_title_saved_*` poke16 at PRG:0x01C822/24/26 — the name-screen display script's JAPAN entry, "S A V I O R" -> "S A V E D"; declared in all three manifests, deduped to ONE shared row); GROUP C vsw.33m/37m (the M13 glyph); count unchanged; QSound/Z80 untouched)
