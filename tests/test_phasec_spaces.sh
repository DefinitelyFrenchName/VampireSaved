...
EXPECT="${1:-67fdc4de59245fec82f3fceb77c1c09232c695f8}"  # RE-FROZEN 14z-183 (was c54f1fb8, donovan-m23-stock): the M20 freeze moved the stock twin (#157, Donovan's six hit-pair stores; build/rc183/deltas_measurer.txt)  # RE-FROZEN 14z-170 (was e86e1d04, donovan-m19-stock): the M19 freeze moved the stock twin by design — the ruled #136 fixes minus the defense rows (only_variant_slot) and the x2b7ef4 resolver repair — registered donovan-m23-stock; the delta attributed op by op against build/m5_stock17 at the freeze (tools/attribute_patch_delta.py, every EDIT a designed edit kind, 0 unexplained: build/rc170/freeze/rc_final/attr2_m5_stock18.txt, edits_check_m5_stock18.txt; the image checked against its patch: image_vs_patch.txt, image_delta_classes.txt). Missed by the freeze's static pass — a freeze-cadence gate — and caught by the 14z-170 close tier.
WORK="$(mktemp -d)"
