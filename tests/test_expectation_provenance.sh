...
perturb() {  # perturb <name> <out>
    case "$1" in
    deleted-row) grep -v '^| `registry.tsv`' "$PAGE" > "$2" ;;
    dead-row)    { cat "$PAGE"; echo '| `no_such_file.txt` | x | y | in-emulator | z | 14z-1 |'; } > "$2" ;;
    esac
}
# THE EXECUTABLE FORM: under CONTROL=<name> the perturbed copy IS the page
# every section reads — section 1 must FAIL.
if [ -n "$VS_CTL" ]; then perturb "$VS_CTL" "$T/mode.md"; PAGE="$T/mode.md"; fi
...
perturb deleted-row "$T/page_a.md"
...
PY
then
    vs_ctl_dead deleted-row "a deleted row was NOT caught"; rc=1
else
    vs_ctl_fired deleted-row "a deleted row is caught as an unprovenanced file"
fi
# B: a row for a file that does not exist -> caught
perturb dead-row "$T/page_b.md"
...
PY
then
    vs_ctl_dead dead-row "a dead row was NOT caught"; rc=1
else
    vs_ctl_fired dead-row "a row naming a missing file is caught"
fi
