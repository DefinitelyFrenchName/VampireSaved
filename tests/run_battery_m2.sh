...
# tracks are present.
if [ -f "$OUTBASE/rompath/vsavjw.zip" ] && [ -f "$REPO/build/m5_stock7/rompath/vsavj.zip" ]; then
    bat tests/test_wide_render_content.sh "$REPO/build/m5_stock7/rompath" "$OUTBASE/rompath"
else
    echo "note: WIDE rendering gate skipped (not a WIDE build, or no stock twin"
    echo "      at build/m5_stock7 — tests/test_wide_render_content.sh)"
