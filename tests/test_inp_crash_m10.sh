#!/bin/sh
# test_inp_crash_m10.sh — THE #99 NATURAL-PATH CAPTURE, as a gate (14z-111).
# tests/inp/crash_m10 is the maintainer's own MAME session (WIDE_RECORD): 1P
...
BUILD="${BUILD:-build/m3b_merged15}"   # re-point at every merged freeze
MODE="${MODE:-defect}"
W="$(mktemp -d)"; trap 'rm -rf "$W"' EXIT
# stage the tracked recording where run_inp_guarded.sh expects it, isolated
export HOME_INP="$W/inp"; mkdir -p "$W/inp"; cp -R tests/inp/crash_m10 "$W/inp/"
