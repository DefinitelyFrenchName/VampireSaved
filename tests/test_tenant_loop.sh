...
FROZEN_1="donovan:344 huitzil:375 pyron:312"  # RE-FROZEN 14z-170 (was 342/373/312, 2-tenant 618 / sum 677, 3-tenant 831 / sum 946): +2 DONOVAN and +2 HUITZIL = each one's two `[[data_port]]` defense rows (defense_curve_row + defense_rally_threshold, the ruled #136 defense-row fix: vs2's curve row and rally-threshold word over the vsavj variant row's copy). Per-tenant rows at per-tenant addresses, so nothing dedupes; pyron declares none (his rows are identical between the games). The other three 14z-170 fixes are in-blob edits (port_patch, site_thunk bodies, reaction_hook case bytes, region_fix retirement) and add 0 ops.  # RE-FROZEN 14z-143 (pyron 310->312, 3-tenant 829->831 / sum 944->946): +2 PYRON ONLY = his `pyron_capture_keyframes` data_port row — the placed 0xB80 capture-keyframe blob + the one poke32 on capture_kf_ptr row 0x11 (PRG:0x0BE2BE). Declared by pyron.toml alone, so nothing dedupes and donovan/huitzil/the 2-tenant leg are all unchanged. Gate: audit_pyron_capture_block EXPECT_MATCH=1.  # RE-FROZEN 14z-127 (was 339/370/307): +3 EVERY tenant = the three `boot_title_saved_*` aux_poke poke16 rows (the boot name screen SAVIOR -> SAVED, PRG:0x01C822/24/26). They are DECLARED IN ALL THREE manifests and are byte-identical, so `_same_row` dedups them to ONE shared engine row on a merged build: declared +9, emitted +3, deduped +6 — which is what makes them safe to repeat (maintainer-decided 2026-09-02).  # RE-FROZEN 14z-119 (was 336/370/307): +3 DONOVAN ONLY = his param32_a / param32_b / jump_params bank-row value ops now emitted (`port_param32 = true`, maintainer-ruled 2026-08-29 "use VS2 parameters and not the shell character's"; STATE 14z-118 charmap, patch_notes 14z-118 (charmap, 2)); the M12 freeze.  # RE-FROZEN 14z-117 (was 332/366/303): +4 EVERY tenant = the profile-gated random_select_bound + random_select_roster site_thunks (two bodies + two sites; the "?" draw table lists this build's tenants).  # RE-FROZEN 14z-111 (was 327/365/298): +5 every tenant = the #99 AI-script unpark, see the note below.  # RE-FROZEN 14z-105 (was 323/363/296): +2 EVERY tenant = the profile-gated oboro_select_hook site_thunk (body + site); version_text adds 0 ops (it grows the wheel record/coord DATA ops). 14z-102: hui 361->363 = the #109 row-31 fix
...
# RE-FROZEN 14z-105 (was 598/648 and 804/901): the Oboro select hook —
...
check_n "2 tenants" "$WORK/two"   622 681  # RE-FROZEN 14z-170 (was 618 / 677): +2 donovan +2 huitzil, the defense-row data_port rows (see FROZEN_1).  # RE-FROZEN 14z-127 (was 615 / 671): +3 EVERY tenant = the three `boot_title_saved_*` aux_poke poke16 rows (the boot name screen SAVIOR -> SAVED, PRG:0x01C822/24/26). They are DECLARED IN ALL THREE manifests and are byte-identical, so `_same_row` dedups them to ONE shared engine row on a merged build: declared +9, emitted +3, deduped +6 — which is what makes them safe to repeat (maintainer-decided 2026-09-02).   # 14z-119: +3 (Donovan's three physics value-row ops, per-tenant addresses — nothing dedupes), sum +3 (was 612 / 668)  # 14z-117: +4 (random_select_bound + _roster, ENGINE-SITE emitted once), sum +8 — READ FROM THE GATE'S OWN PRINT, see the 14z-117 note
check_n "3 tenants" "$WORK/three" 835 950  # RE-FROZEN 14z-170 (was 831 / 946): +4 = the four defense-row data_port rows (see FROZEN_1).  # RE-FROZEN 14z-143 (pyron 310->312, 3-tenant 829->831 / sum 944->946): +2 PYRON ONLY = his `pyron_capture_keyframes` data_port row — the placed 0xB80 capture-keyframe blob + the one poke32 on capture_kf_ptr row 0x11 (PRG:0x0BE2BE). Declared by pyron.toml alone, so nothing dedupes and donovan/huitzil/the 2-tenant leg are all unchanged. Gate: audit_pyron_capture_block EXPECT_MATCH=1.  # RE-FROZEN 14z-127 (was 826 / 935): +3 EVERY tenant = the three `boot_title_saved_*` aux_poke poke16 rows (the boot name screen SAVIOR -> SAVED, PRG:0x01C822/24/26). They are DECLARED IN ALL THREE manifests and are byte-identical, so `_same_row` dedups them to ONE shared engine row on a merged build: declared +9, emitted +3, deduped +6 — which is what makes them safe to repeat (maintainer-decided 2026-09-02).   # 14z-119: +3 / sum +3 (was 823 / 932), same three ops  # 14z-117: +4 / sum +12 (was 819 / 920)
