...
ROMDIR="$ROMDIR" "$REPO/tools/build_donovan.sh" 2 "$REPO/build/donovan" \
    > "$WORK/build.log" 2>&1 || { tail -15 "$WORK/build.log"; exit 1; }
tail -2 "$WORK/build.log"
RP="$REPO/build/donovan/rompath;$ROMDIR"

WANT_BASE=$(python3 - "$REPO/build/donovan/patch/placements.json" <<'PY'
import json, sys
p = json.load(open(sys.argv[1]))["regions"]["hitbox"]
print(f"{p['dst'] + (0x0C8DF8 - p['src']):08x}")
PY
)
WANT_COMP=$(python3 - "$REPO/build/donovan/patch/placements.json" <<'PY'
import json, sys
p = json.load(open(sys.argv[1]))["regions"]["hitbox"]
