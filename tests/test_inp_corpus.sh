...
#
# Usage: ROMDIR=... [BUILD=build/m3b_merged23] [ONLY=name] tests/test_inp_corpus.sh  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
# Emulator tier (MAME, ~1 min per recording). NOT ci_static.
set -eu
REPO="$(cd "$(dirname "$0")/.." && pwd)"; cd "$REPO"
ROMDIR="${ROMDIR:?set ROMDIR}"
BUILD="${BUILD:-build/m3b_merged23}"   # re-point at every merged freeze  # re-pointed 14z-117b (random-select freeze) <- 14z-117  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
