...
#   rompath_dir default: build/m5_stock16/rompath — the current STOCK twin,
...
RPDIR="${1:-$REPO/build/m5_stock16/rompath}"  # re-pointed 14z-128 <- build/donovan (M2a-era; the emulator-tier sweep ran the bare default and it FAILED there while PASSING on the current stock twin)
