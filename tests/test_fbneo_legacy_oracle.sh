...
BUILD="${1:-build/m3b_merged25}"  # re-pointed 14z-133b: MERGED (B2, maintainer-ruled; expectation set merged-m16)  # re-pointed 14z-117b (random-select freeze) <- 14z-117  # re-pointed 14z-119 (physics-port freeze) <- 14z-117b
[ $# -gt 0 ] && shift
...
for R in $REPLAYS; do
    ...
    case "$R" in
        ...
        # Re-scanned on don_m15 (14z-115; ~25 instants, both legs): 600/2250/2800/3900  # HISTORICAL build name restored 14z-119 (earlier sweeps had rolled it)
