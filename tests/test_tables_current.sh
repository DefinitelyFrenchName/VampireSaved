...
#
# Usage: tests/test_tables_current.sh   [DON=build/don_m23 HUI=build/hui57 PYR=build/pyron42]  # re-pointed 14z-130 (M13 boot-title freeze) <- 14z-119
#
...
cd "$REPO"
DON="${DON:-build/don_m23}"  # re-pointed 14z-130 (M13 boot-title freeze) <- 14z-119
HUI="${HUI:-build/hui57}"  # re-pointed 14z-130 (M13 boot-title freeze) <- 14z-119
PYR="${PYR:-build/pyron42}"  # re-pointed 14z-130 (M13 boot-title freeze) <- 14z-119
fail=0
ok()  { printf '  ok    %s\n' "$1"; }
bad() { printf '  FAIL  %s\n' "$1"; fail=1; }
