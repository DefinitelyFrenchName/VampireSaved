...
| **what a shipped ROM carries but never reaches — and what the port merely RESTORED** (the vanilla characters' VS-style Dark Force handlers in vs2/vh2 — the newcomers' are REACHED by their vs2 EX moves, corrected 14z-168 —, Oboro's dataset in vsavj, the unused Z80 song class) | [`game/preserved_data.md`](game/preserved_data.md) — one entry per measured case: where, why unreachable, how measured, what the port does, what is not known |
...
| **the MiSTer discipline, before touching the core, the lane, the MRAs or a board report** | the three SKILLS — `.claude/skills/mister-jtframe-core/SKILL.md` (level 0, any jtframe core, `[MJC-NN]`, since 14z-134), `.claude/skills/mister-cps2-wide-core/SKILL.md` (level 1, the CPS-2 core, `[MSC-NN]`; its lifted IDs redirect to MJC) and `.claude/skills/mister-vampire-saved/SKILL.md` (level 2, this romset, `[MSV-NN]`) — each rule anchored `**[MSC-NN]**` in the paragraph it distils and locked by `tools/checkskills.py` (`tests/test_checkskills.sh`); the scope is [`project/mister_scope.md`](project/mister_scope.md) |
...
| **how a player who has no Python gets `vsavjw.zip` — the planned browser applier, why a web page and not a per-OS binary, and what must be proven before it ships** | [`project/applier_app_scope.md`](project/applier_app_scope.md) — the maintainer's ask verbatim, the measured feasibility (every capability native: `deflate-raw`, `crypto.subtle` SHA-1, and a JS VCDIFF port already proven on all 20 shipped patches), the slices A1-A6, the rule-7 client-side constraint, and the FIDELITY contract (byte-identical to `apply_release.py`) |
| **how the agent's discipline is to be enforced STRUCTURALLY — orchestrator, checkers it cannot override, narrowly-specified workers (#172)** | [`project/agent_architecture_scope.md`](project/agent_architecture_scope.md) — the failure measured from the session transcripts (idle gaps and what ended them, detached vs tracked launches), what Claude Code's hooks were MEASURED to do, the recommendation (deterministic hook checks first, a context-free procedural checker beside `rulecheck`), the slices S1-S5, and the rulings it needs |
| **how every gate is to be QUALIFIED — proven to measure what it claims and to notice when its inputs move (#171); may a lane's green be CARRIED (`tools/audit_lane_carry.py`); which emulator gates are STALE (`tests/test_emulator_staleness.sh`, `tests/run_all_emulator.sh --stale`)** | [`project/gate_qualification_scope.md`](project/gate_qualification_scope.md) — the four failure shapes of the M19 release tier re-measured against the tree (dead references, empty tables, undeclared inputs, unchecked timeouts, poke read-back), what the classifier, the must-fire contract, the cadence file and the provenance register already cover, the slices Q1-Q6 (a module-reference gate, a MEASURES grammar, `# FOLLOWS:` declarations with a reconciliation control, a staleness gate over the newest emulator run, a headroom check, the read-back census) and the rulings it needs; which gates sample what their rig pokes (`tests/expected/poke_readback.tsv`, the maintainer's classification) |
| **run a freeze or release tier IMMUNE to the working tree, at a named commit, and know afterwards exactly what it tested (#153)** | [`project/snapshot_runs.md`](project/snapshot_runs.md) — `tests/run_on_snapshot.sh`: the clone, the copy-on-write snapshot of `build/`, the submodules and the siblings, the run record, and the immunity gate `tests/test_run_on_snapshot.sh` |
| **describe a gate — write or read its `# WHAT:` / `# HOW:` / `# EXPECTS:` fields, or know what a header may declare** | [`project/gate_header_contract.md`](project/gate_header_contract.md) — the grammar, the one reader `tools/gate_descriptions.py`, the census `tests/test_gate_descriptions.sh`; the rendered view for READING is [`project/gate_coverage.md`](project/gate_coverage.md) (GENERATED, one entry per gate by family; `docs/site/project/gate_coverage.html` once `tools/mk_docs_site.py` has run), the table form the second table of every family in `project/gate_index.md`; `# FOLLOWS:` (the paths an emulator gate's verdict depends on), its reader `tools/gate_follows.py`, the census and reconciliation `tests/test_gate_follows.sh`; `# MEASURES:` / `MEASURED:` (a produced table's floor), its reader `tests/lib/measures.sh`, ground truth `tests/test_measures_contract.sh` |
...
- [`GOTCHAS.md`](GOTCHAS.md) — the index of every trap paid for, grouped
  by bucket, linking to `*/gotchas.md` (one `##` heading per entry, so
  `grep -c '^## '` across the three bucket files is the count — it is not
  quoted here, because HANDOFF dropped exactly this figure for going stale
  by 25 within a few sessions). Citations of `docs/GOTCHAS.md` across the
  repo all still land somewhere useful.
...
- [`gate_coverage.md`](project/gate_coverage.md) — **GENERATED** · WHAT EACH GATE TESTS — every gate's `# WHAT:` / `# HOW:` / `# EXPECTS:` header fields rendered by family for reading; the functional-coverage view (#171 Q0); `python3 tools/gen_gate_coverage.py`
...
- [`must_fire_contract.md`](project/must_fire_contract.md) — **REFERENCE** · THE MUST-FIRE CONTRACT — how a gate DECLARES its controls (`# MUST-FIRE: <shape>: <name> — …` in the leading comment block), FIRES them (`CONTROL FIRED:` / `CONTROL DEAD:` at column 0, read by every runner into plain FAIL) and honours each as an executable MODE (`CONTROL=<name> tests/<gate>.sh` must reach the gate's own FAIL); the runners' readouts and the census's three frozen classes. The tree's copy of BBX's R10/R29/R30, independent but compatible
- [`rule_checker.md`](project/rule_checker.md) — **REFERENCE** · THE RULE-CHECKER — the independent adversarial check at every decision point (GitHub #152): a FRESH agent reads the ARTIFACTS behind a build, a freeze, an expectation or a recommendation and answers five fixed questions with a structured verdict; a VIOLATED verdict stops the action, every run carries a planted known violation, and the ledger `tests/rulecheck/ledger.tsv` is what the maintainer audits. `[VSP-183]`, `[VSP-184]`
...
- [`applier_app_scope.md`](project/applier_app_scope.md) — **REFERENCE** · THE APPLIER APP SCOPE — a static, client-side browser page that turns the player's own dumps into `vsavjw.zip` with no Python and no signed binary: the slices A1-A6 and the byte-identity contract
- [`agent_architecture_scope.md`](project/agent_architecture_scope.md) — **REFERENCE** · THE AGENT ARCHITECTURE SCOPE — #172: bind the working discipline structurally (hooks the agent cannot override, a procedural checker beside the rule-checker, worker definitions): the measurements, the slices S1-S5 and the rulings needed
- [`gate_qualification_scope.md`](project/gate_qualification_scope.md) — **REFERENCE** · THE GATE QUALIFICATION SCOPE — #171: prove every gate measures what it claims and notices when its inputs move: the four failure shapes measured, the slices Q1-Q6 and the rulings needed
- [`snapshot_runs.md`](project/snapshot_runs.md) — **REFERENCE** · SNAPSHOT RUNS — #153: a tier run on a clone at a named commit with every out-of-git input snapshotted by copy-on-write, immune to the working tree while it runs, and the record of what it tested
- [`gate_header_contract.md`](project/gate_header_contract.md) — **REFERENCE** · THE GATE HEADER CONTRACT — what a gate declares about itself in its leading comment block and who reads it: the WHAT/HOW/EXPECTS description, FOLLOWS and MEASURES (all live)
- [`worker_spec.md`](project/worker_spec.md) — **REFERENCE** · THE WORKER SPEC — #172 slice S4: the template every task handed to a `.claude/agents/` worker (`measurer`, `reader`) is written from — TASK as a question, pinned INPUTS, numbered COMMANDS, one `FIG <name> = <value> <- C<n>` line per figure, MUST NOT, STOP — so C1 can read the spec against the return
