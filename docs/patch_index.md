...
| donovan-m2 | in progress (stages 1-3 PASS; stage 4 runs, 1 open delta) | patch_prg tooling, extract_char oracle, reconciliation.toml | replaces Jedah slot 0x0F | M2 proof-of-life: Donovan into vsavj. Generated by `tools/gen_donovan_patch.py --stage N` from the oracle-validated extraction (`tools/build_donovan.sh N`). ~335K of 336.6K free ROM placed; two engine hooks (extended type-dispatch tables); R1 map ~120 verified rows. Gates: `tests/test_m2a_stage{1,2,3}*.sh` PASS. Stage-4 build boots, picks, and plays a live match; open: one anim state-index delta (docs/NEXT_SESSION.md). Per-stage detail: docs/patch_notes.md. |
