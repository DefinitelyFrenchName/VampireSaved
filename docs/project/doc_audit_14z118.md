...
| 13 | the STOCK CONTROL cadence (e) — a ruling, not a measurement | (this commit) | maintainer 2026-08-29: KEEP, run once per NEW `.rbf`, off the per-release checklist. Marked DECIDED in STATE; `mister_field.md` §2 (the `[MSV-22]` paragraph, marker kept) and the NEXT_SESSION opener say RULED; the skill's rule text already stated the cadence and needed no change | `grep -rn 'did not rule\|unruled' docs` = HISTORY blocks only |
| 14 | the whole-corpus non-DF palette-seq census (the `0xAA` question) | (this commit) | 73 vsavj-targeted replays on pristine vsavj, all END-clean, uncapped: union `26 27 2E 6F-72 AE-B1`; `0xAA-0xAD` requested nowhere — **and NOT FREE**: `0xAE-0xB1` came from a CPU Sasquatch (`$381`=1) and the routine table puts the `0xAA` base in his row (`0x02A988`). THE FAMILY RULE read off the routines: 8 ids per owner, `BASE + ($381<<2) + phase`; every 14z-79b census row was one half. `0x2E` = Demitri non-DF. The 14z-118 (9) "no known requester -> candidate free" reasoning retracted in place (the census fallacy). Persisted: `REPLAYS=all` mode + frozen union `tests/expected/palette_seq_ids_corpus.txt`; `ram.md` `+0x381` row [D]; evidence `build/nondf_census_14z118.log` | `grep -n 'no known requester' docs` = the struck text only |
| 15 | `+0x381`'s meaning (the row (14) left "not established") | (this commit) | word-aligned write tap on three replays: written ONCE at init — `0x0058A4` `move.b #$0,$781(a5)`, `0x0058AA` `move.b #$1,$b81(a5)`, `0x009074` — never at pick time. It is the PLAYER-SIDE index; the family rule becomes `BASE + (side<<2) + phase`: 4 ids per side, mirror matches never share a sequence. The "costume index" reading retracted before it ever became a claim. First tap attempt with the odd byte address logged NOTHING (the word-alignment gotcha, `platform/gotchas.md`) | n/a |
| 16 | `$FF8127`'s meaning (row (11) left it open) | (this commit) | write-tapped: writer `PRG:0x02228E`, every match frame — `1 if (P1 obj)+0x10 < (P2 obj)+0x10 else 0`. Not match state; the 14z-104 "P1 downs-won" AND my own row-(11) "1 after a P2-won down" were the comparison caught at a down/refill. Row rewritten [T+D]; the `b27` assertion I had added to `audit_tenant_timeout.sh` (freezing a coincidence) REMOVED the same session. Open sub-question: what object byte `+0x10` is | n/a |
