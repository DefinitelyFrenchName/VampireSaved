...
| `atlas/select_screen.md` | 1007 | measured 14z-60; last tag 14z-118 | 4 | 69 refs; **INTERNAL CONFLICT: header "all 128 navigation transitions measured in MAME" vs L175 "reproduces at best 100 of 128 transitions (78%)"** | **medium-heavy:** resolve the 128-vs-100 pair by re-reading `test_select_wheel.sh` §2 (which asserts what?); L933 "folds into the pending re-freeze" (which freeze? done?) |
...
| `patch_index.md` | 340 | [VSP-99] same-commit; tag 14z-118 | LIVE | 6 | fingerprints per generation | **L237 "believed unused" (type-6 drawer) is the file's one GUESSED claim — and the DEADNESS REGISTER already measured it FALSE (14z-89)**: retract in place |
...
| `tables/README.md` + `tables/*` | — | README: "Empty until a ported character exists" | LIVE, **stale by its own words** | — | — | medium: README rewritten; `donovan.md` (2026-08-09, never updated) has NO Huitzil/Pyron siblings though the README promises them — [VSP-6] says the tunables live here: either generate the two missing manifests from the same source as `donovan.md` or retract the promise |
