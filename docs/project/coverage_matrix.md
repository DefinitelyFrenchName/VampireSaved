...
| tenant moves vs native vsav2 (normals, specials, ES/EX, supers, DF attacks; meter, live hitboxes, movement) | **GAP — #136 (census 14z-155)** | Throws and captures are the only move class swept against native (`audit_capture_matrix.sh`, 640 cells). Move batteries against native vsav2 exist for Donovan (`test_m2a_stage4_oracle.sh`, default build the STOCK twin) and Phobos (`test_hui_oracle.sh`, default build the SOLO freeze): match-start anchors, neutral-window fields, the HP-loss sequence and a divergence bound — not per-move data, and neither on the merged WIDE build. None for Pyron. Single moves: `test_don_immortal_native.sh` (421+P hit count, damage and hit frames, speed level and RNG pinned), `audit_fg_parity.sh`, `audit_projectile_clash.sh`. Comparable across the two games once the speed level and the RNG are pinned on both legs ([VSE-84]): hit counts, damage, meter, reaction and projectile classes, and durations in engine ticks and in video frames; unpinned, no video-frame duration is. |
