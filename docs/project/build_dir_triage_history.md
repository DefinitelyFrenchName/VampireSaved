# BUILD-DIR TRIAGE — HISTORY (blocks moved verbatim from `build_dir_triage.md`)

Moved at 14z-122 by the documentation rationalization pass. Historical
entries are not rewritten (CLAUDE.md §5 [VSP-13] step 4); corrections live
in `build_dir_triage.md`. This file holds, verbatim: the 14z-101 decision
package with its A1-C inventory and the 14z-102 ruling/execution, the
14z-103 A4 pin-cleanup record, and the superseded per-freeze sweep records
(14z-115, 14z-117, 14z-117b — the LATEST record stays live in the
reference file). The dir counts and classifications are AS OF their day;
the live policy and current inventory are in `build_dir_triage.md`.
No `**[PFX-N]**` anchor lives in this file.

*(moved from `build_dir_triage.md` at 14z-122)*
