...
| **H10 the skills lock** (14z-139, §9.3) | `tools/checkskills.py` + `tools/gen_skill_guide.py` → `bbh check-skills` / `bbh skill-guide`, table-driven from `[skills]` + one `[skill_<PFX>]` table per skill (the TOML subset has no arrays of tables); the three hard-coded lineage names as config (the archive exemptions, the guide header's lines with `{title}` `{name}` `{origin}`) | `selftest/test_skills.sh` (ten must-fires on a synthetic consumer + the harness's own lock) | **LANDED 14z-139**, harness `185158e`: F11 EXACT — `bbh check-skills -v` with the lineage's GENERATED `[skills]` section == `tools/checkskills.py -v` over the eight skills (18 lines, 553 rules), `bbh skill-guide --check` finds both committed guides CURRENT |
| **THE SKILL** (§9) | `skill/blackbox-harness/{SKILL.md, GUIDE.md}` — `[BBH-1..87]`, eight sections, level 0 plus the lineage tokens and every other skill's bracket IDs forbidden; 87 anchors across nine harness pages (`docs/doctrine.md` written in the pass as the doctrine rules' home; three paragraphs split so each carries one rule); `skill/skills.toml` its lock; the guide GENERATED | `test_skills` §5 (the real lock and the guide's currency) | **LANDED 14z-139**, harness `e643219`: 87 rules against the ~75-80 estimate; installed as the symlink `~/.claude/skills/blackbox-harness` |
...
| **F11 the skills lock and the guide generator (added 14z-139, H10)** | this tree's eight skills and two committed guides; the `[skills]` section of `bbh.vampire.toml` GENERATED from `tools/checkskills.py`'s table | `tools/checkskills.py -v`, `tools/gen_skill_guide.py --check` | `bbh check-skills --config … -v`, `bbh skill-guide --config … --check` — stdout + exit identical; both guides CURRENT byte for byte | nothing |
...
## 9. THE SKILL — scope (the plan before the work), and its execution

**STATUS: ALL SEVEN DECISIONS RULED AND THE SKILL LANDED THE SAME DAY
(14z-139, 2026-09-07) — §9.6 executed in order: the staleness pass
(harness `e5bc3a5`), H10 the lifted lock (`185158e`), the skill `e643219`; 87 rules against the ~75-80 estimated;
`~/.claude/skills/blackbox-harness` a symlink into the harness. The rest of
this section is the plan as ruled.**
