...
| emulator superset invariant | `tests/test_mister_sim_anchor.sh` runs the CPS-2 WIDE core `cps2w` on **stock unmodified `vsavj`** against the expectations measured on the REFERENCE core (MAME 2146 / sim 2502 / skew 356 ± 30). The expectations are never re-measured on `cps2w`; reproducing the reference core's numbers is the whole assertion. Plus, at the image layer, the stock `vsavj.rom` from `cps2w` is bit-identical to the reference one and its profile byte is the 0xFF fill (`tests/test_mister_mra_map.sh`) |
