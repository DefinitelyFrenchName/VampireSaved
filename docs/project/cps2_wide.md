...
|---|---|
| bounded and declarative | a file is copied into `cores/cps2w/hdl` **only when it must differ**; the set is ENUMERATED (`tests/test_jtcores_twin.sh` 2a) and the diff of each override against the shared original is FROZEN line by line (`tests/expect/cps2w_rtl_delta.txt`, `tests/test_mister_wide_gate.sh` 1) |
...
| ratified per version | **per SLICE**, `docs/project/mister_map.md` §10: each slice carries its own smallest-thing-that-proves-it, its own must-fire control, and re-runs the superset leg. D0 (the MRA trim) and D1 (the QSound sample-bank width) are done; D2-D4 are not |

**The gated sites, as of D1 — one:** the QSound sample-bank latch
(`cores/cps2w/hdl/jtcps2w_qsnd_bank.v`), `wide_en ? dsp_ab[7:0] :
{1'b0, dsp_ab[6:0]}`. With `wide_en` low it is bit-for-bit the stock
expression, reset value included, and that is proven exhaustively over all
65,536 values of `dsp_ab` rather than argued. D2-D4 add the group-C GFX
redirect, the object tile-code promote (the RTL twin of the `Cps2ObjDraw`
19-bit promote above) and the 68k PRG window; each one lands on the same
