...
|---|---|
| bounded and declarative | a file is copied into `cores/cps2w/hdl` **only when it must differ**; the set is ENUMERATED (`tests/test_jtcores_twin.sh` 2a — four files at D1, six at D2, **TWELVE since D3+D4**) and the diff of each override against the shared original is FROZEN line by line (`tests/expect/cps2w_rtl_delta.txt`, `tests/test_mister_wide_gate.sh` 1). **D2 also puts a file OUTSIDE `cores/` for the first time** — `modules/jtframe/hdl/sdram/jtframe_ram1_7slots.v`, a mechanical member of a jtframe family that stops at 5 — so the enumeration was widened to the fork's WHOLE-TREE `git diff --name-status`, held to 24 declared paths (`test_jtcores_twin` 2f). The new module is pulled by `cores/cps2w`'s own `game.yaml` and NOT by jtframe's shared `jtframe_sdram64.yaml`, so no other core's compile list moves (`test_mister_wide_gate` 5b/7n) |
...
| ratified per version | **per SLICE**, `docs/project/mister_map.md` §10: each slice carries its own smallest-thing-that-proves-it, its own must-fire control, and re-runs the superset leg. **D0-D4 are ALL DONE (14z-107 (10)):** the MRA trim, the QSound sample-bank width, the SDRAM placement, the CPS-2 Turbo object promote and the 6 MB program window |

**The gated sites, as of D4 — EIGHT.** `tests/test_mister_wide_gate.sh`
re-reads each one verbatim on every run (7i-7l, 8a-8h):
