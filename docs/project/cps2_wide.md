...
|---|---|
| bounded and declarative | a file is copied into `cores/cps2w/hdl` **only when it must differ**; the set is ENUMERATED (`tests/test_jtcores_twin.sh` 2a — four files at D1, SIX since D2) and the diff of each override against the shared original is FROZEN line by line (`tests/expect/cps2w_rtl_delta.txt`, `tests/test_mister_wide_gate.sh` 1). **D2 also puts a file OUTSIDE `cores/` for the first time** — `modules/jtframe/hdl/sdram/jtframe_ram1_7slots.v`, a mechanical member of a jtframe family that stops at 5 — so the enumeration was widened to the fork's WHOLE-TREE `git diff --name-status`, held to 18 declared paths (`test_jtcores_twin` 2f). The new module is pulled by `cores/cps2w`'s own `game.yaml` and NOT by jtframe's shared `jtframe_sdram64.yaml`, so no other core's compile list moves (`test_mister_wide_gate` 5b/7n) |
...
| ratified per version | **per SLICE**, `docs/project/mister_map.md` §10: each slice carries its own smallest-thing-that-proves-it, its own must-fire control, and re-runs the superset leg. D0 (the MRA trim), D1 (the QSound sample-bank width) and **D2 (the SDRAM placement)** are done; D3 (the obj promote) and D4 (the PRG window) are not |

**The gated sites, as of D2 — five.** D1 added the first; D2 added four more,
all of them the same shape (`wide_en & <one address bit>`), and
`tests/test_mister_wide_gate.sh` 7i-7l re-reads each conjunction verbatim on
every run:

| site | expression | file |
|---|---|---|
| QSound sample-bank latch (D1) | `wide_en ? dsp_ab[7:0] : {1'b0, dsp_ab[6:0]}` | `jtcps2w_qsnd_bank.v` |
| QSound download split (D2) | `is_pcmhi = wide_en & pcm_addr[23]` | `jtcps1_prom_we.v` |
| group-C GFX download redirect (D2) | `is_gfxc = wide_en & gfx_addr[25]` | `jtcps1_prom_we.v` |
| QSound read split (D2) | `pcmh_sel = wide_en & pcm_addr[PCM_AW]` | `jtcps1_sdram.v` |
| group-C obj read select (D2) | `gfxc_sel = wide_en & rom0_bank[2]` | `jtcps1_sdram.v` |

With `wide_en` low every one of them is constant 0 and the surrounding
expression is bit-for-bit the stock core's; the D1 latch is additionally
proven exhaustively over all 65,536 values of `dsp_ab` rather than argued.
...
D3 and D4 add the object tile-code promote (the RTL twin of the `Cps2ObjDraw`
19-bit promote above) and the 68k PRG window; each lands on the same
`wide_en` and under the same clauses.
