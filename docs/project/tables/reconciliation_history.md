# R1 reconciliation — HISTORY (blocks moved verbatim from `reconciliation.md`)

Moved at 14z-122 by the documentation rationalization pass (the pass's
SPECIMEN document). Rules: historical entries are not rewritten (CLAUDE.md
§5 [VSP-13] step 4); a correction is made where the live claim lives, in
`reconciliation.md`, and marked here only if it was already marked when the
block moved. Blocks are in their original order (oldest sessions carry the
project's early frontier language). No `**[PFX-N]**` anchor lives in this
file.

Known-superseded claims INSIDE these blocks, corrected in the live file:
"RESTORE AT M5" on the sound stubs (M5 shipped at 14z-86 through authored
voice ids and the alias thunk — the per-row `note` fields of
`build/manifest/reconciliation.toml` carry each row's actual fate);
"18/15 tripwired targets remain" (a point-in-time count — the live count is
measured from the toml, 40 open rows at 14z-122).

*(moved from `reconciliation.md` :71-658 at commit 55eb42b's successor)*
