...
| `../charpages/framedata/<tenant>.html` (OUT of the tree) | the CHARACTER PAGE (14z-121 (5)): a wiki-style rendering — physics, every move of the move list with frame data, damage, meter, class, pushback, the move's own hit/hurt/push box diagram and the maintainer's notes, projectiles, reactions as the victim, provenance. **Per-move frame data is not published in this repository** (maintainer-ruled 2026-08-31, STATE 14z-126): the page is regenerated from the romsets and its currency is locked by SHA-256 in `tests/expected/charmap_pages.sha256` (`tests/test_charmap_current.sh`). Published as PRIVATE artifacts from that output: Donovan https://claude.ai/code/artifact/85d7fd52-9b14-4b19-b3a1-d76334f2cb3e · Huitzil https://claude.ai/code/artifact/f0dddc83-5b9b-4139-a637-91c55695fdf7 · Pyron https://claude.ai/code/artifact/ad618f12-5166-4a88-94e8-d89625a3500e | `ROMDIR=... tools/framedata_pages.sh` |
| `../charpages/framedata/<tenant>_anim.md` (OUT of the tree) | the ANIM-CHAIN APPENDIX: one block per chain, one line per node, with the derived startup / active / recovery per chain. Same rule and same hash lock as the page above | `ROMDIR=... tools/framedata_pages.sh` |
...
| file | what | regenerate |
|---|---|---|
| `community_crosscheck.md` | **GENERATED — the VERDICT-ONLY page** (per-move numbers, ours and the workbook's, are on the out-of-tree full page below). Snapshot published as a PRIVATE artifact (maintainer-ruled 2026-08-31: internal only, like the character pages that carry IP): https://claude.ai/code/artifact/f572a468-2e35-441c-aa1a-130353e9c9ff — a curated summary, NOT the exhaustive page; re-publish it from the same file path when the findings move. Our DERIVED frame data for all 15 vanilla characters against the maintainer's community workbook, classified by the ruled vocabulary (EXACT / CONSTANT OFFSET / CONSTANT RATIO / INCONSISTENT / UNCOMPARABLE) | `python3 tools/crosscheck_framedata.py --md docs/project/tables/community_crosscheck.md --tsv tests/expected/community_crosscheck.txt` |
| `../../../tests/expected/vanilla_normal_slots.tsv` | the MEASURED slot map: which anim chain each character's standing normals enter, at two distances (180 rows) — chain ids, not frame data, so it stays in the tree | `ROMDIR=... FREEZE=1 tests/test_vanilla_frame_join.sh` |
| `../charpages/framedata/community_crosscheck_full.md` (OUT of the tree) | the FULL move-by-move comparison — every INCONSISTENT column's moves, the workbook's values beside ours | `ROMDIR=... tools/framedata_pages.sh` (needs the workbook) |
| `../charpages/framedata/vanilla_hit_damage.tsv` (OUT of the tree) | the MEASURED dealt damage and hit counts (90 rows); only its SHA-256 is frozen, in `tests/expected/vanilla_hit_damage.sha256` | `ROMDIR=... FREEZE=1 tests/test_vanilla_frame_join.sh` |
