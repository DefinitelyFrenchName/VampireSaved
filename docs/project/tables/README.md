...
| `build/charpages/<tenant>_internal.html` (UNTRACKED) | the INTERNAL character page WITH SPRITES (14z-121 (6)): the same page with the character's own sprite at each chain's first active frame beside the box diagram — captured from the native game's OBJ list and palette page at the frame the naming rigs reach it (`tests/lua/sprite_capture.lua`), drawn from `$ROMDIR/vsav2.zip`'s tiles (`tools/sprite_render.py`). Rendered art stays out of the tree and off the artifacts by the maintainer's word | `ROMDIR=... tools/charpages_internal.sh` (~15 min, 48 MAME legs) |
