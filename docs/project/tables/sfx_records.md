...
| | **0x198 0x199** | measured SHARED 14z-85: identical keyon signatures both sets — (0,20480,12548), the same sample family as don's kept 0x119. **CAVEAT 14z-95 (GitHub #93): the keyon signature (a REGISTER triple) is identical; the sample CONTENT is not — the inclusive endpoint 0x6C5000 differs (vsav 0xFF, vsav2 0x00), the byte packing law #3 says is played** |
...
| zeroed: voice block | 0x735-0x74E (16 ids) | his vs2 voice/sfx bank; vsavj keys OTHER content across that gap (3-8 keyons each) — unfaithful. **14z-85g: nodes 10/11 (0x739 trap spawn-eject / 0x73A trap timer-detonation) were the measured trap-sound parity gap. 0x73A RESTORED SAME SESSION without any sample port — its content is byte-identical in vsav's own QSound image (0x6C0000; vsavj ids 0x198/0x199 family) [CORRECTED 14z-95, GitHub #93: for 20,480 of 20,481 bytes — the inclusive endpoint 0x6C5000, the played byte, differs: vsav 0xFF, vsav2 0x00]: the detonation fires via the sound-farm stub vs2 0x4F2E (NOT this record — the record's node 11 is remapped 0x73A→0x199 as content-faithful defense in depth), restored by the kind=sound_stub recon row. 0x739 (ejection) REMAINS the M5 target: no vsavj equivalent measured; it keyed nothing in the 12-frame sweep (delayed attack — use the 45-frame re-probe when porting). Gate: audit_trap_parity.** |
