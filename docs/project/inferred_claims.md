...
| 5 | slot 0x0B "(Shadow/Marionette machinery?)" guess marked UNCONFIRMED in the table row while the 14z-116 decode sits directly below it | `docs/game/atlas/character_tables.md:284` (vs :295-300) | STALE-CARRIER | none — the same file already carries the measured answer ("the ? RANDOM cell", every mechanism decoded); the row cell cites it | — | T0 | OPEN |
| 6 | "copying a TENANT is structurally expected to work — and it has never been run" | `docs/game/atlas/select_screen.md:401` | STALE-CARRIER | none — it WAS run 14z-116: `tests/test_shadow_tenant.sh` (Shadow beats tenant Donovan, becomes id 0x13 with Donovan's OWN record `0x003FA9D0`, guard-clean END 21120), CRT-confirmed 2026-08-28. The sentence contradicts a green gate | `test_shadow_tenant.sh` | T0 | OPEN |
