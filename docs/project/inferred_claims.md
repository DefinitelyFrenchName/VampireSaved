...
> or ruled-parked (row 11 alone — **NOW MEASURED AND CLOSED 2026-09-01, so
> the pass has no parked rows left**; the maintainer's camera evidence on #113 —
...
|---|---|---|---|---|---|---|---|
| 1 | "+0x161 accumulator … Live only for an Aulba[th victim]" — the whole +0x15B/+0x15E/+0x161/+0x162 family: static decode, arming path never observed live ("+0x15E = 0 on every frame of every naming and victim rig") | `docs/game/atlas/ram.md:146-148`; `docs/game/engine_internals.md:898` | DERIVED-UNMEASURED | an Aulbath-VICTIM rig with Dark Force ACTIVE (poke `$FF8509`, assert `$FF802E`=1 per [VSP-123]); write-tap `+0x15E/+0x161/+0x162` on the victim through contacts; freeze the accumulate/threshold shape | `tests/replays/df/97_df_mech.rpl` family + `audit_df_framework.sh` DF-entry mechanics; `trace_writes.lua` | T3 | **MEASURED 14z-123 — mechanism CONFIRMED, character RETRACTED: it is SASQUATCH's Dark Force armor (`dispatch_16` row 0x0A, `PRG:0x047E60`; LP+LK/MP+MK arm 0x200, HP+HK never arms); cr.LP/MP/HP add 20/30/40, break past 60, decay 240; Aulbath's DF arms 0x7FFF full armor instead. `tests/audit_df_accumulator.sh` + 9 frozen lines; carriers corrected: engine_internals readers cell, ram.md ×2, STATE 14z-121 (3) marked** |
...
| 4 | "$FF8127 … a per-frame COMPARATOR of the two fighters' object byte +0x10 … Open: what object byte +0x10 is" (14z-118 (16) leftover) | `docs/game/atlas/ram.md:102` | DERIVED-UNMEASURED | write-tap object `+0x10` on both fighters across one match (spawn → contact → down) and name the byte from its writers | `trace_writes.lua` / `read_tap.lua` on any 2P replay | T2 | **MEASURED 14z-123 — REFINED: `+0x10` is a byte of the fighter's ANIM NODE (not the object), a per-pose draw-order key; `$FF8127` = who draws in FRONT; identity exact on every non-capture frame of replay 37. `tests/audit_front_comparator.sh`; ram.md row rewritten** |
...
|---|---|---|---|---|---|---|---|
| 7 | attract-palette writers "(vsavj 0xB0AC attract path, table 0x3A3CA0 …) not yet repointed — if the attract demo shows wrong Donovan colors, that is the mechanism" + "UNVERIFIED … nobody has re-measured it since M2b"; bounded DERIVED 14z-118 (9) ("a tenant-vs-tenant VS screen in 1P would show the placeholder ramp … Not measured on screen") | `docs/game/engine_internals.md:262-276,303` | DERIVED-UNMEASURED | reach the one reachable surface: 1P arcade AS a tenant until the CPU draws a tenant (ladder rows 16/17/19), snapshot the VS screen, compare the ramp at `0x3A3CA0 + id*32` vs what renders | `snapshot_frames.lua` + the venue byte `$FF8121` steer (`audit_don_vs_cpu.sh` mechanics) | T3 | **MEASURED 14z-123 — RETRACTED on both counts: `PRG:0x00B094` is the 1P opponent-ROULETTE screen's palette load (once per ladder match, `$FF8008 == 0x0008`), not attract; the VS screen never reads it (1P-vs-CPU-Phobos VS screen pixel-identical to the 2P path, 0 px). REFINED: the roulette TAG shows the base character's name/mini-art for a tenant opponent ("BULLETA" for Phobos, pool row `0x10`'s brown ramp) — cosmetic, backlog. Gate `tests/test_ladder_tenant_vs_palette.sh` + replay 111; carriers corrected: engine_internals, id_space ×2** |
...
| 11 | white-frame mechanism "(palette RAM blanked vs a CPS-B layer/priority register) is NOT measured; only the framebuffer is" | `docs/game/engine_internals.md:2887` | TESTIMONY/PARKED | NONE THIS PASS — #113 is maintainer-parked pending their camera evidence ("do not re-derive"); the sentence is an honest boundary and stays as "What is NOT known" | `test_down_flash_vanilla.sh` (the framebuffer half) | T4 | RULED-PARKED (maintainer, 2026-08-28) → **UNBLOCKED 2026-09-01: #113 CLOSED as vanilla (board agrees), so the park reason is gone. The claim is UNCHANGED and still correct — the mechanism was never measured. **CLOSED 2026-09-01 — MEASURED, and the claim is now FALSE as written.** The mechanism is a PALETTE-BASE SWAP: CPS-A `0x80410a` (`CPS1_PALETTE_BASE`) goes `0x90c0` -> `0x9240` for one frame and `0x924000` is all `ffff`. So it is neither "palette RAM blanked" NOR "a CPS-B layer/priority register" — both named candidates are retracted; palette CONTENTS are untouched. 4/4 discriminator, FBNeo-confirmed. See STATE's #113 entry and engine_internals "the one-frame white-out"** |
...
| 13 | "H/P anim movability is inferred from the manifests, not measured — a 'runs' verdict for them needs a liveness control first" | `HANDOFF.md:1595-1598` (= `audit_region_movability.sh` header) | DERIVED-UNMEASURED | run the audit's H/P legs with the liveness control its own header specifies (prove the moved region EXECUTES on the leg, then the runs/crash verdict) | `tests/audit_region_movability.sh` | T2 | **MEASURED 14z-123: H/P anim run from `wide_ext` WITH the liveness control (`+0x60.l` == the build's own table row; `+0x1C` inside the moved range at three in-match frames) — new `case_tenant`, `CASES=` selects; `test_movability_liveness.sh` re-scoped to the four Donovan cases. RETRACTED as superseded** |
| 14 | "Merged+legacy+AUTO is UNMEASURED (its one attempt mashed past the KO — void)" | `HANDOFF.md:2078-2082` (= `audit_win_pal_auto.sh` header) | DERIVED-UNMEASURED | re-run the audit with a merged+LEGACY+AUTO leg whose inputs END AT THE KO (the documented rig rule); freeze COLORED | `tests/audit_win_pal_auto.sh` + replay 103/104 pattern | T2 | **MEASURED 14z-123: leg E (merged + LEGACY AUTO winner, replay `105_legacy_2pwin_auto` = 103 with P1 on the default cell, inputs ended at the KO) = COLORED, liveness asserted (Demitri's base, P2 white HP < 0). The #105 defect never touched the legacy path. RETRACTED** |
| 15 | `test_pyron_blink` native-leg guard reads `+0x382` in match — "a borrow there yields a false REFUSE … Fix = gate on `+0x60.l`, blocked on freezing our tenant hitbox bases" — the blocker is STALE: the bases ARE frozen (`tests/expected/.../bases.tsv`, re-derived every freeze since 14z-100) | `HANDOFF.md:2615-2622`; GitHub #16 | STALE-CARRIER (blocker) + T2 fix | switch the guard to the `+0x60.l` hitbox-base signature (the `audit_legacy_pairings` signature) and re-run the gate once | `tests/test_pyron_blink.sh`; `bases.tsv` | T2 | **REFINED 14z-123: the guard had ALREADY moved to `+0x60.l` at 14z-92 (`5d62153`, #16 FIXED) — the HANDOFF paragraph was a stale carrier; what remained weak (one non-zero base passed a WRONG loaded character) is hardened: `--expect-base` derived at run time from each leg's own hitbox_base table, plus a `wrongchar` refusal control** |
...
- 14z-123 (G2 + T2, eight measurements): rows 1, 4, 7, 8, 9, 13, 14, 15
  MEASURED — three RETRACTED (1: Sasquatch's DF armor, not Aulbath's; 7:
  the roulette tag, not the VS screen; 8: the advancing guard, not a throw
  escape), three REFINED (4, 15, 13), two CONFIRMED (9, 14). Each landed
  with its carriers corrected and a frozen gate; the retraction greps are
...
- 14z-124 (G3 (c), the last document commit): engine_internals :2944-end →
  `engine_internals_history.md` (19 blocks); row 10 CLOSED; the document
  flipped to REFERENCE (banner + atlas-rows), `checkdocshape --no-pending`
  green. Retraction grep in the commit body. REMAINING: row 11 ruled-parked
  (the maintainer's camera evidence) — the inventory is otherwise closed.
