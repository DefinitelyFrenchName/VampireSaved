...
Fourteen entries that are EMULATOR or TOOLCHAIN facts — MAME/FBNeo packaging and
audit behaviour, cross-emulator replay content, the video-blind harnesses and
their canaries, `git apply`/`git submodule` in a foreign tree, the `:IN2` EEPROM
line, `WIDE=0` not reverting, probe PCs off instruction boundaries, palette-RAM
pokes, the `-debug` timeline grammar — were filed here by TASK (the trap
`docs/README.md` rule 1 names; flagged in `skills_scope.md` §4 row B). They now
live in `docs/platform/gotchas.md` under the banner "RE-FILED FROM project/gotchas.md
(14z-118)", verbatim, with their `[CPE-*]` anchors. The GOTCHAS index points there.
