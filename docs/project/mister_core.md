...
| **A tenant sprite FETCHED on the core** | **HAPPENED 14z-107 (11)** — the wheel half. With D5 in, the read probe counts **9,038,400 reads over 105 distinct tile codes `0x74D6-0xFE41`** in group-C obj **bank 5** (the select-wheel art), first at simulated frame 1556, every code inside the roster's frozen live extent `0xFFDB`; the control leg reads zero. **Obj bank 4 — the FIGHTER art — is still ZERO and `test_mister_gfxc_fetch` is still RED for it, and the reason is the HARNESS, not the RTL.** The replay was written and run: `36_pick_tenant_cell` on `cps2w` + the WIDE romset, into a match. It fetched nothing from bank 4 because **the simulator's direction bits are REVERSED end for end** (row below; measured in full and fixed 14z-108) — the cursor moved on every press, just not where asked, landed on Victor, and the core drew the legacy character it was handed. The zero is a true reading of a run that never selected a tenant. |
...
| **Bank 1's group-C half under load** | **STILL UNMEASURED, and the 14z-107 (12) run does not bound it.** The run above picks Demitri, so obj bank 4 is never fetched and ba1's 13,890 accesses/frame are PCM alone — identical to stock. The repack risk this instrument was built for, obj fetches interleaving with the PCM stream *inside bank 1*, needs a tenant in a match, which needs the direction-bit defect in the row below fixed first. |
| **A tenant SELECTED on the core** | **WAS BLOCKED BY THE HARNESS; THE BLOCK IS REMOVED (measured in full and fixed 14z-108).** The simulator's direction bits were REVERSED end for end, not transposed in two: measured on the game's own input mirror `RAM:$FF8058.w` over four single-direction presses (`tests/replays/107_four_directions.rpl`, stock `vsavj`, MAME vs `cps2w`, both dump sets integrity-checked), Up arrived as `0x0001` (Right), Down as `0x0002` (Left), Left as `0x0004` (Down) and Right as `0x0008` (Up), where MAME reads the requested bit every time. 14z-107 (12) saw only the Left/Down half and inferred a two-bit swap, which would have fixed half of it. The bits ARRIVE and are PERMUTED, not lost (12 of 811 dumped frames carry a direction, the expected count). **Up and Right are NOT exercised by those samples and that half is UNTESTED** — measure all four before changing a bit map. Likely cause, stated as inference from two data points: `tools/rpl2siminputs.py` emits U/D/L/R per its own docstring where the harness appears to consume U/L/D/R. `docs/platform/gotchas.md`, and `docs/platform/mister.md` "THE SIMULATED JOYSTICK'S DIRECTIONS ARE TRANSPOSED". **This is the fourth input-path defect in this lane, and the seventh instrument defect overall** — three of the four were invisible until a replay first needed the feature. |
| **A tenant FIGHTING on the core** | never, and it is the next thing worth having. It needs the row above fixed, then one run of `test_mister_gfxc_fetch`'s fighter half. Everything below it in the stack is proven: the promote (D3), the program window (D4), the decryption range (D5), the placement (D2) and the download (D0). |
...
| **Video compared against MAME** | never — **and the two committed select-screen images three rows up do not change this.** They were looked at, not compared: no golden, no checker, no gate. Every cross-implementation verdict to date is work-RAM fields at a sync anchor; no frame has been compared, so "the picture looks right" has never been a checked claim. |
