...
`vampire-saved`, pinned at `emu/jtcores` = **`2bf41090`** (upstream tag
`v1.7.3` = `63688ce5`, plus **twenty-eight** fork commits, mirrored as
`emu/jtcores-patches/0001-0028`; the pin is read from
`tools/setup_jtcores.sh`, not from this sentence — **UPDATED 14z-119** (the
catalogue commit for merged-m14); 14z-117b had it at `f997cfe1` + twenty-seven, 14z-117 at `80e08111` + twenty-six, 14z-115 at `202fc3e6` + twenty-five, 14z-113 at `63496069` + twenty-four,
and before that `dd242a65` + fifteen since 14z-107 (10)). The romset is the
merged build of the current freeze — **merged-m14 (M12), `build/m3b_merged21`,
program fingerprint `6649523a`** (14z-119, Donovan's physics rows carry VS2's
values; merged-m13 = `build/m3b_merged20`, `a1b7cb82`, M11, was the previous). The measurements quoted in this document were
...
sizes or tile extents. Two download images anchor the D0-D5 measurements:
stock `vsavj.rom` **46,407,744 B** sha1 `f9dc2987…`, and the merged-m6 WIDE
`vsavjw.rom` **66,265,152 B** sha1 `d462e55a…` (a later freeze's image has
the same size and header words and a different sha1 — `test_mister_mra_map`
...
`release/<name>/`, plus a stock-`vsav` reference-leg MRA. **Both halves
have since happened (updated 14z-113):** the field test PASSED on a
DE10-Nano at 14z-109 and again on bundle 14z112 (2026-08-28, no regression,
...
**[MSC-1]** **[MJC-1]** **The separate-core mechanism is jtcores' own, not something we invented.**
...
> **[MSV-3]** **Two counts of "group C live codes" are in circulation, and they are
...
> **45,736 + 6,272 = 52,008 codes / 6.349 MB** (14z-117: the M10 mark's third glyph adds ONE live code to obj bank 5, extent `0xFE41` -> `0xFE42`; 14z-115 had 45,736 + 6,271 = 52,007 — the select-wheel outline sprites added 26; it was 45,736 + 6,245 = 51,981 / 6.345 MB through 14z-113) from a *non-blank census* of
...
> **[MSC-7]** **[MJC-7]** **A retraction worth carrying, because it is the ordinary lesson in a new
...
|---|---|---|---|---|
| **live bytes** — art that exists | 45,736 codes | 6,272 codes (6,271 until 14z-117, 6,245 until 14z-115) | **6.349 MB** (6.345 until 14z-115; write set: 6.39 MB) | the ROMSET. Nothing else |
...
kind of decision that ruins frame timing. **[MSC-24]** **[MJC-24]** **It was measured before it was
...
**[MSC-27]** Widening SDRAM does not widen the core. **Four caps live in the CPS-2 RTL
...
**[MSC-34]** **THE FIFTH CAP IS THE ONE NOBODY PREDICTED, AND IT IS WORTH THE PARAGRAPH.**
...
**[MSC-33]** **[MJC-33]** **Nine sites are gated as of slice D5**, and `tests/test_mister_wide_gate.sh`
...
**[MSC-14]** **[MJC-14]** **The obj promote is gated at BOTH ends and that is deliberate.** `gfxc_sel`
...
**[MSC-15]** **[MJC-15]** **The one ungated change is declared rather than hidden: the bank-0
...
**[MSC-13]** **[MJC-13]** **A second profile gate operates one layer up, in the mapping tool.**
...
**[MSV-30]** Every claim above is held by an instrument that fails loudly, and every
...
* **[MSC-36]** **[MJC-36]** **A fourth graphics group, or any growth of the declared group-C region.**
...
* **[MSV-16]** **A re-freeze of the romset.** `jtframe`'s `mra2rom` locates every zip
