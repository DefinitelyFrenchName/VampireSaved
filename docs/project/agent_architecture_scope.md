...
| **a definition run as the MAIN session (`claude -p --agent <name>`) takes its model and tools but NOT its effort** | **MEASURED** (A12, 14z-177) | the definition's Sonnet ran and its tool list bound (no Write -> `NO-TOOL`; A12c, a copy with Write, created its marker), but the session ran at the settings' default effort (`high`) where the definition said `low`; an explicit `--effort low` pinned it. So a reader launched this way needs its effort on the command line — unlike a subagent, whose definition's effort applies (A3). A session also loads its agent list at START: a definition written mid-session cannot be spawned by that session (`Agent type 'measurer' not found`, 14z-177) |
