...
| **the built-in `Explore` with no `model` follows the caller BELOW the cap and is capped at Opus-class above it** | **MEASURED** (A11; the archive) | Sonnet under a Sonnet parent, **Opus 5.5 under a Fable 5.1 parent** (A11), Opus 5 under every archived Fable parent. A first A11 assumed it "carries its own model" and FAILED. `claude-code-guide` does not exist in a headless run ("this agent type doesn't exist"), so it cannot be probed |
