...
> through; gated by `tests/test_agent_hooks.sh` (18 cases and the real ledger; controls `open-push-hook`, `misread-ledger`, `no-target-check`). **S4 LANDED 14z-177/178** (§5): the
...
`tools/agent/probe_agents.sh` (A1-A12, ~6 min, every "cannot" leg beside its "can" leg).
...
| **a project PreToolUse hook on `Agent` sees the spec and can DENY the call** | **MEASURED** (A8, A8c; the prompt asserted in the hook's input since run `2026-09-23-110`) | its input carries `subagent_type`, `model`, `prompt`, `description`; a deny on a `model` override was refused with its reason and no worker ran, and the same gate let the plain call run |
...
| **a definition with no `model`, and a `general-purpose` call with none, run on the CALLER's model** | **MEASURED** (A9, both parents; A11 under Fable) | haiku under a haiku parent, sonnet under sonnet, and **Fable 5.1 under a Fable 5.1 parent** (A11): an omitted model is the orchestrator's, which S5 makes Fable |
| **the built-in `Explore` with no `model` follows the caller BELOW the cap and is capped at Opus-class above it** | **MEASURED** (A11; the archive) | Sonnet under a Sonnet parent, **Opus 5.5 under a Fable 5.1 parent** (A11), and Opus 5 under every archived Fable parent (an inline count over the archive, NOT promoted into a tool — named in STATE 14z-177's close row). A first A11 assumed it "carries its own model" and FAILED. `claude-code-guide` does not exist in a headless run ("this agent type doesn't exist"), so it cannot be probed |
...
| **a definition run as the MAIN session (`claude -p --agent <name>`) takes its model and tools but NOT its effort** | **MEASURED** (A12, 14z-177) | the definition's Sonnet ran and its tool list bound (no Write -> `NO-TOOL`; A12c, a copy with Write, created its marker), but the session ran at the settings' default effort (`high`) where the definition said `low`; an explicit `--effort low` pinned it. So a reader launched this way needs its effort on the command line — unlike a subagent, whose definition's effort applies (A3). A session also loads its agent list at START: a definition written mid-session cannot be spawned by that session (`Agent type 'measurer' not found`, 14z-177) *— CORRECTED 14z-178: on 2.1.281 a definition written mid-session was announced by the harness at a later turn and spawned at its caps; when the list reloads is not measured* |
| **where a worker's words land** | **MEASURED** (A10, controlled; the background form from 14z-176's transcript only) | the SPEC is the Agent call's `prompt`; a background worker's REPORT reaches the orchestrator as an `<agent-message from="<id>">[Subagent hand-back]` record (the completion `<task-notification>` explicitly does NOT repeat it), a foreground worker's as the call's tool result; the worker's COMMANDS exist only in its own transcript. `extract.py` carries 300 characters of the spec and neither the report nor the commands. A10 checks the link, the command's place (in the worker's transcript, NOT the orchestrator's) and the report's arrival by an output token the spec cannot supply (`LOW-42` from `echo LOW-$((6*7))`), matched only in THIS call's result or hand-back, with a wrong-id control — and a FOREGROUND report arrived as the call's tool result. The BACKGROUND hand-back form rests on 14z-176's transcript alone, not on a controlled leg. (A10's first version matched the spec's own token and passed vacuously — rule-checker run `2026-09-23-110`.) **14z-178, 2.1.281: A10 FAILED, then a THIRD form** — a headless run now launches every worker in the BACKGROUND and its report arrives as the `<result>` of the completion `<task-notification>` for that call's tool-use id; the matcher learned it (and stopped counting a notification as the prompt), and `extract.py` now sources it with an `H` line. An interactive session the same hour delivered hand-backs. |
| **a subagent is handed the project's CLAUDE.md, and (interactively) the auto-memory index; `omitClaudeMd: true` removes CLAUDE.md** | **MEASURED** (A13, 14z-178, 2.1.281; the memory half an interactive observation and the archive) | a random CLAUDE.md codeword was named, with no tool, by the main session, a rule-checker-shaped definition and `general-purpose`, each worker's `instructions` attachment agreeing; the same definition with `omitClaudeMd: true` named nothing and carried no attachment, as did `Explore`. In THIS interactive session a no-tool `general-purpose` worker quoted `MEMORY.md`'s first three entries — against the docs' *"auto memory isn't loaded"* in a non-fork subagent; a headless session loads no auto memory, so A13's memory half is VOID there. The archive: every `general-purpose` worker since 2.1.270 (217, read 2026-09-24 — the census covers the last 30 days only, `docs/platform/gotchas.md`) and the `measurer` carried both (`transcript_gaps.py --subagents all`). **So the rule-checker's readers, ruled context-free ("never CLAUDE.md"), were handed CLAUDE.md and the memory index on every run.** And `omitClaudeMd` removes BOTH: the pinned `rule-checker`, spawned interactively in the same session, carried no attachment and named neither file (Opus 5.5, effort `high`) |
...
**RULED 2026-09-23 (14z-177, `DECISIONS_HISTORY.md` "Ruled 2026-09-23 (14z-177)")** on that
basis: the caps are enforced on the CALL by a hook (a proposal the maintainer installs) and on
every definition by a static gate; FORKS pass as the orchestrator continuing; the rule-checker's
readers become a pinned definition (`opus`, effort `high`) after a recalibration of all eight
fixtures; C1 widens QP3 to a worker's figures and gains QP5 SPEC-CONFORMANCE; the order is the
static gate + `measurer` / `reader` + the spec template, then the extract and the S4 gate, then
the call-gate hook, then the recalibration.
