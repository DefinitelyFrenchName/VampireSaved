...
| hitclass_map_extend (site_thunk row, `huitzil.toml` + `pyron.toml`, deduped on the merge) | vsavj's projectile hit sweep maps both colliding objects' type bytes through ONE 64-entry byte map (`0x1A888`, seven `bsr` callers); vs2's has 80. A ported type >= 64 in the `$FF94xx` pool that LANDS A HIT over-indexes it — the f7997 vec3. Body GENERATED (`tools/gen_hitclass_map_thunk.py`): vanilla's 64 bytes verbatim (incl. the 59-63 zeros — Donovan's 61/62 stay vanilla by maintainer decision) + vs2's 16 extension entries + a loud `>= 80` ILLEGAL. **Donovan does NOT declare it** (his 59-63 fit). Gates: `test_hitclass_map_thunk` (reconstruction), `audit_hitclass_map_cost` (cost + both fire censuses). **DEPRECATION CANDIDATE — OPEN, and read both halves.** Its crash rig is dead: `pyron/70_pyron_mash` + PYR_SOAK no longer reaches the map at all, so the no-thunk twin has nothing to crash on (measured 14z-93, audit section 4). That is a RIG failure, not evidence the fix is unneeded — 36 frozen stamp rows still carry types 64-75 into this pool. Keep-or-drop needs a rig that produces a POOL-vs-POOL contact; no current rig does (section 3). Do not drop the row on the dead control alone. | 14z-82b; ADOPTED 14z-82c |
