...
understand the game. One `##` per subsystem; every section names the
`atlas/` rows it depends on and the gates that lock it. Facts carry an
`[M: gate, session]` tag (measured) or `[D: session]` (derived, not yet
observed live); a claim with neither is a reading. Chronology lives in
`engine_internals_history.md`, where blocks moved verbatim as this document
was rationalized (14z-123). Two rules keep it findable: when you work on a
subsystem, write its section as part of that work; when a finding is
...
**Atlas rows this section depends on:** `atlas/character_tables.md` (the
per-character bank and its variant half), `atlas/id_space.md` (the 5-bit
id and the alias rule), `atlas/select_screen.md`, `atlas/sprite_lists.md`
(the drawer table that cannot grow).
**Gates:** `tests/test_variant_dispatch.sh`, `tests/test_capture_pose_sources.sh`,
`tests/test_tenant_hud.sh`, `tests/test_beam_list_type6.sh`,
...
**Atlas rows this section depends on:** `atlas/character_tables.md` (the
per-char OBJ bank table), `atlas/sprite_lists.md` (the per-game tile-code
bias table).
**Gates:** `tests/test_gfx_tiles.sh`, `tests/test_gfx_layout3.sh`,
`tests/test_gfx_tile_codec.sh`, `tests/test_gfx_chain.sh`,
`tests/test_gfx_collision_gate.sh`.
...
and the record formats), `atlas/character_tables.md` (the per-char OBJ bank
table `PRG:0x282D4`), `atlas/ram.md` (object `+0x18`/`+0x1A` OBJ words,
`+0x1C` anim node).
**Gates:** `tests/test_gfx_tiles.sh`, `tests/test_obj_records.sh`,
`tests/test_obj_record_walk.sh`, `tests/test_m2b_stage6.sh`,
`tests/test_m2b_scroll3.sh`.
...
The M2b-era slot-0x0F placement (Donovan into Jedah's band at +0x2750, the
per-slot exclusivity census with Sasquatch sharing 44 head tiles) is
superseded by WIDE group C; the record is in `engine_internals_history.md`
and its method survives as the inventories-are-SPANS rule of
`../project/porting_sprite_lists.md`.

### Two facts from the M2b tile step that still bind

- Tile placement is a 16-aligned constant delta applied to every main-band
  tile word of the OBJ records, with the canonical extraction's inverse
  (`gfx_tiles.write_tile`) round-trip asserted on every placed tile
  (`tools/build_gfx_donovan.py`).
- **A ported per-char OBJ bank table must carry vsavj's values for EVERY
  row.** A copy truncated at row 9 and carrying VS2 bank words (row 0x0F =
  0x0000, bank-0 reads) WAS the main-sprite garble — confirmed by its fix
  (`[table_fix]`, `tests/test_m2b_stage6.sh`). The stage-by-stage M2b
...
`tests/test_m2b_stage6.sh` (a stage-6 build with static verification inside,
five guarded soaks, the full masked legacy gate) and `tests/test_m2b_scroll3.sh`
(scroll3-vs-placement exclusivity: `tests/lua/scroll3_watch.lua` scans the
scroll3 tilemap every frame; the scroll3 base register is write-only, written
ONCE at boot at PC `0x926`, constant across 42,000 marathon frames). MAME
Lua: `emu.register_frame_done` is a single slot (use
`add_machine_frame_notifier`) and notifier subscriptions must be pinned in
globals (`../platform/gotchas.md`). The M2b freeze list this replaced is in
the history — everything on it shipped (select art 14z-62/63, effects 14z-71,
the `0x3A3CA0` pool measured 14z-123).
...
**Atlas rows this section depends on:** `atlas/select_screen.md` (CANONICAL for
the wheel, the cell/id identity, the record-pointer arrays, the drawer and
the ring), `atlas/ram.md` (`$FFB800` menu objects, `+0x382` select id),
`atlas/venue_assets.md`, `atlas/id_space.md`.
**Gates:** `tests/test_select_wheel.sh`, `tests/test_select_arrays.sh`,
`tests/test_tenant_select_records.sh`, `tests/test_wheel_bank5.sh`,
`tests/test_gfx_menus.sh`, `tests/test_oboro_select.sh`,
`tests/test_random_select_tenants.sh`.
...
### The select screen's laws (the atlas is canonical)

- **Per-wheel-slot pointer ARRAYS, not a per-character table.** The hovered
  cell's portrait / name / highlight ride arrays indexed by the wheel cell
  (stride 8 per step); the player offset is **+0x80** (`d1 = 0x80` at
  `PRG:0x06C0E0`), and the +0x40 block is the VARIANT HALF (rows 0x10-0x1F
  aliasing 0x00-0x0F) — it looks like P2 from a differential dump precisely
  because it aliases [M: 14z-61, `atlas/select_screen.md` "The
  RECORD-POINTER array"]. At a variant id a tenant has its OWN rows in both
  blocks: a two-long repoint, never content surgery.
- **Portrait/name palettes** upload from an 11-variant x 16-char grid at
  `PRG:0x3AC000` (uploader `0x5F136`: row = base + (variant*16 + char)*0x20,
  char from the +0x382 select id; palette RAM rows 0x1B/0x1C). vs2
  special-cases its newcomers in code — three shapes, "Select-portrait
  palette dispatch" below.
- **The VS-splash busts** are drawn by objects `$FFC100/$FFC180` whose
  `+0x1C` anchors are root-table cells in three char-scaled families (d0 =
  char, 4*char, 0x80+char; P2 = +0x20 within each) over `0x2672AA` (vs2
  `0x2A05E2`) — multi-record slide-in chains whose record sizes do not pair
  between the games.
- **The wheel record `PRG:0x272A68`** (fmt 2, count 17 vanilla; the appended
  cells since 14z-62) cannot grow in place; the medallion art lives in group
  C bank 5 and the ring/highlight position comes from the 32-row base table
  `PRG:0x5FAE2` (`atlas/select_screen.md` "Drawing the new medallions", "The
  wheel DRAWER", "The ring/highlight POSITION source").
- **Standing order (CLAUDE.md §4's watch):** one M2-era select-gate
  invocation diverged unreproducibly and was green on every rerun; the
  masked-legacy helpers PRESERVE failing logs (`build/gate_failures/`) so a
  recurrence self-documents — if it recurs, stop and root-cause before any
  freeze.

The derivation — the slot-0x0F phases 1-2 (in-place record surgery, the
borrowed tile placements), the splash/win map and the gate anomaly as
logged — is in `engine_internals_history.md`; the in-place approach was
retired at 14z-62 when the tenants moved to variant ids.

## Anim-script walker + hit-freeze / reaction subsystem (session 14z-42, measured)

**Atlas rows this section depends on:** `atlas/ram.md` (object `+0x1C` node,
`+0x20/+0x21` countdown and flags, `+0x5C` hit-freeze, `+0x64/+0x94` hitbox
family, `+0x147` re-hit gate, `+0x54` reaction class),
`atlas/character_tables.md` (the anim index family, `tail_data_ptr`).
**Gates:** `tests/test_anim_node_walk.sh`, `tests/test_move_naming.sh`,
`tests/test_don_sword.sh`, `tests/test_don_reactions.sh`,
`tests/test_reaction_hook_d2.sh`.
...
the newcomer classes **0x50-0x53**, which vsavj's tables end before — an
over-index into code bytes (measured signature: vec3 at PC `0x01850E`,
ADDR `0x018511`, D0 = 0xA2, register-forced on don_m11). Why the fix is
CODE and not a data remap: no vanilla class value is dispatcher-exact on
all three tables AND property-preserving — `0x51 -> 0x19` diverges on
dispatcher 3 and fails the 14z-43 `cmpi.b #$51` guard; `0x51 -> 0x4E/0x4F`
swaps `property[0x51] = 0x19` for `0x0F` (the ES-freeze family). The shipped
shape: the vanilla site `0x018458` becomes `jmp thunk`; the 82-byte
`reaction_hook` thunk keeps the vanilla `tst.b ($38,a1)` prefix and, on each
arm, routes ids `0xA0-0xA7` through an ext table of four VERBATIM vs2 case
handlers (`d2_case_a0..a6`, re-derived from vsav2.zip by the gate on every
run) and everything else to the untouched vanilla dispatcher — legacy pays
two `cmpi` per dispatch and nothing else. The tenants' node DATA stays
native; the six deity node bytes were later renamed 0x51 -> 0x44 (14z-110b,
a measured five-consumer equivalence), so the window's live inventory is
EMPTY on the shipped build and the window stays as the guard. The field
#99 crash's ROOT cause sat upstream — the CPU AI script alias, "The CPU AI
action-script system". [M: `tests/test_reaction_hook_d2.sh` (reconstruction
from the reference ROMs + three verdict controls); bytes in
...
the victim".
**Gates:** `tests/test_reactions.sh`, `tests/test_move_naming.sh`,
`tests/test_killshread_es.sh`, `tests/test_projectile_census.sh`,
`tests/test_projectile_params.sh`.

- **The reaction paths are a cross-character CONVENTION of the anim index,
  not per-tenant tables** [M: `tests/test_reactions.sh`, re-frozen 14z-121
  under deterministic labelling]: light/medium `b:0x03`, heavy `b:0x03 ->
  b:0x23`, the sweep knockdown `b:0x09 -> 0x0a -> 0x0b -> b:0x44`, an air hit
  `b:0x03 -> b:0x2a -> b:0x14 -> b:0x15 -> b:0x44` (Pyron `b:0x19 -> b:0x2a ->
  b:0x14 -> b:0x29 -> b:0x44`), a throw `c:0x01` (Donovan) / `c:0x0d`
  (Huitzil) / `c:0x02` (Pyron) held, the block stance `a:0x13`/`a:0x14`, the
  stand `a:0x00` — the same shape as the per-victim pose index of [VSE-44].
  A reaction node sits on MANY chains (indexed from several seqs of table
...
- **How the stun runs (frame by frame, Donovan taking Victor's 5LP / 5MP,
  ...
  pushback's duration, i.e. the attack record's `+0x1C`. **What moves the victim on a light/medium hit is a STEP TABLE, not a
  velocity** (`+0x40` stays 0) [M: 14z-121 (3); the 14z-120 (12) reading that
  named the pushbox-separation routine had tapped one light hit's overlap
  frames — history, and `project/gotchas.md`]. Tapped through 5LP / 5MP /
...
| `+0x1C` | `0x16B70` ONLY | ADDED to the victim's accumulator `+0x161` (decay timer `+0x162` := 240 f; the per-frame tick `0x20E6A` clears both when it expires) and compared with the per-character threshold `+0x15B` — the bank row **`byte15b`, 60 for every character in vsavj AND vs2** — but ONLY while the victim's `+0x15E` is armed and it is grounded (`+0x38` = 0), with `+0x11F`/`+0x18F` clear. **`+0x15E` is SASQUATCH'S DARK-FORCE ARMOR** [M: `tests/audit_df_accumulator.sh`, frozen `tests/expected/df_accumulator.txt`, 14z-123 — this cell read "live only for an Aulbath VICTIM … `0x4900A` inside Aulbath's block … armed nowhere the rigs reach … the fork OPEN" from 14z-121; RETRACTED: the block attribution had read the NEXT table's (`0xD9538`) head addresses as code boundaries]. The 0x200-arming state (vsavj `PRG:0x047E60`, vs2 `0x48F9C`) is row 0x0A of `dispatch_16` (`PRG:0x0BF31A`, the per-character seq-0x16 activation dispatch reached from `PRG:0x02325E`). An LP+LK or MP+MK activation takes sub-state 2 and, when the activation chain ends, arms `+0x15E` = 0x200 with `+0x18F` clear (`PRG:0x047EDA`); an HP+HK activation (`+0x122` = 0x4400) takes sub-state 4 and never arms. Armed, each contact adds the record's `+0x1C` (Victor cr.LP 20 / cr.MP 30 / cr.HP 40), reloads `+0x162` = 240 and takes the ARMOR path `PRG:0x0187BC` (vs2 `0x170DE`): no reaction, `+0x54` unwritten, damage taken (5/12 vs 4/10 unarmored). The contact that carries the sum PAST 60 reacts normally (`+0x54` = class, seq 2) and clears `+0x161/+0x162`; the next starts a fresh sum. `+0x162` expiry and DF end clear everything. The merged build's field trace is byte-identical to pristine vsavj. Aulbath's DF arms the OTHER family — `PRG:0x045FAA` writes `+0x15E` = 0x7FFF with `+0x18F` = 1 (full armor, no accumulator: the `+0x18F` test routes every hit to the armor path); 0x7FFF sites are engine `0x23532/0x23570/0x2377A/0x3DA60` and character blocks `0x45FA4`, `0x4C78E/0x4C7A0`, `0x4D020/0x4D040` — "four sites" undercounted. The tenants' `+0x1C` bytes ride by value and feed Sasquatch's armor when he is the victim. Not reached: the CPU-side choice (`+0x3B4` / `$B4(a5)`), the node-byte-+0xB armor exception, the timers block's per-frame rate |
...
**Atlas rows this section depends on:** `atlas/venue_assets.md` (the HUD
mugshot / name tables), `atlas/select_screen.md` (the wheel record and its
palette blocks), `atlas/ram.md` (`$FF5D94` staged HUD records, the
`$FF4182` fade-staging family).
**Gates:** `tests/test_tenant_hud.sh`, `tests/test_don_colors.sh`,
`tests/test_don_reactions.sh`, `tests/test_select_wheel.sh`,
`tests/test_wheel_bank5.sh`, `tests/audit_palette_seq_ids.sh`,
`tests/test_variant_dispatch.sh`, `tests/test_pyron_blink.sh`.

Two venue-asset families, both per-slot (the tenants' rows were the
dead-row class's instance 5).
...
### Select wheel (the medallion ring) — canonical: `atlas/select_screen.md`

ONE static OBJ record — the **wheel record** `PRG:0x272A68` (fmt 2, count 17
vanilla + the appended cells; a header long points at the centre-relative
coordinate list, wheel centre raw ≈ (256,176)) — no rotation, no hover zoom;
only the cursor ring moves. Cells are fixed perspective sizes (3x2 close,
2x2 far, ONE 3x3 — Gallon's, at (264,64)); cell -> character is by ART, not
by palette index. Each cell owns a select-venue palette row (row == attr
pal index) from block A `0x3A3800` (live-verified: rows land at `90C000 +
row*0x20` with the bright nibble forced to F); block B `0x3A3C00` is another
sub-venue's. vs2's wheel is the same record family at `0x2A6D8C+` with three
variants (18 cells + two 21-cell variants appending the newcomers).
Gates: `tests/test_don_colors.sh` (select section), `tests/test_select_wheel.sh`.
The M2b slot-0x0F recolour of Jedah's cell is in the history.
...
### The version string and the Oboro pick (atlas: select_screen.md)

- **The wheel record draws the VERSION STRING** (since 14z-105): the copied
  roster21 record gains N 1x1 glyph entries — authored tiles in group C's
  upper bank (`0x1FE40+`), pal row 0x19 (thunk-re-asserted every select
  frame), placed at screen (324,202) = OBJ (388,218) — a glyph at x=340 would
  put its last ink column at pixel 384, off the 384-wide screen. Measured on
...
`tests/test_qs_ledger_binding.sh`, `tests/test_kernel_voice_tables.sh`,
`tests/audit_voice_borrow.sh`, `tests/audit_qs_voice_batch.sh`,
...
### The shared sfx library uses IDENTICAL ids in both games (the id-space sweep, `docs/project/m5/keyons_*.json`)
...
content-verified). (The 214P/214K music bug was root-caused 14z-52 — STATE_HISTORY "M5 phase
1: music bug root-caused".)
...
window). Sample ROMs are FULL (no blank 64K blocks), which is why the WIDE profile
carries a 16 MB QSound region (`../project/cps2_wide.md`) and the M5 voice
batch (14z-86, "The QSound Z80 driver" below) is authored into it.
...
- **The trap's real sfx** [M: 14z-85g/86/95]: the spawn-EJECT sound is
  per-node record node 10 (id 0x0739, dispatched on Phobos' anim context);
  the TIMER-DETONATION sound is NOT a record dispatch — it is sound-farm stub
  vs2 `0x4F2E` (`jsr $330E; move.l #$73A,d1; bsr helper; jmp $3306`), jsr'd
  from the mine handler at `x068458+0x120`; vs2's engine carries a FARM of
  such one-id stubs around 0x4EE0-0x4F60. vsavj keys MUSIC-family content at
  the same 0x7xx ids, but 0x73A's SAMPLE CONTENT is in vsav's own QSound
  image at `0x6C0000` keyed as the 0x198/0x199 family — identical for 20,480
  of 20,481 bytes; the INCLUSIVE endpoint `0x6C5000`, which the packing law
  says is PLAYED, differs between the original sample ROMs (vsav 0xFF, vsav2
  0x00; GitHub #93) — so the restoration is a synthesized vsavj twin stub
  playing 0x199, no sample port. 0x739's sample content, record (#0x5C, bank
  0x18 @0x18D800) AND note-table-1 entry 0x28 all exist on vsavj — only the
  Z80 SONG is missing ("The QSound Z80 driver"). Rule: a 0x7xx id's
...
### The QSound Z80 driver: id table, songs, streams, sample records (measured 14z-86, reader-traced on live vsavj, id 0x119)

**The file mapping (read this first).** MAME loads `vm3.01` split: file
0x0000-0x7FFF at region 0, file 0x8000-0x1FFFF at region 0x10000
(`ROM_CONTINUE`), `vm3.02` at region 0x28000 — region != file offset above
0x10000, and a table read at the wrong offset looks plausible (14z-85d paid
for it; history).
...
### The Z80 command dispatch, the note-table array, the dead type-C song class and the alias bit (measured 14z-86, the voice batch)
