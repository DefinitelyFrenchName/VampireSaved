...
| `PRG:0x00A43E` | `(id & 0x0F)` → `$130(a5)`, plus a struct pointer at `$13A(a5)` and a state kick (`$4(a5)=0x0A`, `$106(a5)=1`) | ~~`$130(a5)` is written ONLY here and read at 15 sites~~ `$130(a5)` gets a FOLDED id only here — four other sites write constants (measured 14z-157, `tests/audit_ff8130_writers.sh`) — and is read at 14 sites (#100), most clustered at `0x01BF9x-0x01C38x` and `0x021AC8-0x021C8E` — beside the select-screen code, i.e. the **per-slot venue-asset display family** (mugshot / name / medallion), whose arrays are 16-wide | **medium** — the same 16-wide per-slot arrays the project already ports for Donovan; extend those, then widen |
...
| `PRG:0x04FAC4` | `(id & 0x0F) * 24` into `PRG:0x04FFA8` — 12 words per character (6 pairs; `tst.b $bc(a5)` selects +0 or +2), values `0x0370-0x03D7` for every row except slot `0x8` | **nothing structural.** Measured: that table is **32 rows × 24 bytes**, ending cleanly at `0x0502A8`, with rows `0x10-0x1F` byte-identical copies of `0x00-0x0F`. **Two exceptions the range hides, measured 14z-142 by `tools/checkdocs_rom.py` and stated here because a porter reads this cell for what a row contains:** slot `0x8` carries its own block `0x02A5-0x02AD` (Bishamon — the same slot that is the variant-alias exception in `character_tables.md`), and row `0x0B`'s 24 bytes are a byte copy of row `0x4`'s, i.e. the gap slot the random-select table also skips holds Zabel's numbers rather than data of its own | **easy** — the rows already exist; fill the tenant's row and widen the mask to `#$1f` |
