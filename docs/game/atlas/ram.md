...
| +0x161.b / +0x162.b | the accumulator of attack-record `+0x1C` values while +0x15E is armed, and its 240-frame decay timer (both cleared on expiry, `0x20E6A`, and on every contact that does not accumulate, `0x16B9C`); threshold +0x15B | [D: 14z-121; never nonzero in a rig] |
