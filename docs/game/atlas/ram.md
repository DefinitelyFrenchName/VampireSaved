...
| `+0x381` (`$FF8781`/`$FF8B81`) | selects the UPPER or LOWER 4-id half of the owner's palette-seq block in the per-character palette routines (`0x02A8A4` rows: `id = BASE + ($381<<2) + phase`). Read 1 on a CPU Sasquatch (arcade ladder, 26_don_arcade_mash f12949 -> ids `0xAE-0xB1`), 0 on P1 in the Demitri control and in the alt-colour pick replay 41 — a costume/colour index is the natural reading, NOT established | [D: 14z-118, three dumps; `engine_internals.md` "THE FAMILY RULE"] |
