...
| +0x3BD (`$FF87BD`/`$FF8BBD`) | a COPY OF THE CHARACTER ID written by the select CONFIRM — on vsav2 `PRG:0x01F6CE` at the confirm frame (1299 in the naming rigs), value = the id of the cell the cursor is on. NOT rewritten by the early-window id poke ([VSP-123]), so a forced-pick leg carries the confirmed character's id here for the whole match. No reader in the first 3,400 frames of Phobos's part-1 rig (a PC-attributed read+write tap over `$FF87BC-$FF87E3`); its consumer is unmeasured | [M: 14z-160, `tests/audit_forced_pick_fidelity.sh`, GitHub #151] |
| +0x3C2 (`$FF87C2`/`$FF8BC2`) | THE VS2/VH2 FLAVOR LATCH — `atlas/character_tables.md` "Start-hold flavor" is the spec. Written at the select CONFIRM per character (vsav2 `PRG:0x01F848` writes Phobos's default 00; Donovan's default is 01) — BEFORE the early-window id poke, which is why a forced-pick native Phobos carries Donovan's 01 (GitHub #147/#151). Phobos's readers in play: `PRG:0x026318` (gates the seq-4 sub-4 startup state on flavor ≠ 0 — with flavor 0 the routine returns 0 and no such state is played), `PRG:0x02595A` (the float timer, 0x3C under flavor 1 instead of 0x78) and `PRG:0x02598A` (the hover fork) | [M: 14z-160, tap + capstone on the vs2 opcode view; `tests/audit_forced_pick_fidelity.sh`] |
| +0x3E0 (`$FF87E0`/`$FF8BE0`) | a second CONFIRM-TIME COPY OF THE CHARACTER ID — vsav2 `PRG:0x01F6C8`, same frame and value as +0x3BD, likewise untouched by the id poke. No reader in the same 3,400-frame window; consumer unmeasured | [M: 14z-160, `tests/audit_forced_pick_fidelity.sh`] |
