...
| `RAM:$FF8100` | the VOICE-CLASS index, written by `0x00af10` as `pool[$FF8114]` from the borrow pool at `$FF1E50` (sign-extended byte). **It can hold the pool's `0x18` TERMINATOR** — measured at the #92 crash — and consumers must treat `0x18` as end-of-list, not as a class. Downstream, `0x05ffb6` uses it as `A0 = 0x26775A + 2*v - 4`, a row of the per-char long-pointer table `0x26771E`; the last row whose FOLLOWER is a real pointer is 0x19, and `v=24` selects row 0x1A whose follower is that table's own `0x00400000` terminator | [D: 14z-93, GitHub #92 — write tap + sparse guard probe on the crashing timeline] |
...
| — voice-class borrow block (14z-87, `A5=$FF8000` frame). **THE LIST IS TERMINATOR-DELIMITED BY `0x18`, and the live list is SHORTER than the ROM row (measured 14z-93, GitHub #92):** every ROM row at `0x00B268` ends with `0x18` at byte 7 (read via the DATA view — it is inside the crypt window), and the pool holds DOUBLED values (`0x0e`=2*7, `0x12`=2*9). At the #92 crash the pool read `0e 12 18 0a 00 14 16 0a` — terminator at index **2** — and the selector `$FF8114` was 2, so `0x00af10` read the TERMINATOR as a class into `$FF8100`. Anything indexing `$FF8100` must therefore stop at `0x18`, not consume it. | pool `RAM:$FF1E48` (8 candidate classes, copied per event from ROM `0x00B268` row `(0x382,opponent)<<6 + $FF8121`), voice-number list `RAM:$FF1E50` (from ROM `0x00BB68`), in-use mask `RAM:$FF8110.l` (bit = class; sound-state-fed — the run-to-run lottery), chosen index `RAM:$FF8114.w`, scan bound `RAM:$FF8138.w` (=6 measured), venue byte `RAM:$FF8121` | [D, 14z-87] |
