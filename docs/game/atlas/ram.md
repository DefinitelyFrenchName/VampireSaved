...
| `RAM:$FF8127` | ~~P1 downs-won counter~~ ~~(14z-118 first reading: 1 after a P2-won down)~~ **RESOLVED 14z-118 by its WRITER: a per-frame COMPARATOR, not match state.** `PRG:0x02228E`: `d1 = (P1 object)+0x10; cmp.b (P2 object)+0x10,d1; beq/bcc -> 0, else 1; move.b d0,$127(a5)` — i.e. **1 while P1's fighter-object byte `+0x10` is LESS than P2's, 0 otherwise**, written every match frame (11,972 writes in 5,300 frames of the timeout rig; toggles for 1-46-frame spans in a 2P match). Both earlier readings were this comparison caught at a down or a refill. What `+0x10` of the fighter object IS (an 8-bit ordering key — draw priority or depth is the natural guess) is unmapped: OPEN, listed under the object fields | [T+D: 14z-118, write tap `build/c8127_tap_14z118.log`; the two 14z-104/14z-118 single-edge readings retracted] |
...
| +0x15E.w | the ARMING counter of the +0x161 accumulator: 0x7FFF at four state entries inside the per-character DARK FORCE handlers (vs2 `0x2203A/0x22078/0x22282/0x3E928`, with +0x18F/+0x190/+0x143), decremented per frame; **0 on every frame of every naming and victim rig** (write-tapped on both fighters 14z-121) | [D: 14z-121; the arming states OPEN] |
| +0x161.b / +0x162.b | the accumulator of attack-record `+0x1C` values while +0x15E is armed, and its 240-frame decay timer (both cleared on expiry, `0x20E6A`, and on every contact that does not accumulate, `0x16B9C`); threshold +0x15B. Live only for an Aulbath victim (the sole `+0x18F` clear that keeps +0x15E armed is `0x4900A` in his block) | [D: 14z-121; never nonzero in a rig] |
...
| +0x1B0.w | the sibling counter of the second step family (`0x27082`, lists `0x2797A`) — unread | [C] |
