...
| `RAM:$FF802E` | the MATCH-LEVEL DARK FORCE FLAG: 1 for the whole mode, 0 before and at expiry, on both games — vs2 raises it for its P+K Dark Force POWER (360 frames; 269 for Morrigan and Lilith), vsavj for its Dark Force CHANGE, 28 frames after the press (per character, 269..540 frames at level 6). vs2's personal-DF EX installs do NOT raise it (Phobos's toggles 3/0 for 19 frames); on our build the tenants' EX route does (451/469/451). Derived by dumping all of work RAM at five phases on both games and keeping only bytes off/off/on/on/off — never infer the mode from the fighter block (`+0x1F4`, `+0x1B5/+0x1B9` are set by JUMPING) | [M: 14z-69 five-phase dump; 14z-168 `tests/audit_df_modes.sh`] |
| `RAM:$FF8081` | the game task's PASS counter — `addq.b #1,$81(a5)` at the top of every logic pass (vsavj `PRG:0x008E10`, vs2 `0x0075DE`), so a frame with an extra TURBO pass steps it by 2; not the frame counter `$FF8080` | [D: 14z-158, #135 — the decider's prediction matches its step on every frame] |
| `RAM:$FF343A.w` / `$FF343C.w` (`-0x4BC6/-0x4BC4(a5)`) | **THE HIT-REGISTRATION PAIR — the (attacker, victim) fighter-block words of the current hit**, read by 34 engine sites (the hit stager's meter award, the damage scaler's attacker reads, the reaction handlers). Written every frame by the collision registration `PRG:0x017FF8/0x018000` once per fighter — (P1,P2) then (P2,P1), so the walk leaves (P2,P1) — and by the throw sites `0x029694/98`, `0x029762/66`, `0x029828/2C` on a throw's contact frame. vs2's pair is `-0x4B74/-0x4B72(a5)` = `$FF348C/$FF348E`; on vsavj THAT range is two words of the four-word ring `-0x4B76..-0x4B70(a5)` that `PRG:0x0194AE` shifts (trigger unmeasured; in the three throw rigs only the boot RAM test read it), and every tenant's placed throw code still writes pointer words into it (the 14x rollback; no ported instruction reads it: all 24 placed sites are writes — `build/meter_probe_14z166/census.txt` parts 4-6) — `engine_internals.md` "THE HIT-REGISTRATION PAIR, MEASURED" | [T+D: 14z-166, `tests/audit_throw_registration.sh`, `build/meter_probe_14z166/`] |
| `RAM:$FF8116` | the SPEED LEVEL (0..15): indexes the 32-bit extra-pass pattern (vsavj `PRG:0x008E6C`, vs2 `0x00763A`); written at the play-mode confirm as `TBL[turbo bit + adjustment]` (vsavj `PRG:0x020DF2`, vs2 `0x01FA48`). At factory settings NORMAL = 6 on both games, TURBO = 7 on vsavj and 8 on vs2 — and vs2 defaults P1 to TURBO. Pin it on both legs of a cross-game comparison ([VSE-84]) | [D: 14z-158, #135; causal: vsavj forced to 8 ticks exactly as vs2] |
| `RAM:$FF8118` | the EXTRA-PASS REQUEST: set by the pass decider (vsavj `PRG:0x008E66`) when the level's pattern bit for the new pass is 1; while set, the post-pass routine (`PRG:0x008EB2`) skips the frame sleep, and the extra pass that follows clears it | [D: 14z-158] |
| `RAM:$FF812D` | the turbo-pass enable the decider tests: 0 outside a round, 1 from the round's start (frame 2544 on the measured rigs, written by vsavj `PRG:0x009788`) | [D: 14z-158; causal: forced 0 = no extra pass] |
| `RAM:$FF8134` | `$FF` during an EXTRA pass, 0 on a normal one — cleared and set by the pass decider (vsavj `PRG:0x008E32` / `0x008E3C`) | [D: 14z-158] |
| `RAM:$FF80A3` | the SPEED SETTING, config entry `$13` copied at boot (`PRG:0x001A92`): factory 5 on vsavj, 6 on vs2; at the play-mode confirm the level index gets +0 / +1 / +2 for 4 / 5 / anything else | [D: 14z-158] |
| `RAM:$FF8407` (select screen) | P1's PLAY-MODE cursor, 0..3 = NORMAL / TURBO / AUTO / AUTO&TURBO, a (turbo, auto) bit pair: set at character confirm to 0 on vsavj (`PRG:0x020D38`) and to 1 on vs2 (`PRG:0x01F98C`) | [D: 14z-158; causal: forced 0 on vs2, the game writes level 6] |
...
| `RAM:$FF8130` | the character class folded to 4 bits — `$382 & 0x0F` of a player struct — stored by `PRG:0x00A446` (the fold is `andi.w #$f,d0` at `PRG:0x00A442`); also written with constants, `0x0E` at `PRG:0x08E336`/`PRG:0x090BE4` and `0x05` at `PRG:0x08E342`/`PRG:0x090BD2`; covered by block writes (`PRG:0x000DD8` at boot, `PRG:0x016DFC` clearing `$FF8100-$FF817F`). None of the five direct writers fired in boot, attract, select, a match or a 1P arcade run on vanilla vsavj. Read at 14 sites, 8 of them folding again (#100, static) — `venue_assets.md` §2 addendum | [T] + static census, 14z-157, `tests/audit_ff8130_writers.sh` |
| `RAM:$FF8131` | written from `$FF80AC` at `PRG:0x02033E` (`0x00` in the measured flows) and bit-set at `PRG:0x020AE8` (`0x01`) — the two writers the 14z-64 venue-asset audit attributed to `$FF8130`; a word tap bucketed by write mask puts both on this byte only | [T] 14z-157, `tests/audit_ff8130_writers.sh` |
| `RAM:$FF8114` | the arcade-ladder SELECTOR: the one index `0x00aeca` scans across both candidate pools (`0x00af10` reads `pool[$FF8114]` into `$FF8100`), stopping at the first candidate whose class bit is clear in `$FF8110.l` or at the bound `$FF8138` (measured 6) — `engine_internals.md` "THE ARCADE LADDER". ~~the SELECTOR into the voice-class borrow pool … **Nothing measured yet bounds it below the terminator index** — at the #92 crash it was 2 and the pool's terminator was at index 2~~: the 14z-93 reading, superseded 14z-94 with #92's root cause (the pools are the arcade ladder's, and `0x18` was a stage value, not a terminator); corrected here 14z-157 | [D: 14z-93; the scan and its bound from `engine_internals.md`] |
...
| `$FF0EA4`, `$FF0EC4`; `$FF0EE0-$FF0FB1` (stride-0x10 records); `$FF1E0F/$FF1E11` | SELECT-SCREEN ROUTE RESIDUE, UNATTRIBUTED — bytes that differ between two REAL picks of the SAME cell by different cursor routes (Phobos L,L,L vs L,U,L on vsav2) and persist through the whole match, with the `$FF0Fxx` records re-populating during the load (2000-2600) and staying different in play. Not the poke's: a same-id poke leaves nothing (`donovan-self`), and Pyron's R,R,R vs R,R,R-plus-poke show none. `$FF0EA4`/`$FF0EC4` (0e/0f vs 12) differ only between a rightward and a leftward route. No reader measured; any cross-route comparison (every ours-vs-native leg, since the wheels differ) carries them, and the 14 IDENTICAL move-parity verdicts show they do not perturb those rigs. The one untested pairing: a leftward and a rightward route to the same cell on Pyron. | [M: 14z-161, `MEASURE=1 WHOLE=1 tests/audit_forced_pick_fidelity.sh`, its REAL-vs-ALT control] |
...
| +0x3BD (`$FF87BD`/`$FF8BBD`) | a COPY OF THE CHARACTER ID written by the select CONFIRM — on vsav2 `PRG:0x01F6CE`, on vsavj `PRG:0x020AC8` (`move.b $382(a6),$3bd(a6)`), value = the id of the cell the cursor is on. NOT rewritten by the early-window id poke ([VSP-123]), so a forced-pick leg carries the confirmed character's id here for the whole match. ITS READERS, enumerated by form (`tools/audit_latch_readers.py`, frozen by `tests/test_latch_readers.sh`): all under the Shadow/Marionette flag +0x3BC — the arcade-ladder base `PRG:0x00AF24` (vs2 `0x00977A`) and the byte-table index `PRG:0x00A782` (vs2 `0x008FC2`) — plus vsavj's `PRG:0x00F85A` word store. Read in play on NO tapped leg (23 legs, both games; `tests/audit_latch_reads.sh`) | [M: 14z-160 `tests/audit_forced_pick_fidelity.sh`; 14z-161 the reader census and taps, GitHub #151] |
| +0x3C2 (`$FF87C2`/`$FF8BC2`) | THE VS2/VH2 FLAVOR LATCH — `atlas/character_tables.md` "Start-hold flavor" is the spec. WHAT THE CONFIRM WRITES, per CELL (vs2 opcode view): cell 0x10 Phobos → 00, or 01 with Start held (`PRG:0x01F832`-`0x01F848`); cell 0x13 Donovan → 01, or 00 with Start held (`PRG:0x01F86A`-`0x01F87E`); EVERY OTHER CELL writes nothing and the select-entry clear `PRG:0x01F5C8` leaves 00. vsavj has NO writer at all (the port's init shim writes the tenant's flavor at char init, after any poke). Written BEFORE the early-window id poke, which is why a forced-pick native Phobos over Donovan's cell carries 01 (GitHub #147/#151) — and why one over Demitri's cell carries the 00 a real pick writes. READERS (census, frozen): Phobos's `PRG:0x026322` (after `cmpi.b #$10,$382(a6)` — the seq-4 sub-4 startup state is admitted only under 01), `0x02595A` (float timer 0x3C vs 0x78), `0x02598A` (the hover fork), `0x025B66`; Donovan's `0x05A650` (the shared per-character code family: `moveq #$43` vs `#$5E`), `0x065FE2` / `0x066020` (x065e5a, an effect variant off the owner's block). Measured in play: Phobos reads it every match frame (00 on a real pick, 01 poked over Donovan); Donovan's readers fired on NONE of his tapped rigs (DF, throw, tech roll, down attack, grab, the immortal rig, eight victim parts); Pyron has no reader | [M: 14z-160 tap + capstone; 14z-161 `tests/test_latch_readers.sh`, `tests/audit_latch_reads.sh`] |
| +0x3E0 (`$FF87E0`/`$FF8BE0`) | a second CONFIRM-TIME COPY OF THE CHARACTER ID — vsav2 `PRG:0x01F6C8`, vsavj `PRG:0x020AC2`, same frame and value as +0x3BD, likewise untouched by the id poke. READERS (census): the select-entry join copy `PRG:0x008A86` (vs2 `0x007254`, `move.b $3e0(a6),$382(a6)` — restores the id when a side joins, frame 939 for P2, BEFORE the confirm), the long copies `PRG:0x0149DC` / `0x014A58` (vs2 `0x0130D0` / `0x01314C`, `move.l $3e0(a6),$8(a0)` into a display object — the HUD-name stager of [VSP-121]), and `PRG:0x014A9A` (vs2 `0x013194`, `cmpi.b #$12` — the Marionette test) under the +0x3BC flag. Read in play on NO tapped leg | [M: 14z-160; 14z-161 the census and taps] |
...
| +0x6C..+0x6F (4 × .b) | **the RECENT-HIT SLOTS — the multi-hit dedup key, one byte per ATTACKING fighter block** (`+0x6C[attacker +0x70]`): the hit test `PRG:0x018064` refuses a record whose hit id (`+0x10`) equals this fighter's slot for that attacker (`0x0180C2`-`0x0180D0`); the contact install `PRG:0x01827C` writes the landed id there; and **`PRG:0x022268` CLEARS the whole ring every engine tick in which the OTHER fighter's current anim node carries no attack record** (`node +0x0A == 0` → `clr.l $86c(a5)` for P2's ring read off P1's node, `clr.l $46c(a5)` the other way). So the slot survives only across CONSECUTIVE attack nodes: consecutive attack nodes sharing an id land once, a same-id record lands again after any gap node (JE 5HP `2(5)2(5)2` lands 3, MO CL.5HK four consecutive records with ids 1-4 land 4). Bypassed entirely when the ATTACKER's `+0x08` is non-zero (untraced; 0 throughout the normals rigs). The 14z-125 wording "rotating index" was a static misreading: the index is the attacker's BLOCK, nothing rotates | [M: 14z-146, `tests/test_rehit_ring.sh` — write tap + field trace, SA/JE/MO hit rigs] |
| +0x70.b | the fighter's BLOCK INDEX (P1 0, P2 1; copied from `+0x381` at load, `0x020A12`-family), inherited by its spawned objects (`move.b $70(a6),$70(a4)` at every spawner) and XORed for the hit-spark on the victim side (`0x018F48`). Indexes the opponent's recent-hit slots (row above) | [D: 14z-146 static; M: the slot index measured as 0 for P1 throughout the rigs] |
| +0x08.b (attacker) | non-zero SKIPS the recent-hit dedup at `0x0180BC` and the slot write at `0x01826C` (the object-hit path: pool objects carry their own +0x08 stamp). Zero on both fighters through every normals rig | [D: 14z-146 static; consumer of the byte untraced] |
...
| +0x54.b | reaction/hit-state id written by the near-death commit `0x18B34` from attack byte `$17(a3)` (observed 0x11 during a death, 0x05 during regen). **At every contact it is written by the STAGER the record's class `+0x17` dispatches to** — ground `0x018468` / air `0x018510` / KO `0x0185DA`, each handler storing the class itself (copy handler `0x01868C`) or a constant (0x06 and 0x38 both become 6 on the ground, 7 in the air, 8 on a KO); 0xFF on a block (`0x0183FC`); then the reaction table `0x2385C` dispatches on it. Nothing in vsavj writes 0x38 here (engine_internals.md, the reaction-class paragraphs) | [D: 14z-98; M: 14z-169 static + write/read tap, `tests/test_reaction_classes.sh`, `tests/audit_reaction_class_live.sh`] |
| +0x56.b / +0x5A.b / +0x59.b / +0x5D.b | per-contact copies from the attack record: `+0x19` → +0x56, `+0xF` → +0x5A, `+0xC` (hit) / `+0xD` (block) → +0x59, and +0x5D = the victim's FACING resolved by the record's `+0xE` rule (vs2 `0x1717E`); at every contact +0x5D is first SEEDED from the attacker's `+0xB` (vs2 `0x17178`, vsavj `0x1884E`) and the rule is then applied; vs2's rule-5 branch writes it at `0x171E0`, while vsavj's resolver `0x18854` has no rule 5 and XORs it in raw at `0x1886C` (1 XOR 5 = 4, GitHub #159), and the pushback step routine (vs2 `0x27038`) negates each step when +0x5D is 0 and adds it otherwise | [D: 14z-121 readers, static; M: 14z-167 write tap with writer PCs, `tests/audit_facing_rule.sh`] |
| +0x121.b | the CROUCH flag: 1 exactly while Down is held (measured on the donovan_1 rig, 2840-2870, native vsav2 and ours). The guard decision (vsavj `0x0182E8`-`0x0183E4`) tests the VICTIM's `+0x121` before comparing the record's class `+0x17`: flag 0, classes 0x02 0x03 0x38 0x39 take the hit path; flag 1, 0x2C 0x37 0x42 0x48 0x4A 0x4D do (read as lows and overheads — no hit against a guard was run) | [M: 14z-169, `tests/audit_crouch_flag.sh`; the lists: `tests/test_reaction_classes.sh`] |
...
| +0x10A.w | current meter-bar fraction; full bar = 0x90 units -> converts to +1 stock (gauge adder caps/converts here). **THE ATTACKER'S GAIN, MEASURED (14z-146): two kinds of step — the SWING COST on the press frame, 0 / 3 / 6 by button strength whether the move whiffs or lands (= the community workbook's `gauge whiff` column on every move read), and ONE ON-HIT step per LANDED hit equal to the attack record's `+0x14` (6 / 12 / 18 on most normals, 9 on Zabel's 5HK records, 5/4 on Q-Bee's 5HK), on the frame the victim's HP drops.** A multi-window move's total is therefore `+0x14` x the windows that CONNECT, and how many connect depends on the victim's state and the geometry (SA CL.5HK lands 1 of 2 on an idle victor, LE J.HP 3 of 6) — a fact about the situation, not the move | [D: 14z-44, disasm; M: 14z-146 `tests/test_meter_gain.sh`] |
| +0x110.b / +0x111.b | the DARK FORCE CHANGE mode (vsav's P+K; vs2's personal-DF EX): set to 1 by vsavj's activation body `PRG:0x027000` (and by vs2's Change entry `0x02622A`); `+0x111` is what vsavj's meter adder tests to suppress gauge inside the mode (`PRG:0x29A1A`), `+0x110` is cleared when the timer `+0x176` reaches 0 (`0x022526`). vs2's own P+K (Dark Force POWER) never sets them | [M: 14z-168, `tests/audit_df_modes.sh`, `tests/audit_df_meter.sh`] |
| +0x176.w | the Dark Force Change TIMER: 0x70 (112) at activation (vsavj `0x02701A`; vs2's Change entry `0x02623C`), decremented once per PERIOD by the reducer (vsavj `0x022520`, vs2 `0x020ED6`) | [M: 14z-168, write tap, `tests/audit_df_modes.sh`] |
| +0x188.b / +0x189.b | the timer's tick countdown and its PERIOD: every engine tick `+0x188` counts down; at zero `+0x176` loses one and `+0x188` reloads from `+0x189`. vsavj's body sets `+0x189` from the per-character table `0x02704E` (3 for Gallon, Morrigan, Lilith; 4 otherwise, rows 0x10-0x13 included) and `+0x188` = it + 0x1E; vs2's Change entry sets the CONSTANTS 5 / 0x23. So a Change mode lasts 112 x the period (vsavj 297 / 388 frames of `+0x111` at level 6; vs2's EX 478) | [D: 14z-168, disasm vsavj `0x022514..0x022530`, vs2 `0x02622A..0x02624A`; M: `tests/audit_df_modes.sh`] |
| +0x1C3.b | [vs2] the Dark Force POWER flag: 1 throughout vs2's P+K mode, 0 throughout vsav's Change (never set by it). vs2's meter adder tests it (`0x28D6C`); every placed tenant copy of vs2 code that reads it sees "not in Dark Force" on our build — 26 placed reads of it on merged-m18, and 18 placed writes to vs2's Power fields (`+0x1C3/+0x1C4/+0x1C6/+0x1C7/+0x1C8`); the #136 corpus executes 7 of the reads | [M: 14z-168, field_trace both games; static `tests/test_df_field_readers.sh`; read/write tap `tests/audit_df_field_readers_live.sh`] |
...
| +0x147.b | **the VICTIM's INVINCIBILITY TIMER** — the hit test's gate (row above), decremented once per engine tick by the System Timer Reducer `PRG:0x02246E`. ARMED by: every character's own Dark Force seq-0x16 handler in its first sub-state (**the DF STARTUP INVINCIBILITY**, per character: BU/DE 0x29, GA/AU 0x22, VI 0x3B, ZA 0x46, MO/AN/LI 0x3C, FE 0x2E, BI 0x2B, SA 0x05, QB 0x03, LE 0x04, JE 0x7F re-armed to 4; the tenants' OWN vs2 handlers: Huitzil 0x4F, Pyron 0x29, Donovan 0x40 — `tests/expected/df_startup_invuln.tsv`), guard cancels (mizuumi's "Add N frames of invincibility" = `move.b #N,$147(a6)`, e.g. `0x02E19A` #$1E), and — the 14z-42 reading, still true OF vs2 — vs2's electric-shake handler (0x0C per hit, vs2's re-hit PERIOD on that handler: ~10f with it, the victim freeze alone without it). **On vsavj it is NOT the multi-hit dedup — that is the recent-hit slot `+0x6C`, cleared on every gap node (14z-146, row above); vsavj's reaction handlers never write +0x147.** Natively on vs2 the DF path writes 1 (`0x025F2A`), cleared before frame_done — no window | [D: 14z-42 periods; 14z-126 gate + DF arm, measured 21 legs] |
