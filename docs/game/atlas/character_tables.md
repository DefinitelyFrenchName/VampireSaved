...
**DONOVAN'S SIDE, AND THE WHOLE CONFIRM TABLE, MEASURED 14z-161 (GitHub #151 step 3).** The confirm's flavor write is a per-CELL table of two rows, read off the vs2 opcode view: `PRG:0x01F832` — `cmpi.b #$10,$382(a6)`: d0 = 0, `eori.b #1` if bit 7 of `+0x394` (Start held), `move.b d0,$3c2(a6)`; `PRG:0x01F86A` — `cmpi.b #$13`: d0 = 1, cleared if Start held, the same store at `0x01F87E`. No other cell writes the byte; the select-entry clear `PRG:0x01F5C8` leaves 00. So a forced pick gets the flavor wrong in exactly two shapes — Phobos poked over Donovan's cell (reads 01) or Donovan poked over any other cell (reads 00) — and gets it RIGHT by construction for Phobos over any non-Donovan cell, which is the shape every `tests/hui/` gate uses (no P1 cursor moves: Demitri's cell). The readers, by form (`tools/audit_latch_readers.py`, frozen by `tests/test_latch_readers.sh`): Phobos's four (`0x026322`, `0x02595A`, `0x02598A`, `0x025B66`) and Donovan's three (`0x05A650` in the shared per-character code family — `moveq #$43,d0; tst.b $3c2(a6); beq; moveq #$5E,d0` — and `0x065FE2` / `0x066020` in x065e5a, an effect variant chosen off the owner's block). Under the tap (`tests/audit_latch_reads.sh`) Phobos reads it every match frame; Donovan's readers fired on none of his tapped rigs, so the tree's Donovan-over-Demitri and Donovan-as-victim legs measure the game as far as any measured read goes, and the census `tests/test_poked_legs.sh` carries each such leg with the tap that licenses it. **vsavj never writes the byte** — the port's init shim does, after the pokes — so no OURS leg can carry a cell's flavor.
