...
- **`xjtcore.sh` retries until a seed passes — "the build passed" is NOT "the design closes timing"** — `jtseed 4` loops `--seed $RANDOM` and breaks on first success. `cps2w` failed 2 of 4 seeds (-0.545..+0.067, straddling zero) while the control passed both. Also: a FAILING seed still emits an `.rbf`, and a sweep overwrites `release/`. Verify the hash before flashing. [platform](platform/gotchas.md)
