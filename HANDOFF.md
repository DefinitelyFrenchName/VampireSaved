...
**Read `docs/project/mister_core.md` FIRST** — the synthesis: what is TRUE
...
bits are TRANSPOSED (Left and Down swap, measured against the game's own
`$FF8058` mirror), so the tenant-picking replay moved the cursor onto Victor.
See `docs/platform/mister.md` "THE SIMULATED JOYSTICK'S DIRECTIONS ARE
TRANSPOSED". The EIGHTEEN fork commits are `b9d0565` (the `cps2w` cfg scaffold), nine
...
eliminations.
    ...
    ROMDIR=... JTSIM_SCRATCH=/tmp/vampire-saved-jtsim \
      tools/run_sim_jtcps2.sh tests/replays/11_pick_donovan.rpl /tmp/out \
        --core cps2w --wide build/m3b_merged13 --frames 4000 \
