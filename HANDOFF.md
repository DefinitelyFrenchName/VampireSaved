...
| `tests/test_checkskills.sh` | ci_portable (~1 s) | **THE SKILLS ARE LOCKED TO THE DOCS (14z-114): the MiSTer pair `[MSC]`/`[MSV]` and the CPS-2 pair `cps2-hardware` `[CPH]` / `cps2-emulation` `[CPE]`, table-driven per prefix.** `tools/checkskills.py`: every `- [PFX-N]` rule in `.claude/skills/*/SKILL.md` is anchored exactly once (`**[MSC-N]**` at the doc paragraph it distils) and every anchor has a rule; the level-1 skill names nothing game-specific (`mister_scope.md` §1); every number a skill quotes appears in a LOG (`platform/mister.md`, `mister_map.md`, `mister_fit.md`, `mister_field.md`, `release_format.md`, the gotchas, `BITSTREAM.txt`) and never only in the synthesis. Cross-references `[PFX-N]` between skills must name a defined rule. Extractors self-tested; six must-fire controls on a perturbed copy (unanchored rule ×2, stripped anchor, a game name in level 1, an uncited number, a dangling cross-reference). **Editing an anchored paragraph: keep the marker with the fact, or move the rule** |
