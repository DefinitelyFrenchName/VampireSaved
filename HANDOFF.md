...
| `tests/test_move_naming.sh` | **emulator (MAME, ~2 min, eight legs in parallel)** | **THE MOVE LIST'S CHAIN IDS ARE WHAT NATIVE VS2 ENTERS (14z-120, phase 1 naming step):** `tools/name_moves.py gen` regenerates the eight Donovan rigs (`tests/replays/naming/donovan_[1-8].{rpl,json}` must match), runs them on native vs2 with P1's `obj+0x1C` sampled per frame, maps every pointer onto `anim_nodes.py`'s graph and compares each event's entered-chain list to `tests/expected/move_naming_donovan.txt`; every `table:seq` in `build/manifest/moves_donovan.toml` must have been entered; positive controls Blizzard HP = vs2 `0x283E58` (replay 59) and Lightning ES = `0x284A64` (replay 56); negative control: a swapped line fails the compare. Run after any change to the move list's seq ids, the rig schedule, the decoder or the vs2 extract |
