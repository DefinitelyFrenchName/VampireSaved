...
tools/run_wide.sh build/don_m5 fbneo       # or the solo builds (hui40,
                                           # pyron25); ... mame
                                           # (dirs reused across m5->m6
                                           # era — the registry row names
...
+ the medallion fix (Pyron wheel pal_row 0x1A->0x1D; Donovan's P2-hover
portrait owns 0x1A by vs2-heritage attr — merged-only collision,
maintainer-confirmed captures both directions). See the registry rows. Previous batch (14z-86, THE M5 VOICE BATCH):
