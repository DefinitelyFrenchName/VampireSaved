...
| **THE CAPTURE-GEOMETRY SHEET (14z-143)** | `tools/capture_sheet.sh <attacker_hex> <victim_hex> [out.png]` — ours vs native `vsav2` as NUMBERS and as a PICTURE, for any (attacker, victim) pair. It dumps both legs, reads the per-frame victim offset, and photographs each leg on the frame it shows the SAME KEYFRAME: the two games do not run the same video frames per engine tick, so same-frame snapshots compare different keyframes and invent a difference ([VSP-136], [VSE-83]). `BEFORE=<build>` adds a third column for a pre-fix build, drawn at ours' frame numbers because a differing geometry has no matching keyframe by definition. A leg that never made the hold is VOID and refused, not drawn ([VSP-170]). It is an INSTRUMENT, not a gate — the verdicts are `audit_capture_matrix` (all 640 cells, static) and `audit_pyron_capture_block` / `audit_don_grab_pose` / `test_hui_grab_victim` (behavioural). Built because the maintainer twice required captures before accepting a capture-geometry finding and the rig was twice thrown away | live |
