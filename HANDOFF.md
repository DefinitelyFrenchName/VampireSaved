...
| Vanilla standing-normal JOIN rig (14z-125) | `tools/vanilla_join_rig.py gen\|analyse` | which chain each character's standing normals enter, MEASURED at two distances on vsavj from `+0x1C`. 180 frozen rows; it OVERTURNED a fixed even/odd close/far model that had been inferred by fitting our numbers against the workbook being checked |
