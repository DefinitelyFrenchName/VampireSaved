...
| `tests/test_checkskills.sh` | ci_portable (~1 s) | **THE MiSTer SKILLS ARE LOCKED TO THE DOCS (14z-114).** `tools/checkskills.py`: every `- [MSC-N]`/`- [MSV-N]` rule in `.claude/skills/{mister-cps2-wide-core,mister-vampire-saved}/SKILL.md` is anchored exactly once (`**[MSC-N]**` at the doc paragraph it distils) and every anchor has a rule; the level-1 skill names nothing game-specific (`mister_scope.md` §1); every number a skill quotes appears in a LOG (`platform/mister.md`, `mister_map.md`, `mister_fit.md`, `mister_field.md`, `release_format.md`, the gotchas, `BITSTREAM.txt`) and never only in the synthesis. Extractors self-tested; four must-fire controls on a perturbed copy (unanchored rule, stripped anchor, a game name in level 1, an uncited number). **Editing an anchored paragraph: keep the marker with the fact, or move the rule** |
