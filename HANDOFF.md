...
|---|---|---|
| **pyron17 — THE BLINK FIXED EVERYWHERE (14z-75 round 2, NOT frozen, THE build to playtest)** | fingerprint `5dc6da060005ab57897f7163ff8e7b3e5632953c` | `build/pyron17`. = pyron16 + TWO more one-word repoints of the same dead row. pyron16 fixed only the IN-MATCH dispatcher; the maintainer's playtest found the blink alive on the SELECT screen and the between-fight ROUTE MAP. There are THREE aliased per-character palette tables, not one: `0x2A8A4`/`0x2B650`/`0x73790`, rows 0x11 patched at `0x2A8C6`/`0x2B672`/`0x737B2` to vs2's own `0040`. Resolver counts now `0x2AD82`=0 and `0x2B7E8`=180 — exactly native's, and exactly Huitzil's (was 581/523). Delta from pyron16: exactly 4 bytes. Legacy replay 02 bit-identical. New gate `tests/test_variant_dispatch.sh` sweeps for the SHAPE and catches all three on pyron15. Run it: `tools/run_wide.sh build/pyron17 fbneo`. OPEN: effect palette, win QUOTE. |
