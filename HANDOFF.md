...
|---|---|---|
| **pyron17 — THE BLINK FIXED EVERYWHERE (14z-75 round 2, MAINTAINER-CONFIRMED, not yet frozen)** | fingerprint `5dc6da060005ab57897f7163ff8e7b3e5632953c` | `build/pyron17`. = pyron16 + TWO more one-word repoints of the same dead row. pyron16 fixed only the IN-MATCH dispatcher; the maintainer's playtest found the blink alive on the SELECT screen and the between-fight ROUTE MAP. There are THREE aliased per-character palette tables, not one: `0x2A8A4`/`0x2B650`/`0x73790`, rows 0x11 patched at `0x2A8C6`/`0x2B672`/`0x737B2` to vs2's own `0040`. Resolver counts now `0x2AD82`=0 and `0x2B7E8`=180 — exactly native's, and exactly Huitzil's (was 581/523). Delta from pyron16: exactly 4 bytes. Legacy replay 02 bit-identical. New gate `tests/test_variant_dispatch.sh` sweeps for the SHAPE and catches all three on pyron15. **Maintainer playtest CONFIRMED: no blink in character select or the route map, all graphics clean, no regression surfaced.** Gate battery on this build: variant_dispatch, pyron_blink, tenant_hud, pyron_cosmo, audit_empty_tiles, gfx_layout3, list_type_census, m3a_reproducible — all PASS. Run it: `tools/run_wide.sh build/pyron17 fbneo`. OPEN: effect palette (unconfirmed whether visible), win QUOTE. READY TO FREEZE (expectation set + registry row) on the maintainer's word. |
| **pyron16 — HUD ART + the IN-MATCH blink fixed (14z-75, superseded by pyron17)** | fingerprint `bd837784d2fe56709296dab0a60ce77c84b29876` | `build/pyron16`. = pyron15 + ONE WORD: `palette_routine_row_11` (`0x2A8C6` `008E -> 0040`). The sprite/HUD blink was a DEAD ROW — the per-character palette-routine table at `0x2A8A4` aliases rows 0x10-0x1F onto 0x00-0x0F, so id 0x11 ran row 0x01's animated-palette handler where vs2's row 0x11 is the default no-op. Verified: spurious resolver calls 581 -> 0, palette row 10 constant and bit-identical to native, mugshot pixel-identical across consecutive frames. Legacy: replay 02 bit-identical to pyron15; whole-romset delta is exactly those 2 bytes; `audit_id_writers.sh` re-run PASS. Gates: `test_pyron_blink.sh` (now `fixed`), `test_tenant_hud.sh`. Run it: `tools/run_wide.sh build/pyron16 fbneo` (or `mame`). OPEN: effect palette, win QUOTE. |
| **pyron15 — + HIS HUD ART (14z-75, superseded by pyron16)** | fingerprint `3fb71586ed185c245156a8a19431189cb1ffa758` | `build/pyron15`. = pyron14 + the HUD art half: the placer is `place_variant_slot_<name>` in `build/manifest/effect_tail.json` (consumed by `build_gfx_donovan.py`; `check_tenant_hud.py` is the GATE, not the placer, and its `TENANTS` dict gained the 0x11 row). His plate now reads **"Pyron"** and his mugshot draws his own art; both were BLANK on pyron14. Anchors are his OWN — name `0xBE94`, mug `0xBE9C` — not H's, which would collide on the M3b merge; all six cells blank in pristine vsav and inside the audited free pool. Byte-attributed against pyron14: exactly TWO program bytes (the poke values) and exactly the SIX anchor tiles. Gates: `test_tenant_hud.sh` PASS incl. runtime staging via the new wheel-pick replay 40, `audit_empty_tiles`, `gfx_layout3`, `audit_romset_identity`, `m3a_reproducible`; all three frozen references rebuild bit-exact (donovan-m3a, m5_stock, and huitzil-m2 checked explicitly because `effect_tail.json` is shared). Also fixed: `pyron.toml` had not PARSED since fcfe5c7 (a comment line lost its `#`), so pyron14 could not be rebuilt from the tree. OPEN: the BLINK (now root-caused and frozen by `test_pyron_blink.sh` — see the gate row), the effect palette, the win QUOTE. Rebuild: `TENANT_MANIFEST=build/manifest/pyron.toml TENANT_CHAR=0x11 GEN_FLAGS="--profile cps2-wide-v1 --allow-plausible --tripwire-open" tools/build_donovan.sh 6 build/pyronNN` |
| **pyron14 — PYRON RENDERS (14z-74, superseded by pyron15)** | fingerprint `34f4b77d4ae9f2661004320ef36f00be9bd7ad79` | `build/pyron14`. The first Pyron build with content: stage 6 unlocked for 0x11, his real art (band 0x4ED5-0x8647, DELTA 0, no remap), select family + wheel, sprite palettes, win screen (position + palette), and three maintainer-confirmed gameplay fixes — the Cosmo Disruption CRASH (dead sub-state row 81 repointed, one word), the air 214+P RUNAWAY (a DATA table inside his crypt-re-encrypted code region, new `data_in_code shape=pointer-inline`), and his palettes. OPEN: the sprite/HUD BLINK (palette row 10 animated by the palette-seq uploader; his anim nodes are correct), HUD ART not placed (plate renders BLANK — the per-tenant HUD config has no 0x11 row), the EFFECT palette (deferred: 16-row table, variant id spills into a vanilla row — a hazard huitzil-m2 shares), and the win QUOTE (the shared variant-id fold). Legacy = the ratified select-wheel window only; frozen references bit-exact. Rebuild: `TENANT_MANIFEST=build/manifest/pyron.toml TENANT_CHAR=0x11 GEN_FLAGS="--profile cps2-wide-v1 --allow-plausible --tripwire-open" tools/build_donovan.sh 6 build/pyronNN` |
